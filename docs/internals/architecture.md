...
这篇讲 **surfclam 在一台机器上跑起来之后，进程里到底是什么形状**：谁先启动、
...
dsh web 起来
  └─ 加载插件树
       └─ clam-app（一个普通的 cordis 插件）
            ├─ 写 endpoint 发现文件
            ├─ 需要时构建壳
            └─ open Surfclam.app --clam-endpoint <本次的地址>
                 └─ App 连回这个 dsh，拉 snapshot，编译并装载各插件的 Swift 载荷
```

**App 是 dsh 的客户端外设，不是宿主。** 这一条不是风格选择，它换来两样东西：

- **壳源码不是特权目录。** 它只是 `clam-app` 这个插件的一份载荷（`clam-app/host/`），
  与 `clam-sidebar/swift/` 之于 clam-sidebar 是同一种关系。仓库里因此没有
...
（形状见 `clam-bridge/lib/plugin.js` 的 `CommandDeclaration`）；
...
一份声明喂四样东西：菜单项、默认键位、⌘/ 面板、以及 clam-app 现拼出来的
`clam-shortcuts` 设置 schema。从前这四样在壳里各硬编码一份，分家了不报错。

---

## 3. `clam-app/host/Sources/` 的分工

| 目录 / 文件 | 是什么 |
|---|---|
| `ClamSDK/` | **壳 ↔ 插件的 ABI 词汇**。编成一个 dylib 随 bundle 分发，壳与所有插件链接同一份 |
...
- **`WebPolicy.swift`** —— WKWebView 的导航策略与下载。**它归壳不归插件**，
  因为下载与外链是"逃生舱也得有"的能力：clam-layout 缺席、整窗网页兜底时，
...
### 4.2 `ClamContributions`：多占用的"贡献槽"
...
`clam-layout/swift/LayoutContracts.swift` 的 `ToolbarSpec`——有类型的属性加一个
...
`ClamEventBus` 是进程内广播，载荷 `[String: Any]`，约定只放 JSON 能表达的值
...
## 7. 共享 module：全进程只有一份 ClamSDK

壳链接 ClamSDK，运行时编出来的每一个插件 dylib 也链接 ClamSDK。**必须是同一份文件**，
类型身份才对得上——否则 `as? ClamPlugin` 会安静地失败。所以它不走 SwiftPM 静态链接进壳，
而是编成 dylib 随 bundle 分发：

```
scripts/build-modules.sh   → host/build-sdk/libClamSDK.dylib + .swiftmodule + .swiftinterface
scripts/embed-modules.sh   → Contents/Frameworks/libClamSDK.dylib     ← 壳按 @rpath 加载
                           + Contents/Resources/ClamModules/          ← 插件编译时 -I 的落点
```

两个脚本挂在 `project.yml` 的 pre/postBuildScripts 上，源码没变则秒过。
机制本身是多 module 的（`build_module` 再加一行即可），眼下只剩 ClamSDK 一个住户。
插件只拿到 `.swiftinterface`，这已经足够编译——SDK 编译时带 `-enable-library-evolution`。

**改 ClamSDK 会让所有插件的 contentHash 失效、全量重编。这是有意的。**
编译指纹里含 `ClamSDK.swiftinterface` 的摘要（`CompilerService.toolchainFingerprint`），
...
必须算出同一个数），而信号并没有丢——换工具链必然重编 ClamSDK、必然改写这份 interface。
...
**用户缓存 → bundle 内预编译（`Resources/ClamPlugins/`）→ 现场 swiftc**。
...
装载走 `dlopen(path, RTLD_NOW | RTLD_LOCAL)` + `dlsym(image, "clam_plugin_entry")`。
...
contentHash**（算法在 `clam-bridge/lib/swift-payload.js` 的 `swiftContentHash`，桥每轮重扫时按拓扑序应用一遍）。于是"内容没变就不重编"
...
| 没人占 `root` 槽（clam-layout 缺席 / 被禁用 / 编译失败） | 壳露出**整窗 WebView**——仍是一个能用的 dsh 客户端，只是没有原生外壳 |
...
逃生舱那一档还有一个细节：`ShellRootView` 的兜底 WebView 与 clam-layout 借用的
**是同一个实例**（放在 `ClamObjects` 保管箱里），所以在"插件模式 ↔ 逃生舱"之间
来回切换时页面不重载、JS 状态存活。
...
| 插件 | 一句话 |
|---|---|
| `clam-app` | 宿主插件。壳源码是它的载荷：写 endpoint 发现文件、按需构建壳、拉起 App、provide `clamApp` 服务、把 `commands` 拼成 `clam-shortcuts` 设置面 |
| `clam-bridge` | **唯一的特权插件**。Swift 载荷登记表 + `/clam/bridge` WebSocket + 500ms 盯各 `swift/` 目录。`createSwiftPlugin` 工厂也住这儿 |
| `clam-layout` | 占 `root` 槽：分栏、WebView 排版、开出 `sidebar` 槽与 `toolbar` 贡献槽。client 半边装 `window.__clam` 动作桥并收起 web 侧边栏 |
| `clam-sidebar` | 占 `sidebar` 槽：原生会话侧边栏。**数据面在 node 半边**（订宿主服务与事件，投影经桥推 JSON；Swift 只管画和发动作） |
| `clam-notify` | 桌面通知。不占槽、不贡献界面，缺席即无通知。同时是"有什么在等着你"的**唯一真相**，经 `clamPending` 服务供给侧边栏那个置顶的「待处理」分区 |
| `clam-settings` | 原生设置窗口。不占槽、自己一扇窗；数据面在 dsh 进程里直接消费 `ctx.settings` / `llm` / credentials / `agentPresets` / `pluginInventory` |
| `clam-nativeify` | 让 dsh Web UI 摸起来像原生 App。三半边，主力是 client 那段 CSS；薄 Swift 载荷把 dsh 的 `ui-theme` 投给原生侧（`NSAppearance` + 窗口底色） |
| `clam-memory` | 跨会话持久记忆。**纯 node、零 macOS 依赖**——不占槽、不贡献界面、不碰桥，装到任何一台有 dsh 的机器上都跑得动 |

三条模式贯穿这张表：

- **数据面尽量放 node 半边。** 壳与共享 module 随 app bundle 冻结、用户改不了，
  而 dsh 的 wire 模型是演进最快的那一层——层放错了就得靠发新版 App 来跟进。
  clam-sidebar 与 clam-notify 都是这么分的，两者的 dsh 交互也都关在**一个文件**里
