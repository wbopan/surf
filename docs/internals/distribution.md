...
这份文档写给**本仓库的维护者**：讲 surf 的分发形态——App bundle 里装着什么、
...
```
Surf.app/Contents/
  MacOS/surf                       壳（预编译，Developer ID 签名）
  Frameworks/libSurfSDK.dylib          共享 module 的运行时那一份（壳按 @rpath 加载）
  Resources/
    SurfModules/                       SurfSDK 的 .swiftmodule / .swiftinterface / .swiftdoc
                                       （运行时编译插件时 -I 的落点）
    SurfNode/<pkg>/                    各插件的 node 半边：package.json + lib/ + swift/
    SurfPayload.json                   载荷清单 + 跳过判据（含 version 与内容 hash）
    SurfPlugins/<Module>/prebuilt/<hash12>/lib<Module>_h<hash12>.dylib
                                       预编译的 Swift 半边（**原地用，不进 profile 镜像**）
    SurfPrebuilt.json                  预编译清单（给人看的，没有程序读它）
```

写它们的是三个 postBuild 脚本，**顺序固定，每一步吃上一步的产物**：
`embed-modules.sh`（`Frameworks/` + `SurfModules/`）→ `pack-payload.sh`（`SurfNode/` +
`SurfPayload.json`）→ `prebuild-plugins.sh`（`SurfPlugins/` + `SurfPrebuilt.json`）。
后两个**只在 Release 跑**，Debug 下直接 `exit 0`——开发形态里 node 半边从仓库 link、
Swift 存盘热替换，打进 bundle 的那份没有任何人读，纯粹拖慢 `./dev` 的循环。

### 1.1 `SurfNode/` 的两条不变量

**目录结构与仓库根一一对应。** surf-\* 之间用相对路径 import
...
装哪些包**以 `surf/cordis.patch.yml` 为准**（编排表是唯一真相），注释掉的行不算。
module 名**不自己算**，用 `surf-bridge/lib/module-name.js`——桥登记时算出的名字与预编译
...
载荷里还夹一个 `swift/.surf-static` 标记：「这份源码是分发载荷，进程活着的时候不会变」。
...
唯一权威，`SurfNode/*/package.json` 构建时全部对齐，⌥⌘D 显示的也是它。不需要"壳与插件
...
App 首次打开时把 `Contents/Resources/SurfNode/` 拷进
`~/.dsh/profiles/surf/.surf/`，并手写 `link:` 行与符号链接。实现是
`surf-app/host/Sources/Native/ProfileBootstrap.swift`，**只在 Release 壳跑**
（Dev 壳的 profile 由 `./dev` 自己备，link 的是仓库源码）。

`surf` 这个 profile 名**归安装形态专属**：开发一律带后缀（主 worktree
`surf-dev`，侧 worktree `surf-<目录名>`），于是"装好的那套日常用 + 主 worktree
...
`.surf/` 里，向上查找**天然经过**那个目录。拷贝还额外买到三样：`.app` 是只读的而
...
5. **符号链接**——`<profile>/node_modules/@wenbo/<pkg>` → `../../.surf/<pkg>`。
...
**构建那一整套代码不随包分发。** 它住在 `surf-app/host-build/`（`index.js` +
`source-hash.js`），而 `surf-app` 的 `files` 白名单只收 `lib/`，`SurfNode/` 载荷也只拷
...
import 得到吗"**（`surf-app/lib/index.js` 顶层的 `hostBuild` 常量）。拿不到就是这份
surf-app 没有构建能力：`build` / `watch` / `restartOnRebuild` 一律关掉，只剩"探测既有
产物 + 写 endpoint 发现文件 + provide `surfApp`"。只有"模块不在"
...
曾经的环境变量 `SURF_RELEASE` 就是那种开关，缺了它 surf-app 会按 dev 形态跑，构建 Debug
产物、再拉起一个 Surf Dev——于是双击 `/Applications` 里的 Release 却多出一个 Dev
...
`isRunning` 跳过；而用户手动 `dsh --profile surf` 时，把装好的 App 带起来正是他要的。
...
2. **bundle 内预编译** `Contents/Resources/SurfPlugins/<Module>/prebuilt/<hash12>/`
3. **现场编译**（写回用户缓存）
...
`surf-bridge/lib/swift-payload.js` 的 `swiftContentHash`（与桥同一份代码）；壳那半边的
hash 与全部 swiftc 参数交给 `scripts/prebuild/` 那个工具，**它把壳自己的
`Native/CompilerService.swift` 原样编进去**，不是第二个编译器；`swiftDeps` /
`sharedModules` / `schemaVersion` 靠 `import` 各插件的 node 半边读 `surfSwift`
...
`SurfSDK.swiftinterface` 摘要承担——**真相取自随 bundle 走的文件，不取自本机环境**，
...
眼下有六个：一个 `libSurfSDK.dylib` 加五个插件 dylib。两处都在构建过程中就地签好，
身份都取 Xcode 解析出来的同一个 `EXPANDED_CODE_SIGN_IDENTITY`：`libSurfSDK.dylib` 由
...
`xcodegen generate` 每次都会照 `targets.surf.entitlements.properties` 那张表重写
`Sources/surf.entitlements`——**手改那个文件是静默丢失的**。
...
声明各自的代表性产物就够了：三个 postBuildScripts 各一条 `libSurfSDK.dylib` /
`SurfPayload.json` / `SurfPrebuilt.json`。验的时候**别用 clean build**（那必然重签、
...
|---|---|---|
| `PRODUCT_NAME` | Surf Dev | Surf |
| Bundle ID | `io.wenbo.surf.dev` | `io.wenbo.surf` |
| 图标（`APP_ICON_NAME`） | `AppIconDev`（橙色 DEV 徽章） | `AppIcon` |
| 位置 | `surf-app/host/build/Build/Products/Debug/` | `/Applications/` 或 `build-dist/` |
| 载荷与预编译 | 无（脚本 `exit 0`） | 有 |
| profile 自举 | 不跑 | 跑 |
| profile | `surf-dev` / `surf-<worktree>` | `surf` |
...
产物里 `#if DEBUG` 整块都不存在。`<AppSupport>/io.wenbo.surf/` 是两者共用的（endpoint
发现文件由后端写，两个构建都要读到同一份），所以那底下的东西都按实例分片，
详见 [`connection.md` §5](connection.md)。

`./release` 装机走 `scripts/build.sh`，收尾会写下 surf-app 认的那份源码 hash marker
...
壳侧同理：surf-app 拿不到产物时只留一句「未找到可用的 `<配置>` 产物，surf-app 优雅缺席
...
`./release` 装完会查一句 `Contents/Resources/SurfNode/` 在不在，**只警告不拦**：缺了它
App 自举不出插件——症状是"起来了，但界面上什么原生东西都没有"。排错的落点是
`<AppSupport>/io.wenbo.surf/logs/surf-app-build.<profile>.<配置>.log`。
...
- **卸载拦截不到**：用户把 App 拖进废纸篓时，`~/.dsh/profiles/surf/` 与
  `<AppSupport>/io.wenbo.surf/` 都会留下。会话与设置是 dsh 的资产，一律不动；
