...
> 权威计划是 `docs/archive/web-header-native-match-plan.md`。相关执行日志原样保留。
...
- **ABI 入口**:每个插件 dylib 导出 `@_cdecl("dash_plugin_entry")`,返回指向 `DashPlugin` 实现的不透明指针
  (`Unmanaged.passRetained`;确切签名 M2 spike 定稿后写进 `docs/extend/native-abi.md`)。
...
- 拉起与构建归 dash-app(§3-3、§7.5),本插件纯通信;M4 交付:
  ...
  4. 读路径(pull 模型,桥对客户端零状态):`snapshot` 请求 → 按拓扑序(由各插件 inject/swiftDeps 排序)返回
     ...
     **级联重编是硬约束**(M2 断言 6 实测,见 `docs/extend/native-abi.md` §4):上游 `contentHash` 变化时,
...
### 6.5 M2 spike 必须验证的断言清单(逐条打勾进 `docs/extend/native-abi.md`,不许抽象带过)
...
- **形态**:右键菜单是操作全集(macOS Finder/Mail/Xcode 的老规矩);列表级别的新增
  (添加工作区)落在列表区段头「工作区」那一行的右端——与 web 的 `sectionHeader`
  同位,新增什么就贴着什么的表头。web 的 hover ⋯ 汇总按钮不跟。
  **修订(2026-08-26,用户):"行上一个 hover 图标都不留"矫枉过正了**——
  M11 第一批把会话行的归档和分组头的加号一并删掉,结果两个高频动作只剩右键一条路。
  改回:右键菜单保持全集不变,另给这两个动作各留一个 hover 图标
  (会话行尾 `archivebox`、分组头 `plus`),`.opacity` 显隐、槽位恒定占位,
  列表静止时仍然干净。低频动作(重命名/分叉/删除工作区)仍然只在右键里。
- **分组头不吃 accent 色**(同日修订):`containsCurrent → Color.accentColor`
  让含当前会话的工作区图标染蓝,读起来像"这个分组被选中了";选中态是会话行的事
  (List 自己画高亮),分组头不掺和。同时分组头标题显式扳回 `.primary`——
  sidebar List 给 Section header 默认涂 secondary,而工作区是用户维护的账
  (能改名能删),涂灰看着像被禁用。
...
| M2 | ABI spike(独立 scratch 工程) | ✅ **已完成** `docs/extend/native-abi.md` + `docs/spikes/m2-abi/`(可复跑) | ✅ §6.5 十条断言全通过（runner 14 项检查）,全链跑通,**R1 解除、不走 Plan B**;附带修正:编译比预期快一个数量级(§7.1 预算)、级联重编成硬约束(§5.1) |
...
|---|---|---|---|
| 2026-08-25 | M2 | `5361621` | **提前于 M1 执行**(§9 允许并行,且它是唯一证伪点)。§6.5 十条断言全通过,R1 解除。就地更新:§5.1-4(级联重编硬约束)、§6.1(dlopen 不需拓扑序、RTLD_LOCAL 必需)、§7.1(门控预算 60s→10s)、§10(R1 解除/R7 不成立)、§9(M2 行)。产出 `docs/extend/native-abi.md` + 可复跑的 `docs/spikes/m2-abi/`。|
...
| 2026-08-26 | (新表面) H0～H5 | (本次) | **主内容区 header 原生化**,新增第六个插件 `@wenbo/dash-header`(1568 行),路线由 `docs/native-header-plan.md` 单独承载。核心决定:**不占 `conversation.session.header` 槽**(路线 A 出局,三条源码证据见该文 §1),改为"web header 折叠 + 工具栏贡献"。`toolbar` 贡献槽因此开出 `region`(sidebar\|content)与 `sizing`(fixed\|dynamic)两维,**两维都缺省成老行为,现有贡献一个字没改**——这是通用能力,不是给 header 开的后门。五条贡献:面包屑 / Chat\|Trajectory 段控 / mode / 后台任务 / 导出。**两条通道同时用**:active view 走页内桥(真相在浏览器进程),session/preset/jobs 走 node 半边 `lib/dsh-source.js`(真相在 dsh 进程)——判据是"这个事实的真相住在哪个进程"。**回退过一版**:曾做 `dash.layout.contentTopInset` 让内容避开标题栏,用户拍板不要(内容从工具栏底下穿过正是 macOS 26 想要的),整体撤销,壳与 dash-layout 因此零改动;顶端切字改由 client 半边一条 `padding-top` 解决。就地更新:`native-header-plan.md` §3(锚点/投影名/折叠粒度三处更正)+ **新增 §6 实施记录**(九条实测)、CLAUDE.md(六插件、槽的两维、五条新踩坑)。**实测澄清**:调研文档给的 `[data-phase] > header` 锚点是错的(中间隔着 slot outlet),正解是 `[data-slot="conversation.session.header"]`;`pointer-events:none` 不挡合成事件,所以折叠掉的 tab 照样点得动。|
| 2026-08-26 | (补齐) 子代理 catalog | (本次) | **原生重画 subagent catalog**，补上上一条里漏掉的 `conversation.session.header.lineage` 槽。上一版 `scope:"full"` 折叠整条 header 时把该槽一起折没了,而**子代理会话不进侧边栏**(上游 README:parent header catalog 是它们唯一的导航入口)——等于砍掉了访问子代理会话的全部路径,是功能倒退。新增 `swift/SubagentCatalogView.swift`(树/展开/键盘导航)与 `swift/HeaderFormatting.swift`(token 与时长格式,逐字复刻上游),面包屑改成"末段计数下拉 + 子代理段兄弟切换器"。**结构性差异**:上游 client 只拿得到直接 catalog 所以逐层懒加载,而 `session.list` 的契约原话是 "v1 returns everything",node 半边一次就有整棵树(含 `projections.values` 的 title/subagent/subagentTiming/tokenUsage)——**展开是纯本地操作、零往返**。三条实测踩坑,全部写进 `docs/archive/native-subagent-catalog.md` 并回灌 CLAUDE.md:(1) **id 形态混合**——`subagents.list` 要 parent 带 `session-` 前缀、回的 child 却是光 uuid,同步翻转两个恰好跳过唯一正确的组合,症状极像"数据坏了";(2) **`openSubagent` 有前置状态**——它校验 "healthy catalog child",认的是 runtime 自己那份 `subagentsByParent`,不先 `refreshSubagents`(返回 Promise,await 它才是可靠信号)就导航一律被挡,所以上游 `setCatalogOpen` 的作用不止文档写的那条;(3) **`ctx.inject` 会抛**,裸调赔掉整个 client 半边,而 node 终端一个字都没有。端到端实测通过:计数触发器 → catalog → 进子代理 → 兄弟切换器 → 跳到另一个兄弟,正文从"伦敦今天"变"伦敦明天",全程侧边栏无选中项。**未实测**:hover 150ms 打开与键盘导航(要抢占物理光标/键盘焦点,会打断正在用机器的人)。|
| 2026-08-26 | (定稿) header 布局 + 工具栏原生化 | (本次) | **按设计定稿把 header 排成「标识靠左·正中留空·段控与动作靠右」,并把五格里的四格换成原生 `NSToolbarItem` 子类**(路线与实测记在 `docs/native-header-plan.md` §7)。① `toolbar` 贡献槽再开三维:`align`(leading\|trailing,两组之间夹一个 `.flexibleSpace`)、`spaced`(本项之前插一个 `.space`——**空隙就是 AppKit 的分组语法**,macOS 26 把相邻项合成一枚玻璃胶囊,断开才另起一枚)、`kind`+`items`+`priority`(选 `NSToolbarItem` 子类)。全部缺省成老行为。② 段控→`NSToolbarItemGroup(images:selectionMode:.selectOne,labels:)`+`.expanded`;mode 与后台任务→`NSMenuToolbarItem`(后者带 `NSItemBadge.count(n)`,macOS 26);导出→`NSToolbarItem`+symbol。**用 `images:` 而不是 `titles:`** 是关键:这样 Icon Only 显图标、Text Only 显 `labels`,"给不给文字"真的交回给了系统。只有面包屑还走 `NSHostingView`——它挂着一棵可展开的子代理树,菜单表达不了。SwiftUI 那四个视图(`HeaderTabsView`/`HeaderModeView`/`HeaderJobsView`/`HeaderExportView`)整体删除。③ **拓扑与流量分家**:metadata 一变就重建整条工具栏,所以徽标/菜单/选中态/显隐改走活通道 `dash.toolbar.update`,消费方记进 `ToolbarItemState` 并就地改活项(项会因换代/溢出重造,不记账就丢状态);回来的动作走 `dash.toolbar.activate`(group 带 `index`/`itemId`)与 `dash.toolbar.menuSelect`。④ 顶部留白改为跟着 `displayMode` 走:`viewDidLayout` + `displayMode` KVO 双保险量 `contentLayoutGuide`,变了广播 `dash.layout.titlebarMetrics`;另配 `dash.layout.requestTitlebarMetrics` 给后到的订阅者(只在变化时广播,不问就永远等不到——同"补发归请求方"那条纪律)。⑤ **液态玻璃底板补在页面一侧**:macOS 26 的 scroll edge effect 由 AppKit 盯 `NSScrollView` 触发,而 macOS 的 WKWebView 里没有 NSScrollView,那条带子什么都不画(条纹探针实测:带子内外逐像素一致,`titlebarAppearsTransparent=false` 也一样;而工具栏项自己那枚胶囊是真玻璃)。改由 `dash-header/lib/client.js` 的 `[data-conversation-scroll]::before` 画(`position:fixed`+`backdrop-filter`+底边 `mask-image` 软收,底色走 `--dsw-alias-bg-base`)。**实测**:段控切 Chat/Trajectory、mode 菜单开合与勾选态、`NSMenu.performActionForItem` 让 AppKit 自己派发→`selectPreset`→桥→node→dsh 回 `has already started; its agent preset is fixed`(锁定会话被拒是对的,证明写通道整条通)、徽标渲染、Icon and Text 下留白 52→66pt 跟随、冷启动、深浅色、520pt 窄窗溢出成 `»`。**四条新硬事实**:(1) `NSToolbar.displayMode` 在 `allowsDisplayModeCustomization`(macOS 15+ 默认 YES)下**改不动**,存过配置后再赋值会被当场弹回,代码里那句只是开局值,要记住用户选择必须开 `autosavesConfiguration`;(2) `withObservationTracking` 的观察者没人强持有就静默死掉,**只在冷启动露馅**(热替换有保管箱种子,第一推就把该显示的都显示了);(3) SwiftUI 的 `maxWidth` 是贪心的,NSHostingView 把它当理想宽度顶回工具栏,超出内容区就整项被挤进溢出——界面一个字都不剩,症状极像"数据没到";(4) `NSHostingView` 默认压缩阻力 750 会让 AppKit 转而把**别的**项收进溢出菜单。就地更新:`native-header-plan.md` §7(7.1～7.7)、CLAUDE.md(插件树 dash-header 一行、贡献槽指针改写、三条新踩坑、shot.sh 按 pid 选窗口)、`tools/shot.sh` 的 `--app` 加认 pid(多 worktree 并存时窗口名与尺寸一模一样,只有 pid 分得开)。 |
| 2026-08-26 | (续) header 定稿：标识出胶囊 + 只留可操作项 | (本次) | **一条设计原则重排了整个 header**：用户指出"把标题放在胶囊里绝对不可接受"、"按钮才应该设成圆形按钮的样子，不能编辑的东西放副标题"。据此:① 会话标识从 `NSHostingView`（被 AppKit 套了一枚玻璃胶囊）搬到 **`window.title` / `window.subtitle`**——Mail / Notes 那条裸文字，位置正好在分隔线右边、内容区左缘，字体字重截断全归系统。② **锁定后的 agent preset 与只读的后台任务退进 subtitle**，工具栏那两格直接消失:preset 跑过一轮就锁死、jobs 上游自己也只给看不给停,做成按钮等于承诺一件按下去不会发生的事。工具栏因此只剩四格,每格都真能按。③ 子代理入口独立成一格 `NSMenuToolbarItem`(徽标走 `NSItemBadge`),菜单收纳祖先导航 / 兄弟切换 / 子代理进入三种交互,树用 `submenu` 递归(node 半边一次 `session.list` 就有全树,零往返);**带子菜单的项自己点不动**(AppKit 只展开不发 action),所以子菜单第一条得是"打开它自己"。④ 槽新增 `dash.toolbar.menuOpen`(菜单将要打开)做 catalog 预热——`openSubagent` 有前置状态,不预热一律被挡。⑤ 补 `dash.window.requestTitle`:标识和 titlebarMetrics 一样只在变化时推,dash-layout 一换代窗口就卡在没标题的透明态。**液态玻璃那条带子的结论被推翻又收敛**:先前"scroll edge effect 要不到"的理由错了(说是 WKWebView 没有 NSScrollView),真相是**这个效果本来就不是一条带子**——载体是私有的 `NSScrollPocket`,由滚动内容视图自己画,Apple 原话"applied underneath toolbar items"、形状"varies based on the content floating above it"。给它一个真实可见、6000pt 内容、滚到 2600 的 `NSScrollView`,条纹照样原样穿过带子,只有工具栏胶囊底下被糊;`preferredScrollEdgeEffectStyle` 只选软硬不召唤;`obscuredContentInsets` 造得出 pocket 但默认纯色(Soft 要 SPI)、且只认主 frame 滚动。**所以"toolbar 天然有效果"是对的,它此刻就在生效——长在工具栏项的胶囊里。**带子最终仍由页面画(`backdrop-filter` + 一条 `--dsw-alias-border-l2` 的 hairline):`NSVisualEffectView` 采不到 WKWebView 的 remote layer 像素,`NSGlassEffectView` 采得到但自带关不掉的边缘高光。**实测**:标题/副标题渲染、锁定会话 preset 落副标题、顶部留白跟随、子代理菜单结构(两个子代理+两行 detail+徽标2)、id 解码(`parentRaw` 带 `session-` 前缀 / `childRaw` 光 uuid,正是既有踩坑记录里那个混合形态)。就地更新:CLAUDE.md(插件树 dash-header、贡献槽指针、六条新踩坑)。 |
| 2026-08-27 | (修复) 标题栏拖动 | (本次) | **装上 `NSToolbar` 之后标题栏拖不动、双击不放大**,用户报"这个区域不响应窗口拖动"。根因是**两道保险同时失效**:① 壳在 contentView 顶部放了块 `WindowDragRegionView`(40pt,`MainWindowController.swift`)专门补 `fullSizeContentView` 的拖动区,但 **titlebar 容器排在 contentView 之上**,`NSToolbarTitleView` 铺满整个内容区宽度(实测 frame `(260,1025,764,52)`)把它整个遮死——那块视图再也收不到事件;② AppKit 自己也不接手,壳注释里那条老实测是对的:这个窗口形态下 `mouseDownCanMoveWindow` **不生效**(复核:从 `NSThemeFrame` 到 `NSToolbarTitleView` 整条链都是 `true`,照样纹丝不动)。**症状只坏一半因而极易误判**:侧边栏那半照常能拖(title view 从 x=260 起够不着)、最顶 4pt 也能拖。修法归 dash-layout(谁遮住的谁补,逃生舱模式下壳那块本来就好使):`installTitlebarDrag()` 用 `NSEvent.addLocalMonitorForEvents` 只接管 `NSToolbarTitleView` 那块地,向上遍历时先撞见 `NSControl` 就放行(按钮/段控/菜单照常),双击读 `AppleActionOnDoubleClick` 而不是写死 zoom;monitor 在 `deinit` 里自摘(它不随控制器回收,留着就是每换一代堆一个僵尸)。**验证方法本身也踩了坑并记进 CLAUDE.md**:CGEvent 合成事件**送不进不在当前 Space / 非前台的窗口**——光标会动、`AXIsProcessTrusted()` 为真、`osascript` 设 `frontmost` 也返回 `true`,但 `leftMouseDown` 根本不进 App,六个采样点全"拖不动",是**纯假阴性**,判据是在 App 里装 local monitor 看事件到没到。最终结论靠**关掉修复的对照组**钉死:关闭时内容区顶下 14/26/40/48pt 全 ❌、侧边栏与顶下 4pt ✅、双击 ❌;开启后六点全 ✅ 且 Δy 精确等于 60、双击放大生效并自动还原。就地更新:CLAUDE.md(两条新踩坑)。 |
