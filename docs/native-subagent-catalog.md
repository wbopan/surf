# 子代理 catalog 原生化：目标追踪（**实测档案**）

> **2026-08-30：本文记录的原生实现已随 header 插件整个删除。** 主内容区回到 dsh
> 自己的 web header，子代理导航因此又回到上游那份 lineage catalog，下面这个缺口
> 不复存在。**留下这份档案，是因为它是仓库里唯一一处记着 dsh 子代理契约的地方**
> ——数据从哪拿、动作怎么发、四个纯算法、以及三条踩坑（其中两条已回灌 CLAUDE.md）。
> 下文一律现在时，读的时候记住那个插件已经不在了。

> 起因：当年那个原生 header 插件在数据面就绪时以 `scope: "full"` 折叠整条 web header，
...
所以 header 插件的 client 半边在自己那个页内桥全局上加一条 `openSubagent(...)`，
...
它抛了，于是**整个 apply 挂掉**，页面上连那个插件的页内桥全局都没有。
...
`confirmNative` 定义一起吃了。语法检查照过（它仍被页内桥那个全局
引用，只是没定义），**只有页面会报**：

```
Failed to load plugins @wenbo/<那个插件>
