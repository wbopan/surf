...
| 2026-08-26 | (补齐) 子代理 catalog | (本次) | **原生重画 subagent catalog**，补上上一条里漏掉的 `conversation.session.header.lineage` 槽。上一版 `scope:"full"` 折叠整条 header 时把该槽一起折没了,而**子代理会话不进侧边栏**(上游 README:parent header catalog 是它们唯一的导航入口)——等于砍掉了访问子代理会话的全部路径,是功能倒退。新增 `swift/SubagentCatalogView.swift`(树/展开/键盘导航)与 `swift/HeaderFormatting.swift`(token 与时长格式,逐字复刻上游),面包屑改成"末段计数下拉 + 子代理段兄弟切换器"。**结构性差异**:上游 client 只拿得到直接 catalog 所以逐层懒加载,而 `session.list` 的契约原话是 "v1 returns everything",node 半边一次就有整棵树(含 `projections.values` 的 title/subagent/subagentTiming/tokenUsage)——**展开是纯本地操作、零往返**。三条实测踩坑,全部写进 `docs/native-subagent-catalog.md` 并回灌 CLAUDE.md:(1) **id 形态混合**——`subagents.list` 要 parent 带 `session-` 前缀、回的 child 却是光 uuid,同步翻转两个恰好跳过唯一正确的组合,症状极像"数据坏了";(2) **`openSubagent` 有前置状态**——它校验 "healthy catalog child",认的是 runtime 自己那份 `subagentsByParent`,不先 `refreshSubagents`(返回 Promise,await 它才是可靠信号)就导航一律被挡,所以上游 `setCatalogOpen` 的作用不止文档写的那条;(3) **`ctx.inject` 会抛**,裸调赔掉整个 client 半边,而 node 终端一个字都没有。端到端实测通过:计数触发器 → catalog → 进子代理 → 兄弟切换器 → 跳到另一个兄弟,正文从"伦敦今天"变"伦敦明天",全程侧边栏无选中项。**未实测**:hover 150ms 打开与键盘导航(要抢占物理光标/键盘焦点,会打断正在用机器的人)。|
