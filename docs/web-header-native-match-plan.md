...
- 2026-08-29：**§3.0～§3.4 落地**（§3.5 滚动穿透按约定不做）。三笔提交：
  ...
  **落地时改掉的三处**（源码为准）：
  1. **子代理下拉做不成右侧第二枚 36 胶囊**。§3.1 把 lineage 当成 header 的直接孩子，
     实际 dsh 把它放在 `nav._crumbs > span._crumbSeg` 里面——**它是面包屑的一段**。
     要挪到第 4 列就得把 nav 和 _crumbSeg 一起 `display:contents` 摊平，那样多段面包屑
     的每个按钮都变成独立格子、互相叠着。所以顺着 DOM 给成 24 高的内联下拉；
     36 会把标题行撑到 36、加上副标题正好 52，副标题只能贴栏底，中线也就散了。
     顺带摘掉 nav 那条 `overflow:hidden`（截断本来靠 `_crumb` 自己的 ellipsis，
     留着只把胶囊上下裁掉——红框调试时上下两条边直接消失，很好认）。
  2. **导出按钮从 SOLID_BORDERED 换到 SOLID_PLAIN**，并抹掉 dsh 那条 1px border。
     §3.3 说「只改几何」，但三枚胶囊并排时它的墨量是邻居的两倍（1×.1 对 .25×.197），
     边黑一圈。换组之后三枚描边同源。
  3. **末段面包屑 `max-width` 220 → 420**。15px 之下 220 只装得下十来个汉字，
     而右边一整行的 `1fr` 空着。窄窗口下这一列是 `minmax(0, auto)`，先于胶囊让位。

  **§3.0 的一条补充**：末段面包屑是个 `disabled` 的 `<button>`（它是标题不是按钮）。
  放行表若把它算进去，标题那块地就再也拖不动窗口，而点它又什么都不发生。
  上报时按 `:not(:disabled):not([aria-disabled=true])` 排掉。

  **实测（浅色，窗口激活，2x 截图逐像素量）**：
  栏高 52 · 三枚胶囊 y=8–44（高 36）· 共用中线 y=26 · 标题起点 x=16（量到 17，
  首字笔画反锯齿）· 标题墨迹 y=12–25（15px/600）· 标题色恰好 `rgb(54,54,54)` ·
  副标题 11px 灰 · 段控 102.5 宽、导出 36×36、项间距 8。
  窗口失活：标题 `rgb(178,178,178)`、玻璃退成平色（胶囊内部 240，激活时 253）。
  深色：标题走 `rgba(255,255,255,.55)`（失活档，量到 154，与算值一致）、
  段控容器 30、选中段 64。
  点击：`peekaboo click` 打段控的「轨迹」（y≈26，正在壳那条 40pt 拖动条里）
  选中态确实切过去了——§3.0 生效。AX：导出按钮仍是 `[button] Session log`。

  **没验的**：`prefers-reduced-transparency` 回退（本分支的 clam-nativeify 里根本
  没有这一层，计划 §2.4 提到的「减少透明度」那一态是别的分支的东西）；
  空地拖窗只做了代码复核，没做合成事件实测（窗口不在前台时 CGEvent 是纯假阴性）。

  顺手修了 `clam-nativeify/tools/dump-css.mjs`：它在 HEAD 上就已经跑不起来
  （桩里缺 `ctx.inject`，装完效果当场抛），而且两张 style 的 textContent 写同一个
  变量、后写的盖掉先写的——**它其实只在验字体那一层**，玻璃那摞从来没被验过。
