...
surf 的跨插件契约**全是字符串**——槽名、事件主题、保管箱键、metadata 键、
hook 名、命令 id。这是有意的：壳是预编译产物，第三方改不了它，所以任何"把词汇冻进
ABI"的做法都会把生态钉死（判据写在 `SurfContributions.swift` 顶注）。
...
**权威**：`surf-bridge/lib/plugin.js` 的 `CommandDeclaration` typedef。

菜单项、默认键位、⌘/ 面板、`surf-shortcuts` 设置页，**四样东西一份声明**。
...
createSwiftPlugin({ commands: [...] })
   └→ 桥的登记表
        ...
        └→ surf-app（surfBridge.commands.list/subscribe）：拼 surf-shortcuts 的 schema
Swift 半边照旧订 SurfEventBus.Topic.menuCommand 应答。
```
...
| `configurable` | bool | | 进不进 `surf-shortcuts` 设置页，默认 `true` |
...
| 插件 | id |
|---|---|
| surf-layout | `openSettings`（⌘,）、`newSession`（⌘N）、`stopGenerating`（Esc，无 `menu`） |
| surf-settings | `openSettings`（与 layout 同一条：谁在场都有这一项，所以两边文案与默认键必须一致） |
| surf-sidebar | `renameSession`、`archiveSession`、`focusSearch`、`prevSession`、`nextSession`、`nextPendingSession`、`sessionDigits`（digits 形态，⌘1-9） |
...
**权威**：`surf-layout/swift/LayoutContracts.swift` 的 `ToolbarSpec`（生产端）
与 `surf-layout/swift/ToolbarContribution.swift`（消费端）。

槽名 `LayoutToolbar.slot`（值 `"toolbar"`）。别抄字面量——`import SurfLayout`
按名字引，拼错就编不过。
...
| `event` | String | `surf.toolbar.activate` | 点击时广播的主题 |
...
### 2.2 流量：`surf.toolbar.update` 活通道
...
| 主题 | 常量 | 载荷 |
|---|---|---|
| `surf.toolbar.activate` | `LayoutToolbar.activateTopic` | `slot` / `owner` / `id`；`group` 另带 `index` 与 `itemId` |
| `surf.toolbar.menuSelect` | `LayoutToolbar.menuSelectTopic` | 同上 + 被选项的 `itemId` |
| `surf.toolbar.menuOpen` | `LayoutToolbar.menuOpenTopic` | `owner` / `id`。菜单**将要打开**，给贡献方一次预热机会 |
...
| 槽名 | 常量 | 谁开的 | 谁占 | 没人占会怎样 |
|---|---|---|---|---|
| `root` | 壳里的字面量 | **壳**（唯一一个壳认得的槽名） | surf-layout | 壳退化成全出血 WebView（逃生舱） |
| `sidebar` | `LayoutSlots.sidebar` | surf-layout | surf-sidebar | 不装那个 `NSSplitViewItem`，分栏退化成单栏 |

想写一个替代 surf-sidebar 的插件：占 `LayoutSlots.sidebar` 即可。**壳不认得这个槽名**
——网页那侧的门控参数由占 `root` 的插件经 `surf.web.query` 说了算（见 §4）。

---

## 4. 事件主题总表（`SurfEventBus`）
...
| 主题 | 常量 | 粘性 | 发布方 | 订阅方 | 载荷 |
|---|---|---|---|---|---|
| `surf.menu.command` | `Topic.menuCommand` | 否 | 壳 | layout / sidebar / settings | `command`（+ `digits` 的 arg） |
| `surf.locale` | `Topic.locale` | ✅ | 壳 | layout / sidebar / settings | `locale`（`zh`\|`en`） |
| `surf.web.query` | 两侧各有一份 internal 常量：壳的 `MainWindowController.webQueryTopic`、layout 的 `LayoutPlugin.webQueryTopic` | ✅ | 占 `root` 的插件 | 壳 | `[参数名: 值]`。壳不设白名单也不解释，只拼进 URL；变了就重载页面 |
| `surf.endpointChanged` | `Topic.endpointChanged` | 否 | 壳 | **当前无人** | `httpBase` |
| `surf.connection.state` | `ConnectionController.stateTopic` | ✅ | 壳（连接状态机） | surf-settings「连接」栏 | `phase`（`searching`\|`connecting`\|`connected`\|`disconnected`\|`unreachable`\|`idle`）/ `mode`（`unset`\|`auto`\|`fixed`\|`managed`，**此刻生效中的那份**）/ `managed`（Bool）/ `attempts` / `bridgeConnected` / `pageReady` /（可选）`url`（= `endpoint`，给只认字符串的消费方）·`fixedURL`·`endpoint`·`endpointSource`·`isOwn`·`reason`·`failure`·`statusCode` / `candidates[]`（`url`·`healthy`·`port`·`startedAt`·`source`·`failure`）|
| `surf.app.relaunch` | `MainWindowController.relaunchTopic` | 否 | surf-settings「连接」栏 | 壳 | —。壳 spawn 一个"等本进程死透再 `open` 自己"的助手然后退出。**不是 `app-restart`**：那条走桥、要 surf-app 在场、且受"一个进程只自请重启一次"的保险丝约束 |
| `surf.page.<type>` | `Topic.pagePrefix + type` | 视消息 | 壳（页内桥转发） | 各插件 | 见 §5 |
| `surf.toolbar.update` | `LayoutToolbar.updateTopic` | 否 | 贡献方 | surf-layout | 见 §2.2 |
| `surf.toolbar.activate` / `.menuSelect` / `.menuOpen` | `LayoutToolbar.*` | 否 | surf-layout | 贡献方 | 见 §2.3 |
| `surf.window.title` | `LayoutToolbar.windowTitleTopic` | ✅ | 标识生产方（**眼下无人生产**：主内容区回到 web header 之后没有插件在产标识） | surf-layout | `title` / `subtitle`。空标题 = 交回给壳 |
| `surf.layout.titlebarMetrics` | `LayoutToolbar.titlebarMetricsTopic` | ✅ | surf-layout | 标识生产方 | `inset`（pt）。显示模式一变就变 |
| `surf.layout.newSession` | `LayoutPlugin.newSessionTopic`（internal，外部照抄字符串） | 否 | 任何人 | surf-layout | — |
| `surf.activateWindow` | `Topic.activateWindow` | — | **无** | **无** | **死词汇**，P1 清 SDK 时删 |

> **注**：这两条原先是纯广播 + 两条 request 通道（`surf.window.requestTitle` /
> `surf.layout.requestTitlebarMetrics`）让后到的订阅者自己喊一嗓子补发。
> P0 #4 把它们改成 `emitSticky` 并删掉了那两条 request 通道——粘性总线就是为这件事
> 设计的，纯减法。**新写这类"状态型"消息别再配 request 通道。**

死通道别当成活的读：`surf.activateWindow` 无发无收；`surf.endpointChanged`、
`surf.page.ready`、保管箱键 `surf.endpoint` 只发不收；`surf.layout.newSession` 只订不发
...
```js
window.webkit.messageHandlers.surf.postMessage({ type: "yourType", ... })
```

的任意 `type` 一律原样广播成 `surf.page.<type>`，载荷就是那个字典本身。
**想接一条新页内消息不用改壳**，订 `SurfEventBus.Topic.pagePrefix + "yourType"` 即可。
...
| 键 | 常量 | 类型 | 谁写 | 谁读 |
|---|---|---|---|---|
| `surf.webView` | `SurfObjects.Key.webView` | `WKWebView` | 壳 | layout / nativeify |
| `surf.endpoint` | `.endpoint` | `NSURL` | 壳 | **当前无人** |
| `surf.conversationSurface` | `.conversationSurface` | `SurfConversationSurface`（协议定义在 **surf-layout**，读者要 `import SurfLayout`） | surf-layout | sidebar / notify |
| `surf.settingsOwner` | `.settingsOwner` | `NSWindow`（**只看在不在**） | surf-settings | surf-layout（有主就不弹页内 modal） |
...
| 键 | 主人 | 用途 |
|---|---|---|
| `surf.sidebar.snapshot` | surf-sidebar | 上一代收到的最后一份 snapshot（`NSDictionary`），新代先拿它渲染再要 fresh |
| `surf.notify.inbox` / `.presented` / `.currentSession` / `.swept` | surf-notify | 待办清单、已发通知账、当前会话、"这个进程已经清过一次"的标记 |

> **`.conversationSurface` 与 `.settingsOwner` 是业务词汇寄居 SDK**（协议本身都不在
> SDK 里），P1 清 SDK 时会挪进 surf-layout 的 `.swiftmodule`。别再往 `SurfObjects.Key`
> 里加新的业务键——纪律是"只放壳自己创建并持有的对象"。

**不占槽的插件用保管箱做"按进程收口"的动作**：Swift 热替换每改一行就 `activate` 一次，
"清掉上一次运行留下的东西"这类动作挂在 `activate` 上会每代都执行一遍。往
`host.objects` 插一个标记键即可——保管箱天然是进程级的（`surf.notify.swept` 就是它）。

---

## 7. hook 名（`SurfHooks`）

**权威**：`surf-app/host/Sources/Native/SystemDelegateRelay.swift` 顶注。
...
现有用法示例：`surf.sidebar.filter.mode` / `.showArchived` / `.hiddenGroups`
（侧边栏的筛选状态）。
...
**权威**：写在 `surf-app/lib/index.js` 的 `writeEndpointFile`，读在
`surf-app/host/Sources/SurfEndpoint.swift` 的 `decodeEndpoint`。

落点 `~/Library/Application Support/io.wenbo.surf/endpoints/<profile>.json`，
**一个 profile 一份**（同时跑好几个 dsh 是常态：一个 worktree 一套）。surf-app 在
dsh 启动时写、fiber 卸载时删（pid 校验，只删自己那份）。壳是"扫目录取候选"而不是
"读一个文件"——它是三级定位的第二级，给**手动双击**起来的壳兜底（第一级是
`--surf-endpoint` flag，由拉起壳的那个 dsh 亲手递来，永远最优先）。

| 字段 | 类型 | 谁用 | 含义 |
|---|---|---|---|
| `httpBase` | string | 壳 | `http://127.0.0.1:<port>`。**没有它这份文件就作废**（decode 返回 nil） |
| `bridgePath` | string | 壳 | WS 桥的路径，真相在 surf-bridge 的 `config.path`；缺省 `/surf/bridge` |
| `pid` | number | 壳（诊断）/ `./release` 的互斥检查 | 写这份文件的 dsh |
| `startedAt` | ISO string | 壳 | 候选排序的第二判据（倒序） |
| `profile` | string | 壳（诊断） | 与文件名同源 |
| `hostDir` | string | 壳 | 那个 dsh 的 `surf-app/host` 绝对路径 |
| `appPath` | string | 壳 | **那个 dsh 期望伺候的 App bundle 绝对路径** |

后两个字段都只为一件事：`SurfEndpoint.isOwn`——"这套 dsh 是不是我这一套"。
...
- `hostDir` == `SurfPaths.ownHostDir`。**对装到 `/Applications` 的 Release 壳必然
  失败**——那份 bundle 不在任何 worktree 的 `build/Build/Products/` 之下，
  `ownHostDir` 推不出来。所以才有了 `appPath`。

老版本 surf-app 写的文件没有 `appPath`（也可能没有 `hostDir`），缺字段 = 那条判据
不成立，其余照旧：**两边都是向后兼容的**，加字段不需要同时升级另一半。

### 10.2 壳的连接偏好（UserDefaults，`io.wenbo.surf[.dev]` 域）

**壳自己的键，不是跨插件契约**——列在这儿是因为它决定壳去连谁，读日志/排错时要认得。
权威在 `surf-app/host/Sources/Native/ConnectionController.swift`，计划见
[`surf-connection-plan.md`](../archive/surf-connection-plan.md) §4。**坏值一律退到缺省。**
...
| `surf.connection.fixedURL` | 完整 URL 字符串 | `fixed` 模式的目标；bridgePath 用默认 `/surf/bridge` |

优先级：**用户当场点的一次性目标 > `--surf-endpoint` flag > 上面这两个键**。
...
`appPath` 写 `/Applications/Surf.app`、`hostDir` **整个不写**（没有那个目录）；
有构建能力时两者都指向本 worktree。

### 10.4 托管后端（`BackendManager`）

`surf.connection.mode = managed` 时壳自己 spawn 一个后端并盯着它。权威在
`surf-app/host/Sources/Native/BackendManager.swift` 与同目录的 `ManagedProcess.swift`，
计划见 [`surf-connection-plan.md`](../archive/surf-connection-plan.md) §5。

| 名字 | 是什么 |
|---|---|
| `logs/managed-dsh.<instanceTag>.log` | 托管后端的输出（stdout/stderr 合流）。**跟着产物分片**，与壳日志同一个键；超过 4MB 下次启动时从头来过 |
| `io.wenbo.surf.dsh` | release 形态那个常驻 LaunchAgent 的 label。托管**查重**要问它一句（`launchctl print gui/$UID/<label>`，`state = running` 即让路） |
| `--surf-backend-command '<shell 命令>'` | **只有 Dev 壳认**（判据是 bundle id 的 `.dev` 后缀）：拿假进程替掉真 spawn，用来实测状态迁移/退避/信号 |
| `--surf-backend-skip-dedup` | 同上，只给实测用：跳过查重 |

spawn 什么：Dev 壳（bundle 路径推得出 worktree）跑 `<worktree>/dev`，Release 壳跑
`dsh --profile surf --port 0 --no-open`；两者都经 `/bin/zsh -lc` 拉起
...
**权威在代码里**：打包侧 `surf-app/host/scripts/pack-payload.mjs` 与
`prebuild-plugins.mjs`，消费侧 `surf-bridge/lib/swift-payload.js` 与
`surf-app/host/Sources/Native/CompilerService.swift`。

| 名字 | 谁写 | 谁读 | 是什么 |
|---|---|---|---|
| `Resources/SurfNode/<pkg>/{package.json, lib/, swift/}` | `pack-payload.mjs` | `ProfileBootstrap`（整份 `ditto` 进 `<profile>/.surf/`） | node 半边 + Swift 源码。**`swift/` 与 `lib/` 平级**，各插件的 `new URL("../swift/", import.meta.url)` 因此在镜像里原样成立 |
| `Resources/SurfNode/<pkg>/swift/.surf-static` | `pack-payload.mjs` | `surf-bridge` 的 `register()` | 「这份源码是分发载荷，进程活着的时候不会变」。桥见到它就**只扫一次、不轮询**（§7.10）。不是 `.swift`，所以不进任何 hash |
| `Resources/SurfPlugins/<Module>/prebuilt/<hash12>/lib<Module>_h<hash12>.dylib` | `prebuild-plugins` | `CompilerService.ProductRoot.prebuilt` | 预编译产物。`<hash12>` = contentHash 前 12 位，与用户缓存 `native-plugins/generations/<Module>/<hash12>/` **同构**（两者的产物永远是"兄弟"，插件间依赖那条 `@loader_path` rpath 因此两边通用） |
| `Resources/SurfPayload.json` | `pack-payload.mjs` | `ProfileBootstrap`（当 `.stamp` 的 `sourceHash`）、`prebuild-plugins.mjs`（module 清单） | `{version, hash, packedAt, packages, modules}` |
| `Resources/SurfPrebuilt.json` | `prebuild-plugins` | **没有程序读**，给人看 | `{version, generatedAt, plugins:[{plugin, module, generatedModule, contentHash, dylib}]}` |
| 插件对象上的 `surfSwift`（不可枚举） | `createSwiftPlugin`（`surf-bridge/lib/plugin.js`） | `prebuild-plugins.mjs` | `{name, swiftDir, swiftDeps, sharedModules, schemaVersion}`。构建期算 contentHash 要用它——**静态解析源码猜这三样是不行的**，猜错了不报错，只是预编译永远命中不了 |
...
| 让页面 URL 带一个参数 | 占 `root` 的插件 `emitSticky("surf.web.query", …)` | §4 |
| 接一条新的页内消息 | 网页 `postMessage({type})` + 订 `pagePrefix + type` | §5 |
| 让状态活过热替换 | `host.objects`（只放系统类型）或 `host.store` | §6 §9 |
| 接一个系统 delegate | `SurfHooks` + 壳的 `SystemDelegateRelay` 多两行 | §7 |
