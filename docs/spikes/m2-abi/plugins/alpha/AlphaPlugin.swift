...
/// 插件自定义协议——**不进 SDK**（模拟 surf-layout 的 SurfSidebarProvider）。
...
final class AlphaHandle: SurfOpaqueHandle, AlphaFeature {
    let gen: Int
    weak var host: SurfHost?
    let model = AlphaModel()
    init(gen: Int, host: SurfHost) { self.gen = gen; self.host = host }
...
final class AlphaPlugin: SurfPlugin {
    func activate(host: SurfHost) -> AnyObject? {
        ...
        if generation != 1, let prevAny = host.object("alpha.handle.g1") {
            ...
            if let prev = prevAny as? SurfOpaqueHandle {
                host.note("A5 sdk-existential-cross-gen: \(prev.ping())")
            } else {
                host.note("A5 FAIL: 上一代对象 as? SurfOpaqueHandle 失败")
            }
...
/// ABI 入口（计划 §4.1）：返回 SurfPlugin 实现的不透明指针。
