...
func loadPlugin(_ path: String) -> (image: UnsafeMutableRawPointer, plugin: DashPlugin)? {
    // RTLD_LOCAL：多个 dylib 导出同名 dash_plugin_entry，必须按 handle 取符号。
    guard let image = dlopen(path, RTLD_NOW | RTLD_LOCAL) else {
        info("dlopen 失败 \(path): \(String(cString: dlerror()))")
        return nil
    }
    guard let sym = dlsym(image, "dash_plugin_entry") else {
    ...
    guard let plugin = obj as? DashPlugin else {
        info("entry 返回值 as? DashPlugin 失败：\(type(of: obj))")
        return nil
    }
    return (image, plugin)
}
...
assert_("A1.dlopen", true, "dlopen + dlsym(dash_plugin_entry) + as? DashPlugin 成功")
...
if let h = g1Handle as? DashOpaqueHandle {
    ...
    assert_("A1.observe", false, "handle as? DashOpaqueHandle 失败")
}
...
if FileManager.default.fileExists(atPath: g2Path), let g2 = loadPlugin(g2Path) {
    ...
    if let bh = betaHandle as? DashOpaqueHandle {
...
if FileManager.default.fileExists(atPath: ifacePath) {
    ...
    if let iface = loadPlugin(ifacePath) {
        ...
        assert_("A8", h != nil && (h as? DashOpaqueHandle) != nil,
                "插件从 interface-only 路径编出的 dylib：dlopen + as? DashPlugin + SDK existential 全部成立（类型身份跨 interface 重建一致）")
    } else {
