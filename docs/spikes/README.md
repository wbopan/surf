...
| 目录 | 钉死的结论 | 怎么跑 |
|---|---|---|
| `apple-visual-effect/` | `-apple-visual-effect` 这类系统材质在**不透明窗口**里还成不成立、采样到什么（Raycast 那套是透明窗口 + 桌面采样，surfclam 是文档型 App，窗口不透明，所以必须自己测） | `./run.sh`；对照组 `CLAM_SPIKE_NO_SYSTEM_APPEARANCE=1 ./run.sh` |
...
| `hover/` | macOS 27 的按钮 **hover / 按下**到底改了什么（结论写在 `clam-nativeify/README.md` 的「hover / 按下：整片着色」一节）。布局是「每行两枚一模一样的」，左列同图未被碰过 = 基准 | `swiftc -O HoverProbe.swift -o HoverProbe`（连同 `warp/mean/meanabs/diffmap/winrect` 几个小工具），再 `./run.sh` 与 `./press.sh` |
...
| `tinted-glass/` | 系统的**蓝键 / 红键带色玻璃**长什么样（结论写在 `clam-nativeify/README.md` 的「带色玻璃」一节）。必须截激活态——失活窗口里 tint 整个丢掉、退成平灰 | `swiftc -O TintProbe.swift -o TintProbe` 等三件套，`./TintProbe --hold &` + `tools/shot.sh`，再 `./bbox` / `./rgbcol` 取值 |
