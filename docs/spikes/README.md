...
| `status-indicator/` | 侧边栏行首那个 20pt 状态槽画什么：用真实行几何与 sidebar 材质把几组候选**并排画在同一张图里**选型。定稿是裸符号（举手 / 问号 / 三角警告 / 勾）+ 系统色，淘汰了 `*.circle.fill` 那种圆底实心色 | `swiftc -O Variants.swift -o Variants && ./Variants --light`（或 `--dark`），再 `tools/shot.sh` |
