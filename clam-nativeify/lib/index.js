 ...
 * clam-settings 的通用页读的也是这一处（`swift/SettingsTabs.swift` 的
...
export default createSwiftPlugin({
	...
	subscribe: (api) => {
		...
		// `clamBridge`。那是同一张编排表里的兄弟、且是 Swift 载荷的必要条件，
		// 与 clam-layout 同一个赌注；它不在就没有壳，CSS 有没有也无所谓了。
		ctx.inject(["settings"], (scoped) => {
			// ── 我们自己的 ns：注册完就没了 ────────────────────────────
			//
			// 值由浏览器半边经 `ctx.settingsScope` 自己读，这半边从不碰它，
			// 也不需要 push 给谁。注册一个 ns 就等于同时点亮了两个界面，两边都
			// 不用改一行：clam-settings 那扇原生窗口（「插件 → 插件配置」把
...
function pushTheme(api, { quiet = false } = {}) {
	...
	if (theme === undefined) {
		if (!quiet && !WARNED.has(api)) {
			...
			api.ctx.logger("clam-nativeify").warn(message);
			// cordis logger 在 `dsh web` 下没有 exporter，终端一个字看不见。
			process.stderr.write(`clam-nativeify: ${message}\n`);
		}
		return;
	}
