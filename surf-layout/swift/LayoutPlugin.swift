...
/// surf-layout 的 Swift 半身：占 `root` 槽，摆好 WebView 与 sidebar 槽，
/// 并把会话展示面放进保管箱供下游使用。
final class LayoutPlugin: SurfPlugin {
    /// 本插件那条工具栏贡献被点击时广播的主题。
    /// 主题名是字符串而不是闭包，所以它天然跨得过世代替换。
    static let newSessionTopic = "surf.layout.newSession"

    /// 告诉壳"页面 URL 要带哪些查询参数"的粘性主题（壳侧同名常量在
    /// `MainWindowController.webQueryTopic`）。
    ///
    /// **参数名是 dsh 网页那一侧的私有词汇，定义权在这儿**：壳既不认得
    /// `surf-native-sidebar`，也不认得 `sidebar` 这个槽名——它从前两样都认得，
    /// 于是第三方写一个占 `sidebar` 槽的替代品必须沿用这两个字符串，
    /// 换个槽名就永远拿不到门控（网页侧边栏被藏、原生侧边栏又不存在）。
    static let webQueryTopic = "surf.web.query"

    /// 原生侧边栏在场时页面要带的那个参数：网页自己的侧边栏让位。
    /// surf-layout 的 client 半边认这一个（`?surf-native-sidebar=1`）。
    static let nativeSidebarParam = "surf-native-sidebar"

    func activate(host: SurfHost) -> AnyObject? {
        guard let webView = host.objects.object(SurfObjects.Key.webView, as: WKWebView.self) else {
        ...
        host.objects.setObject(SurfObjects.Key.conversationSurface, surface)
        ...
        host.events.subscribe(SurfEventBus.Topic.menuCommand) { payload in
            ...
            case "openSettings":
                // 有原生设置窗口就让它来（surf-settings 在场时占着 settingsOwner）。
                // 两边都响应的话，原生窗口开出来的同时主窗口里还会弹一层网页 modal。
                if host.objects.object(SurfObjects.Key.settingsOwner) == nil {
        ...
        // 「新建会话」不再占工具栏：那一格让给了 surf-sidebar 的「筛选」。
