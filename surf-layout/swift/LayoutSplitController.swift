...
final class LayoutSplitController: NSSplitViewController {
    ...
    private static let sidebarAutosaveName = "SurfMainSidebar.v2"
    ...
    var toolbarContributions: [SurfContributions.Contribution] = []
    ...
    private var toolbarUpdates: SurfDisposable?
    ...
    init(host: SurfHost, webView: WKWebView) {
        self.host = host
    ...
    private func installToolbar() {
        ...
        let toolbar = NSToolbar(identifier: "SurfLayoutToolbar")
    ...
    ) -> SurfContributions.Contribution? {
...
public enum LayoutToolbar {
    ...
    public static let updateTopic = "surf.toolbar.update"
    ...
    public static let activateTopic = "surf.toolbar.activate"
    /// 菜单项被选中。载荷同上，另带被选项的 `itemId`。
    public static let menuSelectTopic = "surf.toolbar.menuSelect"
    /// 菜单**将要打开**。给贡献者一个预热的机会（拉数据、刷新勾选态）。
    public static let menuOpenTopic = "surf.toolbar.menuOpen"
    ...
    public static let windowTitleTopic = "surf.window.title"
    /// 标题栏当前厚度（`inset`，pt）。显示模式一变就跟着变。
    /// **粘性**，理由同上：晚到的订阅者一订上就先收到最后一份。
    public static let titlebarMetricsTopic = "surf.layout.titlebarMetrics"
}
...
extension LayoutSplitController {
    ...
    static func align(of contribution: SurfContributions.Contribution) -> String {
    ...
    static func spaced(of contribution: SurfContributions.Contribution) -> Bool {
...
///
/// SDK 的贡献槽只收容器不收词汇（见 `SurfContributions` 顶注）：把
