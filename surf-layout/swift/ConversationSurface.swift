...
///
/// **协议住在消费者侧**（1×N 规则）：拥有 WebView 排版的是 surf-layout，
/// 所以协议定义在这里，随 `SurfLayout.swiftmodule` 传给下游。
/// SDK 只放内核词汇，这种生态词汇不进 SDK（计划 §4.1 结尾）。
///
/// 下游（surf-sidebar）从保管箱按 `SurfObjects.Key.conversationSurface` 取，
/// 转型成本协议使用。上游换代时下游会被级联重编，转型永远对得上。
public protocol SurfConversationSurface: AnyObject {
    /// 切换会话区显示的会话。
...
/// WKWebView 实现：经 `evaluateJavaScript` 调用本包 client 半边（lib/client.js）装的 `window.__surf`。
/// 全部防御式——桥不在（普通浏览器、页面还没加载完）时静默失败，不弹窗不报错。
final class WebViewConversationSurface: SurfConversationSurface {
    ...
    private func call(_ fn: String, args: [String]) {
        ...
        let script = "(window.__surf && typeof window.__surf.\(fn) === 'function')"
            + " ? window.__surf.\(fn)(\(args.joined(separator: ","))) : undefined"
