/*
 * surf-layout，浏览器半边（lazy-CJS 经典脚本，手写、无构建步骤）。
 *
 * **为什么住在 surf-layout**：这里的两件事都是「原生分栏接管排版」的网页对端，
 * 而它们唯一的 Swift 调用方就是本包的 `WebViewConversationSurface`
 * （swift/ConversationSurface.swift）。协议两端同包 = 改一边必然看见另一边，
 * 与 `SurfConversationSurface` 协议「住在消费者侧」是同一条 1×N 规则。
 *
 * 1. **页内动作桥 `window.__surf`**：selectSession / startSession / openSettings
 *    + 当前会话反向回报（window.webkit.messageHandlers.surf.postMessage）。
 * 2. **收起 web 侧边栏**：经 ui-layout 的公开服务 ctx.layout.toggleSidebar()
 *    把侧边栏收起，再用 CSS 抵消折叠后残留的 56px rail 轨道，会话列从窗口
 *    左缘起排——原生侧边栏（surf-sidebar）占的就是那块地方。
 * 3. **键位表投影**：订 settings 的 `surf-shortcuts` 命名空间（schema 由 surf-app
 *    的 node 半边注册），值一变就经同一条页内桥推给壳，壳据此重建主菜单。
 *    住这儿是因为页 → 壳的上报通道（ready / currentSession）本来就归它。
 *    计划：docs/archive/surf-shortcuts-settings-plan.md。
 * 4. **语言投影**：订 `ctx.locale`（dsh 解析后的 active，不是设置里的原始
 *    preference），经同一条页内桥推给壳，壳转成粘性总线主题 `surf.locale`
 *    供原生插件消费。计划：docs/archive/surf-i18n-plan.md。
 *
 * 纯样式的原生化（透出玻璃、红绿灯让位、禁橡皮筋/禁选中）不在这里，
 * 那是 surf-nativeify 的事，它零服务依赖、要抢首帧。
 *
 * 门控：UA 含 "Surf/"（动作桥）之上，收起侧边栏再要求 URL 参数
 * surf-native-sidebar=1。终端 `dsh web` / 普通浏览器不受影响。
 *
 * 选择器说明：dsh Web UI 的类名是 hash 化 CSS module（如 pI_x6G_sidebarCol），
 * hash 随版本变化但语义后缀稳定，因此用 [class*="_sidebarCol"] 防御式命中。
 */
window.__ModuleLoader__.load({
	id: "@wenbo/surf-layout",
	factory: () => {
		var module = { exports: {} };
		var exports = module.exports;
		Object.defineProperty(exports, Symbol.toStringTag, { value: "Module" });

		const STYLE_ID = "surf-layout-style";
		const NATIVE_ATTR = "data-surf-native-sidebar";
		const OCCUPANCY_VAR = "--surf-sidebar-occupancy";
		// ui-layout computeColumns：折叠（sidebar 偏好为 0）时轨道仍占 56px rail。
		const RAIL_PX = 56;

		/* ------------------------------------------------------------------ *
		 * 快捷键设置（ns `surf-shortcuts`，schema 由 surf-app 的 node 半边注册）。
		 * ------------------------------------------------------------------ */

		/** 设置命名空间。与 surf-app/lib/index.js 的 SHORTCUTS_NAMESPACE 逐字一致。 */
		const SHORTCUTS_NS = "surf-shortcuts";
		...
		/** 原生侧边栏模式：UA 门控之上再要求 URL 查询参数 surf-native-sidebar=1。 */
		function nativeSidebarMode() {
			try {
				return insideSurf() && new URLSearchParams(window.location.search).get("surf-native-sidebar") === "1";
			} catch {
				return false;
			}
		}
		...
		/**
		 * 经 window.webkit.messageHandlers.surf 向壳应用发消息；
		 * 普通浏览器（无该 handler）静默跳过。
		 * @param {Record<string, unknown>} msg
		 */
		function postToShell(msg) {
			try {
				const handler = window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.surf;
				if (handler && typeof handler.postMessage === "function") handler.postMessage(msg);
			} catch { /* 上报失败静默 */ }
		}

		/* ------------------------------------------------------------------ *
		 * 页内动作桥 window.__surf（仅 surf UA；防御式，绝不抛）。
		 * ------------------------------------------------------------------ */
		 ...
		 * 每次启动都重装：新实例的服务接线才是活的，沿用上一代的 window.__surf
		...
		function installBridge(ctx, token) {
			...
			// `surf.page.<type>`，这里发的 `keymap` 到那边就是 `surf.page.keymap`。
			//
			...
			try {
				ctx.inject(["settingsScope"], (kctx) => {
					try {
						...
						kctx.effect(() => scope.subscribe(sync), "surf-layout: keymap 投影");
						sync();
					} catch { /* 服务形状不符静默 */ }
				});
			...
			// `surf.page.<type>`），页面生命周期天然解决补发——壳重启 = WebView
			// 重载 = 本文件重跑 = 重新投影。壳那边再转成粘性总线主题，
			// 因为插件比页面装载得晚（见 SurfEventBus.emitSticky）。
			//
				...
				ctx.inject(["locale"], (lctx) => {
					try {
						...
						if (typeof locale.subscribe === "function") {
							lctx.effect(() => locale.subscribe(syncLocale), "surf-layout: locale 投影");
						}
						syncLocale();
					} catch { /* 服务形状不符静默 */ }
				});
				...
				ctx.inject(["sessions"], (sctx) => {
					try {
						...
						if (sessions && sessions.list && typeof sessions.list.subscribe === "function" && typeof sessions.list.getSnapshot === "function") {
							...
							report();
							sctx.effect(() => sessions.list.subscribe(report), "surf-layout: currentSession channel");
						}
						// Esc → 停止当前会话正在生成的回复（语义对齐 Claude Code 桌面版）。
						// 键位可改（设置 `surf-shortcuts.stopGenerating`，上面那段投影
			...
			return () => {
				...
				try {
					...
					if (window.__surf && window.__surf.__surfToken === token) delete window.__surf;
				} catch {
					try { window.__surf = undefined; } catch { /* 静默 */ }
				}
			};
		}
		...
		/**
		 * 插件体：装动作桥（凡 Surf/ UA）+ 收起 web 侧边栏（再要 native 模式）。
		 * @param {import('@deepseek-ai/cordis').Context} ctx
		 */
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				if (native) {
					...
					style.textContent = [
						...
						// --surf-sidebar-occupancy，缺省 56px），侧边栏列移出视口、
						...
						// 挂载成功、就是看不见，且不报任何错。这是 ⌘, 在 surf-settings
		...
		// 重载连带本插件卸载重挂，白白抖掉已装好的 window.__surf。
		exports.inject = [];
