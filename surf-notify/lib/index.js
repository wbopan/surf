/**
 * surf-notify —— 可交互桌面通知（计划 `docs/archive/surf-notify-plan.md`）。
 *
 ...
 *
 * `open`（打开查看）不上行：跳转是纯 Swift 侧动作，走 `SurfConversationSurface`。
 *
 * @module surf-notify
 */
...
/** 设置命名空间。注册一次同时点亮 surf-settings 的原生窗口与 dsh 页内设置对话框。 */
const SETTINGS_NS = "surf-notify";
...
/** 插件实例 → 动作实现（照 surf-sidebar：`expose` 表在模块求值时就要定好）。 */
...
export default createSwiftPlugin({
	...
	subscribe: (api) => {
		const { ctx, push } = api;
		const log = reporter(ctx.logger("surf-notify"));
		let version = 0;
		let timer;
		/** `surfPending` 的订阅者（跨插件，见下面 provide 那段）。 */
		...
		// ——surf-notify 缺席时侧边栏退回自己那份 approval-only，不能不挂载。
		ctx.provide("surfPending", {
			/**
		...
		ctx.inject(["settings"], (scoped) => {
			...
			// `surf-settings/swift/Strings.swift`。
		...
		// 开发用的自测钩子：`SURF_NOTIFY_SELFTEST=1 ./dev` 会在挂载 3 秒后塞两条
		...
		if (process.env.SURF_NOTIFY_SELFTEST === "1") {
			const timer = setTimeout(() => {
				log.warn("自测：塞两条假待办（SURF_NOTIFY_SELFTEST=1）");
			...
			timer.unref?.();
			ctx.effect(() => () => clearTimeout(timer), "surf-notify 自测钩子");
		}
		...
		sentinel.unref?.();
		ctx.effect(() => () => clearTimeout(sentinel), "surf-notify 数据面守望");
		...
		function pushInbox() {
			...
			// 同一拍里叫醒 `surfPending` 的订阅者。一个订阅者抛错不该带塌别人，
			// 更不该带塌通知线本身。
			for (const watcher of watchers) {
				try {
					watcher();
				} catch (error) {
					log.warn(`surfPending 订阅者抛错：${errorText(error)}`);
				}
			}
		}
	},
	...
	expose: Object.fromEntries(ACTIONS.map((action) => [action, async (payload, api) => {
		...
		try {
			...
			api.ctx.logger("surf-notify").warn(`${action} 失败：${message}`);
			process.stderr.write(`surf-notify: ${action} 失败：${message}\n`);
 ...
 * 要给蹲终端的人看的东西必须自己写 stderr（与 surf-bridge / surf-sidebar 同款）。
 */
function reporter(logger) {
	const emit = (level, message) => {
		logger[level](message);
		process.stderr.write(`surf-notify: ${message}\n`);
	};
	return {
