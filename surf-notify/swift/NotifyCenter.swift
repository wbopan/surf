...
/// 里占住它，把回调原样拍平经 `SurfHooks` 转发。壳不解释任何通知语义，
...
final class NotifyCenter {
    ...
    init(log: @escaping (String) -> Void) {
        self.log = log
        self.prefix = "surf.\(Self.instanceTag()).".self
    }
    ...
    func installHooks(host: SurfHost,
                      handle: SurfPluginHandle,
    ...
    /// 区分依据（壳那边的 `SurfPaths.instanceTag` 同源，但它在壳的 Sources 里，
