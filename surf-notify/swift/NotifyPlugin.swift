...
/// surf-notify 的 Swift 半身：把 node 推下来的待办变成 macOS 原生通知。
///
/// **不占任何槽，也不贡献任何界面**——它只跟系统通知中心和 Dock 角标打交道，
/// 缺席时什么都不会缺，这也是它敢在 `surf-layout` 之后挂载的原因。
///
/// 这半边只做四件事：
...
final class NotifyPlugin: SurfPlugin {
    ...
    func activate(host: SurfHost) -> AnyObject? {
        // 会话展示面缺席（surf-layout 没装配）**不是致命的**：通知照发照撤，
        // 只是点了之后跳不过去。与 sidebar 不同——那边缺了面就没有存在意义。
        let surface = host.objects.object(SurfObjects.Key.conversationSurface)
            as? SurfConversationSurface
        if surface == nil {
...
final class NotifyPresenter {
    ...
    private static let inboxKey = "surf.notify.inbox"
    /// 保管箱：已经发出去的那些通知的内容签名（`NSDictionary`）。
    /// 不存的话，每次热替换都会把在屏的通知**重发一遍**——响一声、跳一下，
    /// 改一行 Swift 就来一次，很快就会让人把通知关掉。
    private static let presentedKey = "surf.notify.presented"
    ...
    private static let sweptKey = "surf.notify.swept"
    /// 保管箱：页内桥最后报上来的当前会话（`NSString`）。
    /// 页面只在**切换时**报，新一代拿不到当前值；不存的话热替换后的头一分钟里
    /// "你正看着它"这条判据是瞎的。
    private static let currentSessionKey = "surf.notify.currentSession"

    private let host: SurfHost
    private let surface: SurfConversationSurface?
    private let center: NotifyCenter
    /// 本代所有注册与订阅。**由 presenter 持有**（见 `NotifyPlugin.activate` 的注释）。
    private let handle = SurfPluginHandle()
    ...
    init(host: SurfHost, surface: SurfConversationSurface?) {
        self.host = host
    ...
    func start() {
        ...
        host.events.subscribe(SurfEventBus.Topic.pageCurrentSession) { [weak self] payload in
            MainActor.assumeIsolated {
