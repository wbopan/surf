 ...
 *
 * @module surf-sidebar/dsh-source
 */
...
export function createSessionSource(ctx, log) {
	...
	function groups() {
		...
		if (others.length > 0) {
			...
			result.push({ id: "surf.sidebar.other", workspaceId: null, title: "", sessions: others });
		}
		return result;
	}
	 ...
	 *
	 * 我们自己认领得了的失败（不是上游给的原话）挂一个 `surfCode`，
...
/**
 * 带机器可读原因码的失败。**文案不在这儿**：`surfCode` 经桥的 `error` 频道
