 ...
 *
 * `inject`/`swiftDeps` 里的 `surf-layout` 一份声明两层消费：Cordis 据此保证
 * "layout 未挂好本插件不挂载、layout 换代时本插件级联重载"，桥据此排编译拓扑序
 * （Swift 侧 `import SurfLayout` 拿 `SurfConversationSurface`）。
 *
 * ## 桥协议
 *
 ...
 * surf-notify 供出来的 `surfPending`（见 `withPending`），surf-notify 缺席时
 ...
 * / `failed` / `done` 三个取值（经 `surfPending`，见 `withPending`），
 * v5 = `error` 帧结构化（`message` 不再带中文前缀，改配 `code`；i2 文案双语化）。
 */
const SCHEMA_VERSION = 5;

/**
 * `surfPending` 的原因 → 投影里的 `status`。
 *
 * 键就是 surf-notify 那份待办的 `kind`；它没供出来的类别（将来新加的）在这里
...
export default createSwiftPlugin({
	...
	// 没有 sharedModules：Swift 半身只 import SurfSDK（无条件）与 SurfLayout。
	// 曾经声明的 DSHKit 随本次迁移整体退役。

	// 壳菜单里那些会话命令（形状见 surf-bridge/lib/plugin.js 的 CommandDeclaration）。
	...
	subscribe: (api) => {
		const { ctx, push } = api;
		const log = reporter(ctx.logger("surf-sidebar"));
		let version = 0;
		let timer;
		/** 首份有内容的投影是否已经记过日志。 */
		let announced = false;
		...
		let source;
		/** surf-notify 供出来的「有什么在等着你」。缺席时是 undefined。 */
		let pending;
		...
		// session log，见 dsh-source.js 的 `statusOf`），而 surf-notify 那边为了发
		// 通知已经养着一条 mux 帧流、维护着一份权威的待办表。真相只该有一份，
		// 所以订它，而不是在这儿再养一条。
		//
		// **运行时嵌套 inject**：surf-notify 是可选的，缺席时侧边栏退回自己那份
		// approval-only 的状态点，一切照旧。
		ctx.inject(["surfPending"], (scoped) => {
			pending = scoped.surfPending;
		...
		sentinel.unref?.();
		ctx.effect(() => () => clearTimeout(sentinel), "surf-sidebar 数据面守望");
		...
		/**
		 * 把 `surfPending` 的原因叠进投影的 `status`。
		 *
		 * **只升不降**：两边各自看到的都是真事实，取更该管的那一个（`STATUS_RANK`）。
		 * surf-notify 缺席时原样返回——这条路径必须存在，它是侧边栏的独立性。
		 */
	...
	expose: Object.fromEntries(ACTIONS.map((action) => [action, async (payload, api) => {
		...
		try {
			...
			api.ctx.logger("surf-sidebar").warn(`${action} 失败：${message}`);
			process.stderr.write(`surf-sidebar: ${action} 失败：${message}\n`);
			const code = typeof error?.surfCode === "string" ? error.surfCode : undefined;
 ...
 * 要给蹲在终端的人看的东西必须自己写 stderr——照 surf-bridge 的做法两边都喂。
 */
function reporter(logger) {
	const emit = (level, message) => {
		logger[level](message);
		process.stderr.write(`surf-sidebar: ${message}\n`);
	};
	return {
