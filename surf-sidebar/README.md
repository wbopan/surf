...
- `mode`：列表的组织轴，**三档**，在工具栏「筛选」菜单的「分组方式」里切：
  - **按工作区**：工作区分组头 + 组内会话（默认档）；
  - **按时间**：整个换成日期分段（今天 / 昨天 / 过去 7 天 / 更早）；
  - **按状态**：分成**待处理 / 进行中 / 已结束**三段（`StatusBuckets`）——
    待处理 = 待批准 + 待回答，进行中 = 正在跑，其余（跑完了 / 出错了 / 闲着）
    都归已结束。
  **状态只在「按状态」这一档影响分段**：另外两档一条会话都不往外提，按工作区看
  时项目里的会话是齐的，按时间看时今天的会话都在「今天」里。
  （曾经有个恒常置顶的「待处理」分区，两种视图都摆，正是它把行提走了。）
- `hiddenGroups`：被工具栏「筛选」菜单取消勾选的工作区。三档视图都认它。
