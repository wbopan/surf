...
/// surf-sidebar 的全部**用户可见**文案，zh 与 en 并排写在同一行（审校时一眼对照）。
///
/// ## 纪律（`docs/archive/surf-i18n-plan.md` §4/§5/§8，与壳的 `Strings.swift` 同一套）
///
...
/// ## 取值的地方
///
/// `AppSidebarModel.strings` 现算一份 `L`（读 `SurfLocaleStore.current`）。
/// SwiftUI 视图读 `model.strings` 就建立了对那个 `@Observable` 的观察依赖，
/// 语言一变自动重渲——**不用 `withObservationTracking`**（静默死亡坑，CLAUDE.md）。
/// 工具栏那条贡献不在 SwiftUI 里，由 `SidebarPlugin` 订 `surf.locale` 后重新贡献
