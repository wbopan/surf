...
final class AppSidebarModel: ObservableObject, SidebarModel {
    ...
    /// 于是切语言时列表自动重渲。
    private let locale: SurfLocaleStore
    /// 写一行进壳的日志（`host.log`）。
    private let log: (String) -> Void
    private var lastLoggedCount = -1
    /// 选中会话变化时的回调（插件用它落 SurfStore，热替换后能还原高亮）。
    var onSelectionChange: ((String?) -> Void)?

    init(snapshot: SidebarSnapshot,
         bridge: SurfBridge,
         surface: SurfConversationSurface,
    ...
    /// surf-layout 的 `conversationSurface`（页内 `window.__surf`）。
