...
enum SessionTimestamp {
    ...
    private static var cache: [SurfLocale: Formatters] = [:]

    /// `SurfLocale` → `Foundation.Locale`。zh 明确要简体（`zh` 单独用会解析成
    /// 系统当前的中文变体，繁体机器上就成了繁体，而 dsh 那边只有简体）。
    private static func foundationLocale(_ locale: SurfLocale) -> Locale {
        switch locale {
    ...
    private static func formatters(_ locale: SurfLocale) -> Formatters {
    ...
    static func text(for date: Date, locale: SurfLocale, now: Date = Date()) -> String {
