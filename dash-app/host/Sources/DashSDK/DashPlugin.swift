...
/// 或系统框架。
///
/// SDK 与壳严格同版本：它住在壳源码里（`dash-app/host/Sources/DashSDK/`），
...
/// - `@State` 里只放丢了不心疼的东西。想活过热替换的状态写进 `DashStore`，
///   或者干脆放回 TS 半身。
...
/// 旧注册在析构里自行退场（且只退自己那一份，见 `DashRegistry.register`）。
public final class DashDisposable {
...
/// 一组 `DashDisposable` 的容器——插件 `activate` 的标准返回值。
public final class DashPluginHandle {
    private var disposables: [DashDisposable] = []

    public init() {}

    public func keep(_ disposable: DashDisposable) {
        disposables.append(disposable)
    }

    /// 便于链式写：`host.registry.register(...).kept(by: handle)`。
    public func adopt(_ disposables: DashDisposable...) {
...
extension DashDisposable {
    /// `host.registry.register(...).kept(by: handle)`
    @discardableResult
    public func kept(by handle: DashPluginHandle) -> DashDisposable {
        handle.keep(self)
        return self
    }
}
