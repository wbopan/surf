#!/usr/bin/env node
/**
 * surf 的**开发者**安装器与启动器——一条命令把 profile 备齐并跑起来。
 *
 * **只有一种模式：link 本仓库的源码。**
 *
 *   node surf/bin/surf.js    （或仓库根的 ./dev）
 *   把本 worktree 的各插件 + 伞包全部 link 进 profile。改一行存盘即生效，
 *   不必发布任何东西。
 *
 * **`npx @wenbo/surf` 那条 registry 路径 2026-08-30 随 M4 删了**
 * （`docs/archive/distribution-plan.md` §0 与 §3.3）。它从来就走不通：那条路要求用户机器上
 * 有完整 Xcode（十几 GB）加一个不入库的 `xcodegen` 二进制，缺一样就**优雅缺席**
 * ——dsh 起、HTTP 200、壳一声不响地不存在。正式形态改由 `Surf.dmg` 分发
 ...
 * 编排权只在伞包那张表上。
 *
 * **worktree**：profile 名随 worktree 走（主 worktree = `surf-dev`，其余 =
 * `surf-<目录名>`），端口默认 `--port 0` 让 OS 挑。于是每个 worktree 各有
 * 一套插件、一个 dsh、一个 App 实例（App 产物路径本就随 worktree 不同），
 * 互不打扰。
 *
 * **`surf` 这个名字不属于任何 worktree**（`docs/archive/distribution-plan.md` §3.6）：
 * 它是**安装形态专属**的身份，由 `/Applications/Surf.app` 自己自举
 * （`Native/ProfileBootstrap.swift`：拷 bundle 里的 node 载荷进
 * `<profile>/.surf/`、手写 `link:` 依赖与符号链接）。开发形态一律带后缀，
 ...
 * **它不再备 profile**：`surf` 那个 profile 由 App 自己自举（见上），
...
const UMBRELLA = "@wenbo/surf";

/**
 * xcodegen 二进制的落点（相对仓库根）。surf-app/lib/index.js 与
 * `surf-app/host/scripts/{dev,build}.sh` 都写死这条路径，改它要三处一起改。
 */
const XCODEGEN_REL = "surf-app/host/tools/xcodegen";

/** 必须在 bundles 里、且必须排在最前的三层 patch。dsh 自带的两个不用装。 */
const REQUIRED_BUNDLES = ["@deepseek-ai/dsh-base", "@deepseek-ai/dsh-web-app", UMBRELLA];

/**
 * 主 worktree 的 profile 名。**带 `-dev` 后缀是有意的**：无后缀的 `surf`
 * 归安装形态（见 `defaultProfile` 顶注）。`./release` 的"只许主 worktree"
 * 那道检查比对的就是这个名字。
 */
const MAIN_DEV_PROFILE = "surf-dev";
...
function resolveRepoRoot(pluginNames) {
	...
	fail(`这条命令只在 surf 仓库里跑（${UMBRELLA_DIR} 边上没有插件源码）。\n`
		+ `  开发：git clone 之后在仓库根跑 ./dev\n`
		+ `  日常使用：下载 Surf.dmg 拖进「应用程序」，双击即可（不需要 npm 装任何东西）`);
}

/** `@wenbo/surf-app` → `surf-app`：包名去掉 scope 就是仓库里的目录名。 */
function dirOf(packageName) {
	return packageName.startsWith("@") ? packageName.split("/")[1] : packageName;
}

/**
 * profile 名。主 worktree 用 `surf-dev`，其余 worktree 用
 * `surf-<目录名>`——于是"一个 worktree 一行命令起一套自己的东西"不需要
 * 任何额外记忆。
 *
 * **`surf`（无后缀）不发给任何 worktree**（`docs/archive/distribution-plan.md` §3.6）：
...
/**
 * 保证 surf-app 边上有 `host/tools/xcodegen`。
 *
 * 那个二进制**被 .gitignore 挡在库外**（二进制不该入库，规则本身是对的），
 * 于是任何新克隆 / 新 worktree 里它都不存在，而 surf-app 和两个构建脚本都直接
 * spawn 它。
 ...
 * 变回 ENOENT"，而且 surf-app 的 HASHED_ROOTS 明确把 `tools/` 排除在源码 hash
 * 之外，多这个文件不会触发壳的全量重建。
 *
...
function ensureXcodegen(repoRoot) {
	...
	if (source === undefined) {
		say(`⚠ 缺 ${local}——壳构建会失败，surf-app 优雅缺席（只有浏览器，没有 App）。`);
		say(`  补法（二选一，然后重跑本命令）：`);
 ...
 * 而 App 那边不受影响——surf-app 把实际端口写进 endpoint 文件、也用
 * `--surf-endpoint` 直接递给它拉起的壳。
 */
 ...
 * 把 bundle 里的 node 载荷拷进 `<profile>/.surf/`）。这里留着这个常量
...
/** Release 壳的安装位置，与 build.sh 的 `INSTALL_DIR` 和 surf-app 的 `INSTALLED_RELEASE` 一致。 */
const INSTALLED_APP = "/Applications/Surf.app";

/** Release 壳的构建 + 安装脚本（相对仓库根）。 */
const BUILD_SCRIPT_REL = "surf-app/host/scripts/build.sh";
...
/**
 * 跑 `surf-app/host/scripts/build.sh`：xcodegen → xcodebuild Release →
...
/**
 * 装好的 App 里有没有那份 node 载荷（`Contents/Resources/SurfNode/`）。
 *
...
function warnIfNoPayload() {
	const payload = join(INSTALLED_APP, "Contents", "Resources", "SurfNode");
...
function parseArgs(argv) {
	...
	// release 形态的 profile 与端口都不由这条路径决定（profile 钉 `surf`、
...
function usage() {
	process.stdout.write(`surf —— 开发者的安装器与启动器（dsh + macOS 原生壳）

  ./dev                           = node surf/bin/surf.js
                                  link 本 worktree 的源码进 profile 并启动

  **只在本仓库里跑。** 日常使用请下载 Surf.dmg 拖进「应用程序」——
  正式形态不经 npm（docs/archive/distribution-plan.md §0）。

选项
  --profile <name>   覆盖 profile 名（默认：主 worktree 用 surf-dev，
                     其他 worktree 用 surf-<目录名>；无后缀的 surf
...
function say(message) {
	process.stderr.write(`surf: ${message}\n`);
}
