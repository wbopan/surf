...
surf（macOS 壳应用）专用的 dsh Web UI 原生化插件。**三面包**：

- `dsh.client`（`lib/client.js`）：浏览器半边，dsh-client-modules 的 node 半边扫描后经 `/plugins/surf-nativeify/client.js` 送达页面。**九成实现在这里**。
- `lib/index.js`（node 半边）：两件事——① 注册设置命名空间 `surf-nativeify`（唯一一项是「对话区字号」），走运行时嵌套 `ctx.inject(["settings"])`，缺席即退到默认值；② 登记 `swift/` 载荷，并把 dsh 的 `ui-theme` 偏好投影下去（见「原生侧跟随 dsh 主题」）。
...
`@wenbo/surf` 的那张表上（`surf/cordis.patch.yml` 里的 `ui-surf-nativeify` row），
一处真相；子包各自带一份 patch 是旧结构，早已作废。

## 职责边界

**只做「让 dsh Web UI 摸起来像原生 macOS App」这一件事**，页面一侧的实现基本是注入
一段 CSS（外加一段十行的 JS 把窗口焦点态映射到 `<html data-surf-blur>`，见「四态」
...
那条线的代价是本包多了一个薄 Swift 载荷（因而多一条硬依赖 `surfBridge`），
但它仍然只读不写：**主题的真相始终在 dsh，这里一个偏好都不存。**

门控只有一条：`navigator.userAgent` 含
`Surf/`（带斜杠，防普通子串误命中；壳经 `applicationNameForUserAgent` 声明）。
...
3. **按钮按压手感**（macOS 26 / Tahoe）：按下时容器 `scale: 1.09`，**内容跟着容器
   ...
   > 里用的是经典跳过法：挂 `data-surf-nofx`（配套 CSS `transition: none !important`）
   > → 改 `data-surf-blur` → **读一次 `offsetHeight` 强制刷新样式** → 摘掉标记。
   ...
   `watchPressPoint()` 写成 `--surf-px/--surf-py`（本插件的第二段、也是最后一段 JS）。
   三件事值得记：
   ...
   **亮光只留在深色档**（`--surf-press-glow`：深 `rgba(255,255,255,.17)`、浅 `transparent`）。
...
据此落成 `--surf-tint-hover` / `--surf-tint-press`，做成 `--surf-surface` 里的一层整片
inset。三条是从数据直接读出来的、别凭手感改回去：

- **浅色变暗、深色提亮。** alpha 直接由 Δ 除以底色算：8.6/248.1 = .035、
  17.3/248.1 = .070（黑）；深色 21.9/(255−61.1) = .113（白）。
- **深色的按下不再加码。** 系统那一档 hover 就到顶了，`--surf-tint-press` 与
  `--surf-tint-hover` 同值。深色档多出来的层次由按压亮光提供（那是自己要的第三级，
  系统只有两级）。
...
- `--surf-tint` **必须 `@property` 注册**（`<color>`，initial `transparent`）：
...
规则不再命中，`--surf-dur`（320ms）/ `--surf-dur-fast`（160ms）原样接管，余韵不变。

这里原来有第三个常数 `--surf-dur-press: 90ms`。短，但仍是一段补间，按下去总差着
...
两根支柱都得让位：材质那层 `backdrop-filter` 直接关掉，半透明的 `--surf-glass-fill`
换成不透明近似色。
...
`BODY` 由设置命名空间 `surf-nativeify` 的 `bodyFontSize` 驱动（设置 → 插件 →
...
布线两句话：node 半边 `ctx.settings.register("surf-nativeify", …)` 注册 ns，
浏览器半边 `ctx.settingsScope.bind({namespace})` 订阅它，值一变就重写字体那张 style。
注册一个 ns 就同时点亮了两个界面（surf-settings 那扇原生窗口、dsh 页内设置对话框），
...
**字体那一层是单独一张 `<style id="surf-nativeify-font">`**，与其余 CSS 分开。合在
...
- **底色的合成确实在 sRGB 里**：黑白两点定出的直线预测中灰 199.3、实测 199.2。
  所以 `--surf-glass-fill` 用普通 alpha 合成就对。（那两点解出的是
  `rgba(253,253,253,0.5725)`，比定稿的 `0.5272` 略实；两者在白底上等价，
  只在深色背景上分得出。）
- `saturate` 只给 `--surf-glass-fill` 那一组，**不给 `_primary`**：强调键是实色，
...
下发光 ×4     inset 0 -kpx 0
hover/按下着色 inset 0 0 0 100px            --surf-tint，闲时 transparent
左内侧阴影    inset 14px 0 8px -15px
右内侧阴影    inset -14px 0 8px -15px
────────────  以上是 --surf-surface  ────────────
底色          background-color              --surf-glass-fill，**唯一的给色处**
外投影        0 1px 2px                     非 inset，在 --surf-surface 之外
```
...
底色以前是一层 `inset 0 0 0 100px var(--surf-glass-body)`，理由写在注释里 ——
...
> **2026-08 已退休（用户裁决）**：发送键在 spike 台上和「实心蓝原样」「蓝色液态玻璃
> 七变体」并排比过，用户选了 **dsh 原样**——`_primary` 现在不叠任何手绘玻璃面
> （`--surf-surface` 只剩按压 tint，外投影清零），只保留按压变暗/scale 与失活
> grayscale 这些行为反馈。本节往下的推导（oklch +0.12 模型、色域映射、@property
> 防线）**保留作设计笔记**：@property 注册仍在役（neutral 组 fallback 还用白高光），
> 将来想上蓝玻璃，`docs/spikes/apple-visual-effect/` 的「发送键变体」一节可复跑。
...
```css
--surf-glass-glow-c: oklch(from var(--dsw-alias-button-info-fill) calc(l + 0.12) c h);
```
...
为此 `--surf-glass-glow-c` 从「通道三元组」改成了普通 `<color>`，发光层写成
`rgb(from var(--surf-glass-glow-c) r g b / calc(…))`。**它必须 `@property` 注册**
（`<color>`、`inherits: true`、initial `#fff`），理由和 `--surf-tint` 一样、而且更硬：
...
|---|---|---|---|---|
| `--surf-glass-glow-t` | 上缘最外一像素的峰值 | **.795** | **.06** | .6 |
| `--surf-glass-glow-b` | 下缘峰值 | **.795** | **.06** | .6 |
| `--surf-glass-glow-d` | 每向内一像素乘的衰减，越小掉得越快 | **.45** | **.5** | .5 |
...
所以发光的颜色抽成了 `--surf-glass-glow-c`，默认白，带色时要换掉——**换法是从 dsh
...
`<html data-surf-blur>`。WKWebView 会把承载窗口的激活/失活转成页面的 focus/blur
...
新实例刚打上的 `data-surf-blur`，于是页面明明失焦、按钮却停在激活态那摞玻璃上，
**要等到下一次 focus/blur 才自愈**（窗口一直待在后台就永远不自愈）。现在 `sync()` 把
一个随机 token 写进属性值，cleanup 只清 token 对得上的那份。CSS 那边一律是
`[data-surf-blur]` 存在性匹配、不看值，所以选择器一个字都不用改。
（同文件里字体那张 style 用的是 `fontStyle === style` 的对象身份守卫——同一条纪律的
另一种写法；属性值存不了对象引用，才改用随机串，思路同 surf-layout 的 `makeToken`。）

**刻意不走「让壳注入」那条路**：壳的窗口通知要经 surf-layout 的 Swift 半边才够得着
WebView，那会给 surf-nativeify 添一条跨插件契约。收不到事件的后果只是永远停在激活态
那套值 = 加这段之前的行为，无害，所以宁可要零依赖。

特异性靠 `:root[data-surf-blur] body`（0,2,2）稳压 `body[data-ds-dark-theme]`（0,1,1），
深色失活再多一个属性选择器压住浅色失活。
...
@supports (-apple-visual-effect: -apple-system-glass-material-media-controls) {
  :root:not([data-surf-blur]):not([data-surf-reduce]) <neutral 五条> {
    ...
    --surf-surface: inset 0 0 0 100px var(--surf-tint);   /* 只留交互态那一层 */
    --surf-glass-drop: transparent;             /* 外投影一并交出去 */
  }
  :root:not([data-surf-blur]):not([data-surf-reduce]) <neutral 五条>::before {
...
**减少透明度那道门是根属性 `data-surf-reduce`，不是媒体查询**——本机 WebKit
...
`:root[data-surf-reduce]` 前缀（特异性高于被覆盖的那条，不再依赖源码顺序）。

| 门 | 不成立时 | 落到哪 |
|---|---|---|
| `@supports` | 普通浏览器 / 壳侧 `useSystemAppearance` 没开 | 手绘四态，原样 |
| `:not([data-surf-reduce])` | 系统"降低透明度"开着（JS matchMedia 判定） | 手绘 + 已有的不透明近似色 |
| `:root:not([data-surf-blur])` | 窗口失活 | 手绘失活那两格（平色 + 描边 + `grayscale(1)`） |

**计划里写的是"块内覆写 `-apple-visual-effect: none` 再让手绘层重新生效"，这里改成了
进入条件。** 两者的计算值完全等价，代价差得远：覆写那条路要在块内把
`--surf-surface` 整摞重新列一遍（还得分 plain / bordered 两组，只有前者带描边层），
并按浅深两档复述一遍 `--surf-glass-drop` 的常量——等于给同一份真相开第二个抄本。
...
- **只留 tint 那一层，别的整摞退休。** 材质自带完整外观（模糊、提饱和、边缘一圈
  ...
  再叠上去就是两套边缘打架。**但 `--surf-tint` 必须留**：材质不提供任何交互态
  （它连窗口失活都不变，更不会响应 hover / 按下），那两级层次只能由我们出。
  按压那片跟着指针走的径向光走 `background-image`，不在 `--surf-surface` 里，原样保留。
- **外投影也去掉**（`--surf-glass-drop: transparent`）。它本来是"让一块手绘的半透明白
  在页面上站住"的拐杖；材质是真实合成层，接地关系是它自己的事，两份贴地阴影叠起来
  只会脏。代价是 hover 少掉"浮起来"那一级（1px→2px→0 三档一起没了），**只剩 tint
  一级层次**。真觉得反馈不够，把那一行删掉就整套回来，不牵动别的。
- **`_primary` 不上材质、也不再叠手绘面。** 起初只是"实色强调键背后糊什么都看不见"
  所以不上材质；后来用户在 spike 台裁决连手绘的 oklch 高光也退休——**dsh 原样**。
  它身上只剩按压 tint/scale 与失活 grayscale（见「带色按钮」一节头部的裁决记录）。
...
#### 第二块表面：composer 输入卡片（P6，2026-08-28 用户拍板保留）

`[class*="_card"]:has(textarea)`（收窄理由见字号那节：`_card` 后缀跨档复用）走
`-apple-system-glass-material`——**面板档，不是按钮那档**：一整张 780px 宽的输入卡
需要比胶囊档更厚的浓度，太透的话身后滚过的正文会顶穿到正在敲的字里。三层门控、
`::before` fx 层与按钮组完全同构，失活/降透明度同样整体交还 dsh 原样。

卡片之外必须多动一层：**`_composerSeat` 的渐变幕布**（dsh 用来让正文"滚到卡片
跟前就淡出"的那层 36px 实底渐变）。不让它让位，材质隔着幕布只能采到一块平色，
玻璃就白做了。让位方式是"只剩最底下 36px 实底"：卡片背后全透供材质采样，
卡片以下（统计行 + 底缝）仍有实底垫背——**8px 实测盖不住统计行**，正文半截字
会压在"5 turns · 8 steps"那行背后。

材质自带的那圈**边缘高光**在白底浅色下相当显眼——用户看过真机后拍板保留
（同一轮里 header 那条带子的裁决是反方向：系统材质全部自带罩色或造型，
带子维持手绘纯 CSS 模糊，记录在本包 `lib/client.js` 的 HEADER 段带子注释里）。
...
1. **绘制次序**：`--surf-tint`（inset box-shadow）与按压泛光（`background-image`）
...
4. **hover / 按下层次够不够**：只剩 tint 两级。不够先恢复 `--surf-glass-drop`。
...
| 现成消费者 | surf-settings 通用页 | `surf-settings/swift/SettingsTabs.swift` 的 `GeneralRow(ns: "ui-theme", path: ["preference"])` |
...
刷色的目标窗口判据是「**它装着壳那个 WebView**」（`SurfObjects.Key.webView` 的
`.window`），不是 `NSApp.mainWindow`——后者会在 surf-settings 那扇窗打开时指过去，
把一扇不显示网页的窗户也刷成页面底色。

### 退休语义

把 `ui-surf-nativeify` 从伞包的编排表里摘掉，**当前那个壳进程会保持最后一次设定**
（我们不在析构里恢复，理由见上）。壳重启即回到 `.windowBackgroundColor` + 系统外观。

## 不在这里的

- **`window.__surf` 动作桥、收起 web 侧边栏、rail 轨道抵消**：那些是原生分栏接管
  排版的一部分，住在 surf-layout 的 client 半边（协议两端同包：Swift 侧
...
- **网页侧边栏的任何外观调整**（顶部让位 `topInset`、把 `sidebarCol` 刷成透明
  ...
  |---|---|---|
  | 原生侧边栏（常态） | surf-sidebar 占 sidebar 槽 | `NSSplitViewItem` 右侧，够不着侧边栏那一栏 |
...
**跟着伞包走，没有单装路径。** 仓库根 `./dev` 会把本包连同其余 surf-* 一起 link
进 profile；装哪些、什么顺序由伞包 `@wenbo/surf` 的 `cordis.patch.yml` 决定
（本包是那张表里的 `ui-surf-nativeify` row）。

> **P4 之后本包不能再单独塞进一个普通 web profile 了**：它现在带 Swift 载荷，
> 因而硬依赖同一张表里的 `surfBridge`（`createSwiftPlugin` 自动加的）。桥不在
...
```bash
node surf-nativeify/tools/dump-css.mjs              # 全量
node surf-nativeify/tools/dump-css.mjs nofx _primary  # 只看含关键字的规则块
```
...
- **`_primary` 的高光依赖 `--dsw-alias-button-info-fill`**（发送键自己的底色）。
  这条的失效方向是安全的：`--surf-glass-glow-c` 注册成了 `@property <color>`，
...
- 完整网页模式（逃生舱）下红绿灯会压在网页侧边栏顶部——**这是刻意接受的**：
  逃生舱的定位是「surf-layout 挂了也还能用」的降级路径，不为它做外观修补。
