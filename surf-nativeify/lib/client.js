/*
 * surf-nativeify，浏览器半边（lazy-CJS 经典脚本，手写、无构建步骤）。
 *
 ...
 *
 * **不在这里的**：
 * - `window.__surf` 动作桥、收起 web 侧边栏、rail 轨道抵消——那些是原生分栏
 *   接管排版的一部分，住在 surf-layout 的 client 半边（协议两端同包：Swift 侧
 *   `WebViewConversationSurface` 是它们唯一的调用方）。
 * - 网页侧边栏的任何外观调整（顶部让位、背景透明化以透出原生玻璃）。这些是
 *   「网页侧边栏坐在壳的 NSGlassEffectView 上」那个旧世界的产物，已随 M6 作废：
 *   现在只有两种形态——原生侧边栏（surf-layout 占 root 槽，WebView 装在分栏右侧，
 *   够不着侧边栏那一栏）或完整网页模式（全出血逃生舱，原样展示 dsh UI，不修）。
 *   「用网页侧边栏但把它打扮成原生」这个中间态不存在，别再往回加。
 *
 * 门控只有一条：UA 含 "Surf/"（带斜杠，防普通子串误命中）。终端 `dsh web` /
...
window.__ModuleLoader__.load({
	id: "@wenbo/surf-nativeify",
	factory: () => {
		var module = { exports: {} };
		var exports = module.exports;
		Object.defineProperty(exports, Symbol.toStringTag, { value: "Module" });

		const STYLE_ID = "surf-nativeify-style";
		/**
		 ...
		 */
		const FONT_STYLE_ID = "surf-nativeify-font";
		...
		// 这里以前是一层 `inset 0 0 0 100px var(--surf-glass-fill)`，理由写的是
		...
		// 只会命中第一条选择器。
		const blurSolid = SOLID_BUTTONS.map((sel) => ':root[data-surf-blur] ' + sel).join(",\n");
		// 切焦点那一帧用的「禁过渡」选择器，同样得逐条加前缀。
		const nofxSolid = SOLID_BUTTONS.map((sel) => ':root[data-surf-nofx] ' + sel).join(",\n");
		// 真材质接管的两块表面共用的三层门控里的后两层（第一层是 @supports）。
		// **只在窗口激活时接管** —— 失活整个交还给手绘四态/dsh 原样，理由见文件末尾
		// @supports 块的注释。
		const MATERIAL_GATE = ':root:not([data-surf-blur]):not([data-surf-reduce]) ';
		...
		const reduceNeutral = NEUTRAL.map((sel) => ':root[data-surf-reduce] ' + sel).join(",\n");
		// 真材质接管的第二块表面（P6）：composer 输入卡片。**不是按钮**，所以不进
		// NEUTRAL —— 它没有 hover / 按下两级层次、不吃 `--surf-tint`，与按钮组共用的
		// 只有上面那三层门控。
		...
		const lh = (size) => {
			...
			if (!v) throw new Error(`surf-nativeify: ${size}px 没有实测行高，先补测再用`);
			return v;
		};
		...
		/**
		 * 对话阅读列的字号（设置 → 插件 → surf-nativeify → 对话区字号；本 ns 的
		...
		function headerRules() {
			...
			return [
				...
				S + " {",
				"  --surf-webheader-title: rgb(54, 54, 54);",
				"  --surf-webheader-subtitle: rgb(178, 178, 178);",
				"  --surf-webheader-seg-fill: rgba(0, 0, 0, 0.07);",
				"  --surf-webheader-seg-sep: rgba(0, 0, 0, 0.05);",
				"}",
				"body[data-ds-dark-theme] " + S + " {",
				"  --surf-webheader-title: rgba(255, 255, 255, 0.96);",
				"  --surf-webheader-subtitle: rgb(138, 138, 138);",
				"  --surf-webheader-seg-fill: rgba(255, 255, 255, 0.08);",
				"  --surf-webheader-seg-sep: rgba(255, 255, 255, 0.06);",
				"}",
				":root[data-surf-blur] " + S + " {",
				"  --surf-webheader-title: rgb(178, 178, 178);",
				"  --surf-webheader-seg-fill: rgba(0, 0, 0, 0.05);",
				"}",
				":root[data-surf-blur] body[data-ds-dark-theme] " + S + " {",
				"  --surf-webheader-title: rgba(255, 255, 255, 0.55);",
				"  --surf-webheader-seg-fill: rgba(255, 255, 255, 0.06);",
				"}",
				...
				"  font-weight: 500;",
				"  color: var(--surf-webheader-subtitle);",
				"}",
				...
				"  font-weight: 600;",
				"  color: var(--surf-webheader-title);",
				"}",
				...
				// 用的是同一批 `--surf-glass-*` 变量，四态跟着走，不另起一套。
				S + ' [role="tablist"] {',
				"  box-sizing: border-box;",
				"  height: 36px;",
				"  padding: 4px;",
				...
				"  align-items: center;",
				"  --surf-surface:",
				"    inset 0 0 0 var(--surf-glass-edge-w) var(--surf-glass-edge),",
				...glowLayers(),
				"    inset 14px 0 8px -15px var(--surf-glass-side),",
				"    inset -14px 0 8px -15px var(--surf-glass-side);",
				"  background-color: var(--surf-glass-fill);",
				"  backdrop-filter: blur(var(--surf-glass-blur)) saturate(var(--surf-glass-sat));",
				"  box-shadow: var(--surf-surface), 0 1px 2px var(--surf-glass-drop), var(--surf-glass-outer);",
				"  transition: scale var(--surf-dur) var(--surf-ease);",
				"}",
				...
				"  font-weight: 500;",
				"  color: var(--surf-webheader-title);",
				"  transition: background-color var(--surf-dur-fast) linear;",
				"}",
				S + ' [role="tab"]::after { display: none; }',
				S + ' [role="tab"]:hover:not([aria-selected="true"]) {',
				"  background: var(--surf-tint-hover);",
				"}",
				S + ' [role="tab"][aria-selected="true"] {',
				"  background: var(--surf-webheader-seg-fill);",
				"  color: var(--surf-webheader-title);",
				"}",
				...
				"  width: 1px;",
				"  height: 16px;",
				"  background: var(--surf-webheader-seg-sep);",
				"}",
				...
				// （Apple 自家 App 内容穿过时也有一根极淡的）。显隐由 --surf-scroll-edge
				...
				"  opacity: var(--surf-scroll-edge, 0);",
				"}",
				// 退到 Hard 形态——不透明底、无模糊、齐边收口不渐出。两个入口：
				// 系统「减少透明度」，以及设置里把 headerScrollBlur 关掉
				// （品味开关，node 半边注册；client 侧 sync 翻成 data-surf-webheader-noblur）。
				// 细线（下面那条 ::after）在两种形态下都在，不跟着开关走。
				":root[data-surf-reduce] " + S + ' > header:not([class*="_headerHidden"])::before,',
				":root[data-surf-webheader-noblur] " + S + ' > header:not([class*="_headerHidden"])::before {',
				"  height: 100%;",
		...
		function insideSurf() {
			try {
				return navigator.userAgent.includes("Surf/");
			} catch {
				return false;
			}
		}
		 ...
		 */
		/**
		 * 把承载窗口的激活态映射到 <html data-surf-blur>。
		 *
		 * 页面拿不到 AppKit 的窗口状态，但 WKWebView 会把承载窗口的激活/失活
		 * 转成页面的 focus/blur 事件（Safari 里切 app 也是同一套）。
		 *
		 * **刻意不走"让壳注入"那条路**：壳的窗口通知要经 surf-layout 的 Swift
		 * 半边才够得着 WebView，那会给 surf-nativeify 添一条跨插件契约。收不到
		 ...
		 *
		 *   --surf-glass-glow-t / -b   最外一像素的峰值（上下可以不等，深色下确实不等）
		 *   --surf-glass-glow-d        每向内一像素乘的衰减系数，越小掉得越快
		 *   --surf-glass-glow-c        发光的颜色（`<color>`，无色玻璃是白，带色玻璃
		...
		function glowLayers() {
			const out = [];
			for (let k = 1; k <= 4; k++) {
				const decay = "*var(--surf-glass-glow-d)".repeat(k - 1);
				out.push(`    inset 0 ${k}px 0 rgb(from var(--surf-glass-glow-c) r g b / calc(var(--surf-glass-glow-t)${decay})),`);
				out.push(`    inset 0 -${k}px 0 rgb(from var(--surf-glass-glow-c) r g b / calc(var(--surf-glass-glow-b)${decay})),`);
			}
			return out;
		}
		...
		function menuRules() {
			...
			return [
				// ── 配色变量（浅色缺省 + 深色覆写一处收口）──────────────────────
				box + " {",
				"  --surf-menu-text: rgb(76, 76, 76);",
				"  --surf-menu-header: rgb(115, 115, 115);",
				"  --surf-menu-sep: rgba(0, 0, 0, 0.07);",
				"}",
				boxDark + " {",
				"  --surf-menu-text: rgba(255, 255, 255, 0.96);",
				"  --surf-menu-header: rgba(255, 255, 255, 0.5);",
				"  --surf-menu-sep: rgba(255, 255, 255, 0.08);",
				"}",
				...
				"  border: 0;",
				"  margin: 5px 6px;",
				"  background: var(--surf-menu-sep);",
				"}",
				label + " {",
				"  padding: 3px 11px;",
				"  font-size: 13px;",
				"  font-weight: 700;",
				"  color: var(--surf-menu-header);",
				"}",
		...
		/**
		 * 把按下的位置写成按钮上的 --surf-px / --surf-py（百分比）。
		 *
		...
		function watchPressPoint(sel) {
			const onDown = (e) => {
				...
				el.style.setProperty("--surf-px", ((e.clientX - r.left) / r.width * 100).toFixed(1) + "%");
				el.style.setProperty("--surf-py", ((e.clientY - r.top) / r.height * 100).toFixed(1) + "%");
				...
				el.style.setProperty("--surf-s", Math.min(1.03, 1 + 2.5 / Math.max(r.width, r.height)).toFixed(4));
			};
		 ...
		 * 属性值没法存对象引用，所以这里存一个随机串（同 surf-layout 的 makeToken）。
		 */
		function makeToken() {
		...
		function watchDragPassthrough() {
			...
			const post = () => {
				...
				last = json;
				try {
					window.webkit.messageHandlers.surf.postMessage({ type: "dragPassthrough", rects });
				} catch {
		 ...
		 * 的 --surf-scroll-edge，headerRules 那节的模糊带与细线拿它当 opacity。
		 *
		...
		function watchScrollEdge() {
			...
			const apply = () => {
				...
				root.style.setProperty("--surf-scroll-edge", t.toFixed(3));
			};
			const bind = () => {
		...
		function watchWindowFocus() {
			...
			//
			// 老办法：先挂 data-surf-nofx（配套 CSS 把过渡整个关掉）→ 改值 →
			// **读一次 offsetHeight 强制刷新样式**，让新值以「无过渡」落定 → 摘掉标记。
			// 那次读取是整段的关键，删了就等于没加：不强制刷新，浏览器会把加标记、改值、
			// 摘标记合成一次样式计算，过渡照旧发生。
			//
			// **属性值写实例 token**（CSS 那边一律是 `[data-surf-blur]` 存在性匹配，
			// 不看值，所以带值不影响任何选择器）。sync() 内部那对 set/remove 是同一段
			// 同步代码，自己摘自己，不需要校验；需要校验的是下面的 cleanup。
			const sync = () => {
				root.setAttribute("data-surf-nofx", token);
				if (document.hasFocus()) root.removeAttribute("data-surf-blur");
				else root.setAttribute("data-surf-blur", token);
				void root.offsetHeight;
				root.removeAttribute("data-surf-nofx");
			};
			...
			return () => {
				...
				// 砍掉新实例刚刚打上的 data-surf-blur —— 页面明明失焦，按钮却停在
				// 激活态那摞玻璃上，而且要等到下一次 focus/blur 才自愈（窗口一直
				// 待在后台的话就永远不自愈）。这正是 CLAUDE.md 那条坑的形状。
				if (root.getAttribute("data-surf-blur") === token) root.removeAttribute("data-surf-blur");
				if (root.getAttribute("data-surf-nofx") === token) root.removeAttribute("data-surf-nofx");
			};
		}

		/**
		 * 把「减少透明度」翻译成根属性 `data-surf-reduce`，给 CSS 当门控。
		 *
		...
		function watchReduceTransparency() {
			...
			const sync = () => {
				if (mq && mq.matches) root.setAttribute("data-surf-reduce", token);
				else if (root.getAttribute("data-surf-reduce") === token) root.removeAttribute("data-surf-reduce");
			};
			mq?.addEventListener?.("change", sync);
			sync();
			return () => {
				mq?.removeEventListener?.("change", sync);
				if (root.getAttribute("data-surf-reduce") === token) root.removeAttribute("data-surf-reduce");
			};
		}
		...
		function apply(ctx) {
			...
			// 段控胶囊和 composer 卡片也要喂 --surf-s（整块表面按压缩放）。
			...
			ctx.effect(() => {
				...
				const rules = [
					...
					// 这里原来有第三个常数 `--surf-dur-press: 90ms`：短，但仍是一段补间，
					// 所以按下去总差着一帧的"黏"。现在 `:active` 那条直接写
					// `transition-duration: 0s`，**只有"进入按下"这个方向瞬时**，
					// 离开 :active 时那条规则不再命中，自动回到下面这两档。
					// 一个恒等于 0 的旋钮没有存在价值，`--surf-dur-press` 随之退休。
					":root {",
					"  --surf-ease: cubic-bezier(0.32, 0.72, 0, 1);",
					"  --surf-dur-fast: 160ms;",
					"  --surf-dur: 320ms;",
					...
					//    这条直线预测中灰 127 → 199.3，实测 199.2。所以下面的 --surf-glass-fill
					...
					"  --surf-glass-sat: 1.95;",
					"  --surf-glass-edge: rgba(0, 0, 0, 0.197);",
					"  --surf-glass-edge-w: 0.25px;",
					...
					"  --surf-glass-glow-c: #fff;",
					"  --surf-glass-glow-t: 0.795;",
					"  --surf-glass-glow-b: 0.795;",
					"  --surf-glass-glow-d: 0.45;",
					"  --surf-glass-side: rgba(0, 0, 0, 0.119);",
					// 底色 = 元素的 background-color（不是阴影层）。**这是唯一的给色处**；
					// 半透明 = 页面透上来，压到 1 就是实色。名字以前叫 -body，读起来像"玻璃本体"，
					// 正是那个"再叠一层材料"的错误心智模型的来源，已改名。
					"  --surf-glass-fill: rgba(252, 252, 252, 0.5272);",
					...
					"  --surf-tint-hover: rgba(0, 0, 0, 0.035);",
					"  --surf-tint-press: rgba(0, 0, 0, 0.070);",
					"  --surf-glass-drop: rgba(0, 0, 0, 0.05);",
					// 官方套件里玻璃唯一的真投影层（Liquid Glass Regular-Small 的
					// shadow=(0,8,blur 15)，浅 2% / 深 4% 黑）——非常轻，但没有它按钮
					// 就"浮不起来"（用户 2026-08-29 对照系统按钮发现缺了这层）。
					// 与 --surf-glass-drop（贴地接触影，hover 抬高那套）分工不同，两层并存。
					"  --surf-glass-outer: 0 8px 15px rgba(0, 0, 0, 0.02);",
					"}",
					...
					"  --surf-glass-edge: rgba(255, 255, 255, 0);",
					"  --surf-glass-edge-w: 0.25px;",
					// 系统在深色下上下**不等强**（+4.7 vs +8.0，与浅色反过来），但定稿在校准台上
					// 眼调回了等强 —— 深色玻璃本体已经比背景亮 37，上下差那 3 级看不出来，
					// 反倒是整体的发光强度和衰减速度更要紧。峰值分上下两个变量仍留着，随时能拆开。
					"  --surf-glass-glow-t: 0.06;",
					"  --surf-glass-glow-b: 0.06;",
					"  --surf-glass-glow-d: 0.5;",
					"  --surf-glass-side: rgba(0, 0, 0, 0);",
					"  --surf-glass-fill: rgba(255, 255, 255, 0.137);",
					// 深色档余量大得多，扫过 .20/.30/.40/.55 后取 .30：再高就白成一块。
					"  --surf-press-glow: rgba(255, 255, 255, 0.17);",
					// 深色档 hover 提亮。系统实测 66.6 → 88.5（+21.9），**按下与悬停逐像素
					// 完全一致** —— 深色那档的高光到 hover 就到顶了，按下不再加码。我们仍然在
					// 按下时多叠一层跟指针走的泛光（--surf-press-glow），那是自己要的层次：
					// 闲时 → 悬停（整面提亮）→ 按下（手指底下再亮一块），系统只有前两级。
					"  --surf-tint-hover: rgba(255, 255, 255, 0.113);",
					"  --surf-tint-press: rgba(255, 255, 255, 0.113);",
					"  --surf-glass-drop: rgba(0, 0, 0, 0.25);",
					"  --surf-glass-outer: 0 8px 15px rgba(0, 0, 0, 0.04);",
					"}",
					...
					//
					// 触发靠 data-surf-blur，由下面 watchWindowFocus() 打在 <html> 上。
					// 特异性：:root[…] body 是 (0,2,2)，稳压上面 body[data-ds-dark-theme] 的
					// (0,1,1)；深色失活再多一个属性选择器，压住浅色失活。
					":root[data-surf-blur] body {",
					// 失活描边反过来走「更宽更淡」：0.6px/.095 的墨量（.057）比激活的
					// 0.25px/.197（.049）略重，但摊开后是一圈灰雾而不是一道锐线 —— 正是
					// 失活该有的"退到背景里"的样子。
					"  --surf-glass-edge: rgba(0, 0, 0, 0.095);",
					"  --surf-glass-edge-w: 0.6px;",
					"  --surf-glass-glow-t: 0;",
					"  --surf-glass-glow-b: 0;",
					"  --surf-glass-side: transparent;",
					"  --surf-glass-fill: rgba(0, 0, 0, 0.059);",
					"}",
					":root[data-surf-blur] body[data-ds-dark-theme] {",
					"  --surf-glass-edge: rgba(255, 255, 255, 0);",
					"  --surf-glass-glow-t: 0;",
					"  --surf-glass-glow-b: 0;",
					"  --surf-glass-side: transparent;",
					"  --surf-glass-fill: rgba(255, 255, 255, 0.094);",
					"}",
					...
					"    inset 0 0 0 100px var(--surf-tint),",
					"    inset 14px 0 8px -15px var(--surf-glass-side),",
					"    inset -14px 0 8px -15px var(--surf-glass-side);",
					"  box-shadow: var(--surf-surface), 0 1px 2px var(--surf-glass-drop), var(--surf-glass-outer);",
					"}",
					...
					neutral + " {",
					"  background-color: var(--surf-glass-fill) !important;",
					// 材质层。**跟着 --surf-glass-fill 走，只给这一组**：`_primary` 那枚
					// 强调键是 dsh 自己画的实色，背后糊什么都看不见，给它上 backdrop-filter
					// 只是白白多一个合成层。
					//
					// 不加 `-webkit-` 前缀：本插件由 UA 门控（含 `Surf/`），只会跑在壳的
					...
					solid + " {",
					"  transition:",
					"    scale var(--surf-dur) var(--surf-ease),",
					"    background-color var(--surf-dur-fast) linear,",
					"    box-shadow var(--surf-dur-fast) var(--surf-ease),",
					"    --surf-press-r var(--surf-dur) var(--surf-ease),",
					"    --surf-press-a var(--surf-dur) var(--surf-ease),",
					"    filter var(--surf-dur-fast) linear,",
					"    color var(--surf-dur-fast) linear !important;",
					"}",
					// 无 border 的那组：把描边补进 --surf-surface 的最前面（层序在最上，
					...
					tinted + " {",
					"  --surf-surface: inset 0 0 0 100px var(--surf-tint);",
					"  --surf-glass-drop: transparent;",
					"}",
					...
					// 按钮横向涨十几像素，用户裁决改小并封顶。
					"  scale: var(--surf-s, 1.03);",
					// 150% 就是闲时那个值 —— **定稿故意不收拢**：亮光只淡入淡出，是整面泛光，
					// 不是聚成一小块。位置仍然跟着 --surf-px/--surf-py 走（渐变中心在手指底下，
					// 只是摊得很开）。这一行留着是为了把"不收拢"写在用它的地方，改回收拢就调它。
					"  --surf-press-r: 150%;",
					"  --surf-press-a: var(--surf-press-glow);",
					// **按下即时、松开缓动**（手册量原生控件得来的时序）。这条只在
					// :active 命中时生效，所以它只压住"进入按下"那一次；松手时规则
					// 不再命中，上面 solid 那条 transition 的 --surf-dur / -fast
					...
					// **没注册的自定义属性不参与插值**，会直接跳变。位置走 --surf-px/--surf-py，
					...
					// 不注册它，闲时 var(--surf-tint) 解析不出来会让**整条 box-shadow 失效**，
					...
					"}",
					"@property --surf-press-r {",
					"  syntax: \"<percentage>\";",
					"  inherits: false;",
					"  initial-value: 150%;",
					"}",
					"@property --surf-press-a {",
					"  syntax: \"<color>\";",
					"  inherits: false;",
					"  initial-value: transparent;",
					"}",
					// 发光色也得注册，理由和 --surf-tint 完全一样、而且更硬：带色按钮
					...
					// 整个没了，而且是静默的 —— 不报错、不留痕。
					"  background-image: radial-gradient(circle at var(--surf-px, 50%) var(--surf-py, 50%),",
					"    var(--surf-press-a) 0%, transparent var(--surf-press-r)) !important;",
					"}",

					// 悬停：整面着色（浅色变暗 / 深色提亮，见上面两组 tint 变量），
					// 外加把贴地投影抬高一档（"浮起来"）。表面其余层原样保留 ——
					// box-shadow 是整体覆盖的，所以这里必须重抄 var(--surf-surface)。
					solidHover + " {",
					"  --surf-tint: var(--surf-tint-hover);",
					"  box-shadow: var(--surf-surface), 0 2px 5px var(--surf-glass-drop), var(--surf-glass-outer);",
					"}",
					...
					"  --surf-tint: transparent;",
					"}",
					// 按下：着色加深一档 + 投影压回去（贴地），配合容器放大，像被按进桌面。
					// 排在 hover 之后，所以两态同时命中时这条赢（含上面那条 _primary 的清零）。
					solidActive + " {",
					"  --surf-tint: var(--surf-tint-press);",
					"  box-shadow: var(--surf-surface), 0 0 1px var(--surf-glass-drop), var(--surf-glass-outer);",
					"}",

					// 尊重"减少动态效果"：关掉形变，保留配色反馈。
					"@media (prefers-reduced-motion: reduce) {",
					"  " + solidActive + " { scale: 1 !important; }",
					"  " + solidActive + " { --surf-press-r: 100% !important; }",
					"}",
					...
					// 画的实色，本就不透明。
					// **门控是根属性 `data-surf-reduce`，不是 `@media (prefers-reduced-transparency)`**
					// —— 本机 WebKit 不认识那个特性，媒体查询两个分支双双恒假（见
					// watchReduceTransparency 顶注，真 App 里用角标实测过）。属性由 JS 的
					// matchMedia 打上，选择器带上它之后特异性高于被覆盖的那条，不再依赖
					// "排在整张表最后"的源码顺序。
					":root[data-surf-reduce] { --surf-glass-fill: #FDFDFD; }",
					":root[data-surf-reduce] body[data-ds-dark-theme] { --surf-glass-fill: #3D3D3D; }",
					":root[data-surf-reduce][data-surf-blur] body { --surf-glass-fill: #F0F0F0; }",
					":root[data-surf-reduce][data-surf-blur] body[data-ds-dark-theme] { --surf-glass-fill: #333333; }",
					...
					"  opacity: 1;",
					"  transition-duration: var(--surf-dur-fast);",
					"}",
					"@media (prefers-reduced-motion: reduce) {",
					...
					//   ② `:not([data-surf-reduce])`              "别让我透过控件看背景"
					...
					//   ③ `:root:not([data-surf-blur])`           窗口激活态
					//
					// **②③ 刻意写成条件而不是覆写**，虽然计划里写的是"块内覆写
					// `-apple-visual-effect: none` 再让手绘层重新生效"。两者的计算值
					// 完全等价，代价却差得远：覆写那条路要在块内把手绘的
					// `--surf-surface` 整摞重新列一遍（还得分 plain / bordered 两组，
					// 因为只有前者带描边层），并把 `--surf-glass-drop` 按浅深两档复述
					...
					// data-surf-reduce / data-surf-blur），但**档位不同**：按钮走
			...
			// 设置面：`surf-nativeify` 这个命名空间由本包的 node 半边注册，
			...
			ctx.inject(["settingsScope"], (scoped) => {
				const scope = scoped.settingsScope.bind({ namespace: "surf-nativeify" });
				const sync = () => {
					...
					// 一撤就没有规则再读这个属性（同 --surf-scroll-edge 的理由）。
					const blur = ready ? snap.value?.headerScrollBlur !== false : true;
					const root = document.documentElement;
					if (blur) root.removeAttribute("data-surf-webheader-noblur");
					else root.setAttribute("data-surf-webheader-noblur", "");
				};
