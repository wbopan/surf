...
/// surf-nativeify 的 Swift 半身：**让原生那半边跟着 dsh 的主题走**
...
final class NativeifyPlugin: SurfPlugin {
...
final class ThemeFollower {
    ...
    func start() {
        ...
        handle.keep(SurfDisposable { NotificationCenter.default.removeObserver(token) })
    ...
    /// ——后者会在 surf-settings 那扇窗打开时指过去，把设置窗的底色一起改了
    /// （那扇窗该用系统的 `.windowBackgroundColor`，它不显示网页）。
    private func paintWindow() {
        guard let painted,
              let webView = host.objects.object(SurfObjects.Key.webView, as: WKWebView.self),
