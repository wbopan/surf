...
权威计划：`docs/archive/surf-memory-plan.md`。
...
设置里 ns `surf-memory`、键 `dir`（也可以在编排表的 `config.dir` 里给缺省，
设置界面里的值优先）：
...
前两段合成 `surf-memory:memories`，第三段是 `surf-memory:maintenance`。
...
它待在 `surf-*` 家族里只是出身，任何一台装了 dsh 的机器（含 Linux）都能单独用：

```sh
dsh plugin add @wenbo/surf-memory
```

在本仓库这套编排里它已经在 `surf/cordis.patch.yml` 末尾了，`./dev` 起来即有。
...
```sh
node --test surf-memory/test/*.test.js   # 零依赖、约 0.1s。别省那个通配符
```
