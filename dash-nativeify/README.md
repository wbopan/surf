...
3. **按钮按压手感**（macOS 26 / Tahoe）：按下时容器 `scale: 1.09`，**内容跟着容器
   ...
   留余韵。悬停另加一层极轻的浮起投影。遵循 `prefers-reduced-motion`。
   ...
   定稿两档同取 `rgba(255,255,255,.17)`。**这个值是在深色档上眼调的，浅色档实测抬不动**：

   | | 闲时 | 按住 | 差 |
   |---|---|---|---|
   | 深色 | 61.1 | 92.1 | **+31** |
   | 浅色 | 248.1 | 249.1 | **+1**（一个量化步长） |

   浅色玻璃本体已经 248/255，头顶只剩 7 级，纯白也抬不出多少 —— 白色泛光在浅色档
   物理上就做不出深色那种效果。真要浅色也有反馈，只能反过来**压暗**：
   `rgba(0,0,0,.06)` → 242（−6），`rgba(0,0,0,.10)` → 237（−11）。
   那是"压下去"不是"泛起光"，是另一种语言，所以没有擅自替换。
...
落到 CSS 是描边 + 8 条发光 + 左右阴影 + 底色，再加一层外投影，层序自上而下
（`box-shadow` 里先写的盖后写的）：
...
底色          inset 0 0 0 100px             spread 撑满 = 纯色填充
...
底色用 `inset 0 0 0 100px` 而不是 `background-image: linear-gradient`：前者不碰
