...
3. **按钮按压手感**（macOS 26 / Tahoe）：按下时容器 `scale: 1.06`，**内容跟着容器
   一起走**——`scale` 会往下继承，不需要也不该给图标再写一层。图标只额外压一档
   `opacity: .7`。缓动走 Apple 全系基准曲线 `cubic-bezier(.32,.72,0,1)`，按下 80ms
   跟手、松开 320ms 留余韵。悬停另加一层极轻的浮起投影。遵循 `prefers-reduced-motion`。
