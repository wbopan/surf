...
///
/// 这是 §0.5-5 的落地点：clam-layout 被禁用、编译失败、或者根本没装，
/// surfclam 都还是一个能用的 dsh 客户端，只是没有原生外壳而已。
///
/// `.id(version)` 是世代替换的落地点（§6.3-2）：插件换代 → 版本号跳变 →
/// SwiftUI 整棵重建、`@State` 归零，需要活过替换的状态由 `ClamStore`
...
///
/// 与 clam-layout 借用的是**同一个实例**（保管箱里那个），所以在
