...
enum ConnectionPhase: Equatable, Sendable {
    ...
    /// 诊断面板与 `clam.connection.state` 投影用的稳定标识（不随界面语言变）。
    var key: String {
        switch self {
...
final class ConnectionController {
    ...
    /// 此刻接着的那个端点。nil = 连接页在场。
    private(set) var activeEndpoint: ClamEndpoint?
    ...
    /// 选中了一个端点：装页面 + 连桥。
    var onAttach: ((ClamEndpoint) -> Void)?
    /// 放开当前端点：停桥、停加载。
    var onDetach: (() -> Void)?
    /// 幕变了：盖上/撤掉连接页、重画。
    ...
    private let events: ClamEventBus
    ...
    private var manualTarget: ClamEndpoint?
    ...
    /// 但那个 profile 里没有任何 clam 插件，桥永远 404。不判定的话 App 会
    ...
    func connect(to endpoint: ClamEndpoint) {
    ...
    func connect(toURLString raw: String) -> Bool {
        guard let url = Self.normalizedURL(from: raw) else {
            Log.write("手动连接的地址不成立：\(raw)", to: ClamPaths.logURL, tag: "connection")
            return false
        }
        Log.write("手动连接：\(url.absoluteString)", to: ClamPaths.logURL, tag: "connection")
    ...
    /// 不改幕——**除非连拒三次**：那说明这个后端根本没有 clam 桥（比如
    ...
    /// 判定当前后端不含 Surfclam 组件：放弃它、回引导页、把原因留在
    /// `.bridgeRejected` 上（页面据此附上正确的启动命令提示）。
    private func declareIncompatible() {
        guard let endpoint = activeEndpoint else { return }
        Log.write("桥连续被拒，判定后端不含 clam 组件：\(endpoint.summary)",
                  to: ClamPaths.logURL, tag: "connection")
    ...
    /// 返回"该连谁"与"全部候选的健康态"。
    private func probeRound() async -> (ClamEndpoint?, [CandidateStatus]) {
    ...
    private func adopt(_ endpoint: ClamEndpoint, isReconnect: Bool) {
    ...
    private func enterDisconnected(_ reason: DisconnectReason) {
        ...
        Log.write("与 dsh 断开连接（\(reason)）", to: ClamPaths.logURL, tag: "endpoint")
        disconnectedAt = Date()
        releaseEndpoint()
    ...
    private func setPhase(_ next: ConnectionPhase) {
        ...
        phase = next
        Log.write("连接状态：\(next.key)", to: ClamPaths.logURL, tag: "connection")
        onPhaseChange?(next)
        projectState()
    }
    ...
    /// 粘性主题 `clam.connection.state`。**状态型消息一律 emitSticky**
