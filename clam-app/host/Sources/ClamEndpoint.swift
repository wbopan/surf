...
struct ClamEndpoint: Equatable {
    ...
    /// 这套 dsh 是不是"我这一套"——它的 clam-app 与本产物出自同一个 worktree。
    ///
...
enum EndpointLocator {
    ...
    static func probe(_ endpoint: ClamEndpoint, timeout: TimeInterval = 1.5) async -> Bool {
    ...
    static func locateHealthy() async -> ClamEndpoint? {
