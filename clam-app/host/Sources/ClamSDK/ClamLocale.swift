...
public enum ClamLocale: String, Sendable, CaseIterable {
    ...
    public static func resolve(preferred: [String]) -> ClamLocale {
        for tag in preferred {
            let primary = tag.lowercased().split(separator: "-").first.map(String.init) ?? ""
            if let hit = ClamLocale(rawValue: primary) { return hit }
        }
        return .en
    }
}

/// 语言的可观察持有者：订上粘性主题 `clam.locale`，值一变 `current` 就变，
...
///
/// 用法：插件在 `activate` 里 `ClamLocaleStore(bus: host.events)`，塞进自家 model
/// （model 被视图闭包捕获 = 有生命周期锚），视图读 `model.locale.current`。
/// 订阅句柄由本对象持有，对象被回收时订阅一并撤销。
///
/// 线程约定同总线：**只在主线程使用**。
@Observable
public final class ClamLocaleStore {
    /// 此刻在役的语言。
    public private(set) var current: ClamLocale

    /// 订阅句柄。**必须有人接住**：`ClamDisposable` 一析构就把订阅撤了，
    /// 不存下来等于订完当场退订——不报错、不打日志，只是语言永远不跟着变。
    @ObservationIgnored private var subscription: ClamDisposable?

    /// - Parameters:
    ///   - bus: 事件总线（`host.events`）。
    ///   - initial: 还没收到任何广播时的取值。`clam.locale` 是**粘性**主题，
    ///     壳在启动第一句就 `emitSticky` 过一份，所以这个初值通常在
    ///     `subscribe` 那一刻就被同步覆盖掉了——它只兜住"壳还没发过"的时刻。
    public init(bus: ClamEventBus, initial: ClamLocale = .en) {
        current = initial
        // 粘性主题：subscribe 会同步回调一次最后那份载荷（见 ClamEventBus）。
        subscription = bus.subscribe(ClamEventBus.Topic.locale) { [weak self] payload in
            guard let self,
                  let raw = payload["locale"] as? String,
                  let next = ClamLocale(rawValue: raw),
                  next != self.current
            else { return }
            self.current = next
        }
