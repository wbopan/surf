...
/// 或系统框架。
///
/// SDK 与壳严格同版本：它住在壳源码里（`clam-app/host/Sources/ClamSDK/`），
...
/// - `@State` 里只放丢了不心疼的东西。想活过热替换的状态写进 `ClamStore`，
///   或者干脆放回 TS 半身。
...
/// - `ClamContributions`（多占用"贡献槽"注册表）+ `ClamHost.contributions`
///   + `ClamHost.contribute(to:id:order:metadata:make:)`。给"N 个插件各往一个
///   表面加一条"用（工具栏按钮是第一个消费者，在 clam-layout）。
/// - `ClamEventBus.Topic.pagePrefix`：页内桥的未知消息不再被壳白名单挡掉，
///   一律以 `clam.page.<type>` 广播。
/// - `ClamHooks`（应答式钩子表）+ `ClamHost.hooks` + `ClamHost.handle(hook:_:)`。
...
///
/// 注意：dylib 里的 `contentHash` 指纹含 ClamSDK 的 `.swiftinterface` 摘要，
/// 所以追加声明照样会让全部插件重编一次——这跟 ABI 版本号是两回事。
public let clamABIVersion = 1
...
/// （M2 实测往返无泄漏无过释放）。
public protocol ClamPlugin: AnyObject {
    /// 装载后立即调用一次。
    ///
    ...
    /// 约定把 `activate` 期间拿到的所有 `ClamDisposable` 攒进这个对象，
...
/// 旧注册在析构里自行退场（且只退自己那一份，见 `ClamRegistry.register`）。
public final class ClamDisposable {
...
/// 一组 `ClamDisposable` 的容器——插件 `activate` 的标准返回值。
public final class ClamPluginHandle {
    private var disposables: [ClamDisposable] = []

    public init() {}

    public func keep(_ disposable: ClamDisposable) {
        disposables.append(disposable)
    }

    /// 便于链式写：`host.registry.register(...).kept(by: handle)`。
    public func adopt(_ disposables: ClamDisposable...) {
...
extension ClamDisposable {
    /// `host.registry.register(...).kept(by: handle)`
    @discardableResult
    public func kept(by handle: ClamPluginHandle) -> ClamDisposable {
        handle.keep(self)
        return self
    }
}
