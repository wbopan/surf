...
#
# **排在 pack-payload.sh 之后**：源码从打包好的 `Resources/ClamNode/<pkg>/swift/`
# 读，"编的就是发的"因此是结构性的。三个 postBuildScripts 的顺序是
# embed-modules（ClamSDK 进 Frameworks/ClamModules）→ pack-payload（node 半边 +
