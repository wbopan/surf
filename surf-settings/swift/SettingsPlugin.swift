...
/// surf-settings 的 Swift 半身：一扇原生设置窗口，**不占任何槽**。
///
/// 不占槽是有意的：设置是自己的窗口，不该跟主窗口的布局抢地方，也不该依赖
/// surf-layout 在不在场——完整网页模式（逃生舱）下 ⌘, 照样能开出原生设置窗口。
///
...
/// 第一次 ⌘, 才是秒开而不是等一轮往返。
final class SettingsPlugin: SurfPlugin {
    ...
    func activate(host: SurfHost) -> AnyObject? {
        ...
        // 「连接」栏的数据面：壳的 UserDefaults + 粘性主题 `surf.connection.state`。
        ...
        // **先收拾上一代留下的窗口**。
        //
        // 实测：退休世代的 `SurfPluginHandle` 常常**不 deinit**（壳里 `loaded[name]`
        ...
        if let stale = host.objects.object(SurfObjects.Key.settingsOwner) as? NSWindow {
            host.log("关掉上一代留下的设置窗口")
            stale.orderOut(nil)
            stale.close()
        }

        // 占住"设置面板的主人"：surf-layout 见有主就不再用页内 modal 响应 ⌘,。
        // 不占的话两边都会响应——原生窗口开出来的同时主窗口里还弹一层网页 modal。
        // 窗口还没建，先放一个占位；建好之后换成窗口本身（见下面菜单回调）。
        let owner = NSObject()
        host.objects.setObject(SurfObjects.Key.settingsOwner, owner)
        SurfDisposable {
            ...
            let current = host.objects.object(SurfObjects.Key.settingsOwner)
            if current === owner || (current as? NSWindow) === controller?.window {
                host.objects.setObject(SurfObjects.Key.settingsOwner, nil)
        ...
        host.events.subscribe(SurfEventBus.Topic.menuCommand) { payload in
            ...
            if controller == nil {
                ...
                if host.objects.object(SurfObjects.Key.settingsOwner) === owner,
                   let window = created.window {
                    host.objects.setObject(SurfObjects.Key.settingsOwner, window)
                }
            }
            controller?.present()
        }.kept(by: handle)
