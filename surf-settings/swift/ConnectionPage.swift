...
///
/// **为什么不直接调壳的 `ConnectionController`**：surf-settings 的 swift 半边虽然
/// 与壳同进程，但它是运行时编出来的 dylib，`import` 不到壳里的类型（壳是预编译
/// 产物，不导出 module）。所以这一栏的数据面只有两种东西：
///
/// - **偏好**：`UserDefaults` 直接读写（键名字面量，权威在
///   `surf-app/host/Sources/Native/ConnectionController.swift`，登记在
///   `docs/extend/contracts.md` §10.2）；
/// - **状态**：订粘性主题 `surf.connection.state`（§4），只读。
///
...
final class ConnectionPrefs {
    ...
    static let modeKey = "surf.connection.mode"
    static let fixedURLKey = "surf.connection.fixedURL"
    ...
    // MARK: - 壳此刻跑着的那份（`surf.connection.state`）
    ...
    @ObservationIgnored private let bus: SurfEventBus
    /// **必须有人接住**：`SurfDisposable` 一析构就退订，而这一栏的状态行会
    /// 从此永远停在初值上（不报错、不打日志）。
    @ObservationIgnored private var subscription: SurfDisposable?

    init(bus: SurfEventBus) {
        self.bus = bus
