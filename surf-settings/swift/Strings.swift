...
/// （`docs/archive/surf-i18n-plan.md` §5「typed struct 就是完备性检查」）。
...
struct LocalizedText {
    ...
    subscript(_ locale: SurfLocale) -> String { locale == .zh ? zh : en }
}

/// surf-settings 的全部**用户可见**文案，zh 与 en 并排写在同一行（审校时一眼对照）。
///
/// ## 纪律（`docs/archive/surf-i18n-plan.md` §4/§5/§8，与壳、surf-sidebar 同一套）
///
/// - **只收用户看得见的字**。`host.log(...)` 与 node 半边的日志一律留在原地、
///   保持中文：日志的读者是蹲在终端前的开发者与 agent，跟着界面语言变只会让
///   排错时对不上账。
/// - **一条文案都不进 SurfSDK**：SDK 只有 `SurfLocale` 这个词汇。
...
/// ## 取值的地方
///
/// `SettingsModel.strings` **现算一份 `L`**（读 `SurfLocaleStore.current`，不存快照）。
...
/// `surf.locale`，回调里让 `SettingsWindowController.relocalize()` 重贴标签。
///
/// ## 数据不翻，只翻兜底词
///
