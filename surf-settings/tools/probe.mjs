#!/usr/bin/env node
/**
 * surf-settings 的桥探针：不开窗口、不碰屏幕，直接当一个"壳"连上 `/surf/bridge`，
 * 把 node 半边的数据面从头验一遍。
 *
 ...
 *
 * 用法：
 *   node surf-settings/tools/probe.mjs                  # 快照概览
 *   node surf-settings/tools/probe.mjs --ns shell       # 某个 ns 的详情
 *   node surf-settings/tools/probe.mjs --set            # 跑一遍写入/回滚/冲突用例
 *   node surf-settings/tools/probe.mjs --providers      # 模型页的数据
 *   node surf-settings/tools/probe.mjs --set-value ui-theme preference system
 *
 * 端点自动从 surf-app 的发现文件里找（`<AppSupport>/io.wenbo.surf/endpoints/`）。
 */
...
const ENDPOINT_DIR = join(homedir(), "Library/Application Support/io.wenbo.surf/endpoints");
...
const wsURL = base.replace(/^http/, "ws") + "/surf/bridge";
...
function invoke(action, payload = {}) {
	...
	return new Promise((resolve, reject) => {
		...
		ws.send(JSON.stringify({
			type: "invoke", plugin: "surf-settings", action, payload: { ...payload, id },
		}));
	});
}
...
ws.on("open", () => {
	ws.send(JSON.stringify({ type: "hello", clientId: "surf-settings-probe", protocolVersion: 1 }));
});

ws.on("message", (data) => {
	let frame;
	try { frame = JSON.parse(String(data)); } catch { return; }
	if (frame.type !== "push" || frame.plugin !== "surf-settings") return;
