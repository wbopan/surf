/**
 * surf-settings —— 原生设置窗口（计划 docs/archive/surf-settings-plan.md）。
 *
 ...
 * `DSHKit` 随 M10 退役，眼下唯一的共享 module 是 SurfSDK）。
 *
 ...
 *
 * **不 inject `surf-layout`**：设置是自己的窗口，不占任何槽，也不需要会话展示面
 * ——完整网页模式（layout 缺席的逃生舱）下 ⌘, 照样该开出原生设置窗口。
 *
 * @module surf-settings
 */
	...
	subscribe: (api) => {
		...
		const pushSettings = () => {
			try {
				...
				ctx.logger("surf-settings").warn(`快照失败：${String(error?.message ?? error)}`);
			}
		};
 ...
 * 回执。
 *
 * **桥本身是单向的**——`invoke` 的返回值被 surf-bridge 丢弃（只在抛错时记一行日志）。
