...
权威计划在 [`docs/archive/surf-settings-plan.md`](../docs/archive/surf-settings-plan.md)——
...
`SurfObjects.Key.settingsOwner` → surf-layout 见无主，⌘, 回落到 dsh 自己的页内
...
两张表，分工是「现算 vs 数据」（权威计划 [`docs/archive/surf-i18n-plan.md`](../docs/archive/surf-i18n-plan.md)）：

| | 收什么 | 形态 |
|---|---|---|
| [`swift/Strings.swift`](swift/Strings.swift) 的 `struct L` | 页面上散落的那些字（空态、按钮、提示条、表头、相位词典…） | `t("中文", "English")` 现算，读 `SurfLocaleStore.current` |
...
```sh
node surf-settings/tools/probe.mjs              # 快照概览：ns / 字段数 / 覆盖数 / revision
node surf-settings/tools/probe.mjs --ns shell   # 某个 ns 的 schema 与值
node surf-settings/tools/probe.mjs --providers  # 模型页的数据
node surf-settings/tools/probe.mjs --set        # 写入 / 回读 / 乐观锁冲突 / 还原
```
