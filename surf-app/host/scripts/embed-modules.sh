#!/bin/bash
# postBuild：把共享 module（目前只有 SurfSDK）摆进 app bundle，并给它签名。
#
#   Contents/Frameworks/lib<M>.dylib        ← 壳按 @rpath 加载的那一份
#   Contents/Resources/SurfModules/         ← 运行时编译插件用的 .swiftinterface/.swiftmodule
#
...
MODULES="$BUILT_PRODUCTS_DIR/$CONTENTS_FOLDER_PATH/Resources/SurfModules"
...
echo "embed-modules: SurfSDK → ${WRAPPER_NAME}（签名身份 ${IDENTITY}）"
