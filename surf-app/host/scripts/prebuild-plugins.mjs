 ...
 *   Contents/Resources/SurfPrebuilt.json                      清单（给人看的）
 *
 ...
 *   `surf-bridge/lib/swift-payload.js` 的 `swiftContentHash`，与桥同一份代码；
 * - **壳那半边的 hash**（工具链基线 + SurfSDK 接口摘要）与全部 swiftc 参数 →
 *   交给 `scripts/prebuild/` 那个工具，它把壳的 `Native/CompilerService.swift`
 *   原样编进去；
 * - **`swiftDeps` / `sharedModules` / `schemaVersion`** → `import` 各插件的
 *   node 半边，读 `createSwiftPlugin` 挂上去的 `surfSwift`。**静态解析源码是不行的**：
 *   猜错了不报错，只是 hash 对不上、预编译永远命中不了，静默退回现场编译。
 * - **源码** → 读**打包进 bundle 的那一份**（`Resources/SurfNode/<pkg>/swift/`），
 *   不读仓库。"编的就是发的"因此是结构性的，不靠"这两份应该一样"这种默契。
 *
 * `import` 插件模块需要 `@deepseek-ai/*` 解析得到（surf-notify / surf-nativeify
 ...
 *
 * @module surf-app/host/scripts/prebuild-plugins
 */
...
import { scanSwiftDir, swiftContentHash } from "../../../surf-bridge/lib/swift-payload.js";
...
const payloadPath = join(resources, "SurfPayload.json");
...
for (const { module, plugin } of payload.modules ?? []) {
	...
	const declared = mod.default?.surfSwift;
	if (declared === undefined) {
		fail(`${plugin} 的默认导出上没有 surfSwift`,
			"它应当由 surf-bridge/lib/plugin.js 的 createSwiftPlugin 挂上——"
			+ "这个插件是不是没走那个工厂？");
	}
	...
	const swiftDir = join(resources, "SurfNode", plugin, "swift");
...
if (records.size === 0) fail("SurfPayload.json 里一个 Swift module 都没有");
...
const spec = {
	version,
	modulesDir: join(resources, "SurfModules"),
	frameworksDir: join(contents, "Frameworks"),
	outputRoot: join(resources, "SurfPlugins"),
	manifestPath: join(resources, "SurfPrebuilt.json"),
...
const specPath = join(mkdtempSync(join(tmpdir(), "surf-prebuild-")), "spec.json");
