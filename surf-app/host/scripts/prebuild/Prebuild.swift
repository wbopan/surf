...
/// `prebuild-plugins.mjs` 调 `surf-bridge/lib/swift-payload.js` 算好，
/// 经 spec 递进来当 `bridgeHash`。
///
/// 用法：`surf-prebuild <spec.json>`，spec 的形状见 `Spec`。
@main
struct Prebuild {
    static func main() async {
        let args = CommandLine.arguments
        guard args.count == 2 else {
            fail("用法：surf-prebuild <spec.json>")
        }
        let spec: Spec
        do {
    ...
    private struct Spec: Decodable {
        let version: String
        /// `Contents/Resources/SurfModules`（SurfSDK 的 .swiftinterface/.swiftmodule）。
        let modulesDir: String
        /// `Contents/Frameworks`（libSurfSDK.dylib）。
        let frameworksDir: String
        /// `Contents/Resources/SurfPlugins`。
    ...
    /// `Contents/Resources/SurfPrebuilt.json`：这个 bundle 里躺着哪些预编译产物。
    ///
    ...
    /// （用户改了 surf-sidebar 的源码而 surf-layout 命中预编译），
    /// swiftc 的 `-I` 就指向这里。
    ...
    /// 合计 5457 KB → **2653 KB**，而五个都照常 `dlopen` + `dlsym surf_plugin_entry`
    /// + 调入口拿到插件对象（在一个搬到 `/tmp` 的假 bundle 里验的，纯靠
    /// `@loader_path` 解析上游）。`-x` 只动局部符号，`surf_plugin_entry` 是全局
    ...
    /// `SurfNode/<pkg>/swift/`；只按"这一轮写了什么"删的话，旧布局的残骸会
    /// 安安静静地跟着发出去。）
    ...
    /// `dylib` 相对 `Resources/SurfPlugins` 的路径（清单里写相对路径：绝对路径
...
///
/// 壳里的 `SurfPaths` 建在 `Bundle.main` 上，而这个工具的 `Bundle.main` 是它自己
