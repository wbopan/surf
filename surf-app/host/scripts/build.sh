...
APP_NAME="Surf"                  # product/bundle display name (PRODUCT_NAME)
TARGET_NAME="surf"               # target / scheme / .xcodeproj name
...
# xcodegen 二进制不入库（.gitignore `/surf-app/host/tools/`），新克隆 / 新 worktree
...
# 只等 /Applications 里的 Release 实例退出；Debug 版（Surf Dev.app，
# bundle id io.wenbo.surf.dev）名字不同，不会被误杀，可继续运行。
...
# 清掉历次改名前的旧安装（DSHarness → DeepSeek Harness → dash → Surf）
...
# 记一笔"这个 hash 已经构建过了"。**这份账是 surf-app 的**（node 半边按它决定要不要
