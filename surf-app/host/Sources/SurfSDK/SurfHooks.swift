...
///
/// 表里**一个具体的 hook 名都没有**——与 `SurfRegistry`/`SurfContributions`
...
/// **SDK 与插件协议一个字不动**。
///
/// 载荷同 `SurfEventBus`：只放 JSON 能表达的值。答复也是。
/// 线程约定：只在主线程使用。
public final class SurfHooks {
    /// 进程级单例。壳与插件都用这一份，不必接线（同 `SurfContributions.shared`）。
    public static let shared = SurfHooks()
    ...
    ///   撤销是空操作（与 `SurfRegistry.register` 同款 token 校验）。
    @discardableResult
