...
/// 贡献槽：**多占用**注册表。`SurfRegistry` 的孪生兄弟，区别只在基数。
///
/// - `SurfRegistry` 是"替换槽"：一槽一主，后来者覆盖前者。适合 root / sidebar
///   这种独占表面——两个插件同时画侧边栏是没有意义的。
/// - `SurfContributions` 是"贡献槽"：一槽 N 条，各家追加、互不影响。适合工具栏
///   按钮、状态栏指示器、菜单项这种"谁都可以来一条"的表面。
///
/// **为什么单独一个类而不是给 SurfRegistry 加个数组**：两种槽的撤销语义不一样
/// （替换槽的撤销是"如果还是我就摘掉"，贡献槽的撤销是"只摘我这一条"），
/// 消费方的观察粒度也不一样。混在一个类型里，两边的 API 都会变得要看注释才敢用。
///
/// **只放拓扑，不放流量**（同 SurfRegistry）：这里每次变动都会驱动消费方重建。
///
/// 全进程一份，住在 SDK dylib 里，因此类型身份跨插件、跨世代稳定。
/// 线程约定：**只在主线程使用**。这里不加 `@MainActor`，理由同 `SurfRegistry`
...
/// （如 `toolbar` 槽的约定写在 surf-layout 的 LayoutSplitController 里），
/// 就像槽名本身也是插件之间自行约定的一样。
@Observable
public final class SurfContributions {
    /// 进程级默认实例。
    ///
    /// SDK dylib 全进程只有一份（随 app bundle 分发，壳与所有插件都链接它），
    /// 所以这个 static 天然就是进程级单例——和 `SurfRegistry` 由壳持有再注入
    /// 是同一个效果，只是少了一条穿过壳的接线。壳日后想自己持有，
    /// 给 `SurfHost.init` 传 `contributions:` 覆盖即可。
    public static let shared = SurfContributions()

    /// 一条贡献。
    public struct Contribution {
        /// 贡献者插件名（诊断用，如 `surf-layout`）。
