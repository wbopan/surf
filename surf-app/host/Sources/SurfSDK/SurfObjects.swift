...
/// 线程约定同 `SurfRegistry`：**只在主线程使用**，不加 `@MainActor`
/// （M2 没有覆盖跨 dylib 的 actor 边界，少一个未验证的变量比多一层静态保证划算）。
public final class SurfObjects {
    ...
    /// 取并转型的糖：`host.objects.object("surf.webView", as: WKWebView.self)`。
    ...
    public enum Key {
        ...
        public static let webView = "surf.webView"
        /// 当前 dsh 的 HTTP base，`NSURL`。壳在接入/换端点时更新。
        public static let endpoint = "surf.endpoint"
        /// 会话展示面。协议由 surf-layout 定义（`SurfConversationSurface`），
        /// 消费者需 `import SurfLayout` 后转型。
        public static let conversationSurface = "surf.conversationSurface"
        /// 设置面板的主人。**取值只看在不在**，所以放一个光秃秃的 `NSObject` 就够
        /// ——正因为只看在不在，它可以是系统类型，不违反"禁止放插件自己定义的类型"。
        /// surf-settings 在场时占住它，surf-layout 见有主就不再用页内 modal 响应 ⌘,；
        /// 它不在场时页内 modal 是唯一的设置入口，逃生舱模式也得能改设置。
        public static let settingsOwner = "surf.settingsOwner"
    }
}
