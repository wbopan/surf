...
/// 壳递给插件的世界。
///
/// 是 final class 而不是协议：这样跨 dylib 只剩 `SurfPlugin` 一处协议见证表，
/// ABI 面越窄越不容易在换代时出意外。壳负责构造，每个插件拿到属于自己的一份
/// （`store` 命名空间、`bridge` 通道、`log` 前缀都是本插件专属，
/// 而 `registry`/`objects`/`events` 是全进程共享的那一份）。
public final class SurfHost {
    /// 本插件名（如 `surf-sidebar`）。
    public let plugin: String
    /// 本次装载的世代号。注册槽时原样传给 `SurfRegistry.register`。
    public let generation: Int

    /// 槽注册表（共享）。单占用："谁占了这个表面"。
    public let registry: SurfRegistry
    /// 贡献槽注册表（共享）。多占用："谁往这个表面加了一条"。
    public let contributions: SurfContributions
    /// 宿主对象保管箱（共享）。
    public let objects: SurfObjects
    /// 进程内事件总线（共享）。
    public let events: SurfEventBus
    /// 应答式钩子表（共享）。给"系统要求启动时就位的 delegate"那一类接线用。
    public let hooks: SurfHooks
    /// 本插件的持久化命名空间。
    public let store: SurfStore
    /// 本插件与自己 TS 半身的通道。
    public let bridge: SurfBridge
    ...
    public init(plugin: String,
                generation: Int,
                registry: SurfRegistry,
                contributions: SurfContributions = .shared,
                objects: SurfObjects,
                events: SurfEventBus,
                hooks: SurfHooks = .shared,
                store: SurfStore,
                bridge: SurfBridge,
    ...
    /// 写一行进壳的日志（`<AppSupport>/logs/surf.<worktree>.log`，一个 App 实例一份），
    /// 自动带插件名与世代号。
    ...
    public func handle(hook: String,
                       _ body: @escaping ([String: Any]) -> [String: Any]?) -> SurfDisposable {
        hooks.handle(hook, owner: plugin, version: generation, body)
    }

    /// 注册槽的糖：世代号与插件名自动带上。
    @discardableResult
    public func register(slot: String, make: @escaping () -> AnyView) -> SurfDisposable {
