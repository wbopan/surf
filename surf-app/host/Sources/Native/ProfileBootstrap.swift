...
/// **profile 自举**：让"双击 `/Applications/Surf.app`"这一下就足以跑起一整套
/// surf，用户机器上不需要仓库、不需要 pnpm、不需要知道 profile 是什么。
...
///    无条件重写，我们写了也是白写）。
/// 2. **镜像**：`Contents/Resources/SurfNode/` → `<profile>/.surf/`。
...
///    图标缓存那条坑同源）。
/// 3. **`package.json`**：给每个镜像里的包补一行 `link:./.surf/<pkg>`，
///    并把 `dsh.profile.bundles` 补齐成那三行。
/// 4. **符号链接**：`<profile>/node_modules/@wenbo/<pkg>` → `../../.surf/<pkg>`。
///
...
/// 路径压根不该走到**。两种形态的 profile 名也不同（`surf` vs
/// `surf-dev`，计划 §3.6），所以它们不再互相覆盖。
enum ProfileBootstrap {
    ...
    /// bundle 里那份 node 半边（`Contents/Resources/SurfNode/`，由
    /// `scripts/pack-payload.sh` 打进来）。**不在 = 这个壳不带载荷**，
    /// 自举无事可做（开发形态、或 M1 之前的旧产物）。
    static var payloadRoot: URL? {
        guard let resources = Bundle.main.resourceURL else { return nil }
        let dir = resources.appendingPathComponent("SurfNode", isDirectory: true)
    ...
    /// 打包时写下的清单（`Contents/Resources/SurfPayload.json`）。它里面那个
    ...
    private static func payloadManifest() -> (hash: String, version: String)? {
        guard let resources = Bundle.main.resourceURL else { return nil }
        let url = resources.appendingPathComponent("SurfPayload.json")
    ...
    static func run(profile: String = profileName, home: URL? = nil) throws -> Outcome? {
        guard let payload = payloadRoot else {
            log("bundle 里没有 SurfNode 载荷，跳过自举（这个壳不带 node 半边）")
            return nil
        }
    ...
    /// 镜像里的一个包。`dir` 是目录名（= 仓库里的目录名，surf-* 之间的相对
    ...
    private static func readPayloadPackages(_ payload: URL) throws -> [Package] {
        ...
        guard !out.isEmpty else {
            throw Failure(message: "bundle 里的 SurfNode 载荷是空的：\(payload.path)")
        }
        guard out.contains(where: \.isBundle) else {
            throw Failure(message: "SurfNode 载荷里没有任何包声明 dsh.bundle（伞包缺失），"
    ...
    /// 我们那几行的 link 目标指到 `.surf/` 以外去了 = 这个 profile 是别的
    /// 形态留下的（典型：改名之前主 worktree 的 `./dev` 就装在 `surf` 上）。
    ...
    private static func migration(profile: String, package: String, spec: String) -> Failure {
        ...
        删掉它（rm -rf ~/.dsh/profiles/\(profile)）后重开 App，或者跑一次 ./dev 迁移到 surf-dev。
        """)
    }
    ...
    /// 载荷没有 `SurfPayload.json` 时的兜底：自己遍历一遍算内容 hash。
    ...
    private static func updateManifest(dir: URL, packages: [Package], profile: String) throws -> Bool {
        ...
        // 清理：上一版留下、这一版不再有的包。判据是"link 目标在 .surf/ 之内"
        // ——用户自己的依赖天然不满足它，不受影响。
    ...
    private static func updateLinks(dir: URL, packages: [Package]) throws -> Int {
        ...
        for pkg in packages {
            ...
            // = `parts.count`。`<profile>/node_modules/@wenbo/x` → `../../.surf/x`。
    ...
    private static func log(_ message: String) {
        Log.write(message, to: SurfPaths.logURL, tag: "bootstrap")
    }
}
