...
/// surf-app 播报的壳构建状态（`app-build` 帧，计划 §5.4/§7.5）。
struct AppBuildState {
    ...
    let hash: String?
    let durationMs: Int?
    /// surf-app 配了 `restartOnRebuild`：不必问用户，直接重启。
...
private typealias SurfPluginEntry = @convention(c) () -> UnsafeMutableRawPointer
...
final class NativePluginHost {
    // 共享给所有插件、跨世代稳定的三件套（住在 SurfSDK dylib 里）。
    let registry = SurfRegistry()
    let objects = SurfObjects()
    let events = SurfEventBus()
    ...
    private(set) var commands: [SurfCommand] = []

    /// 命令声明有变时调用。与 `onUpdate` 分开：这条线跟 registry、跟编译都没关系。
    var onCommands: (([SurfCommand]) -> Void)?
    /// 壳自身的构建状态（surf-app v1 经桥播报，§7.5）。壳自己看，不给插件。
    ...
    init() {
        let base = SurfPaths.appSupport.appendingPathComponent("native-plugins", isDirectory: true)
        ...
        compiler = CompilerService(
            modulesDir: Bundle.main.bundleURL
                .appendingPathComponent("Contents/Resources/SurfModules", isDirectory: true),
            ...
            searchRoots: [
                ...
                .prebuilt(Bundle.main.bundleURL
                    .appendingPathComponent("Contents/Resources/SurfPlugins", isDirectory: true)),
            ],
        ...
        bridge.onConnected = { [weak self] connected in
            ...
            if connected {
                Log.write("桥已握手，拉取 snapshot", to: SurfPaths.logURL, tag: "bridge")
                self.bridge.send(["type": "snapshot"])
            } else {
                // 断桥不动 registry：UI 保持最后状态，重连后重新拉全量即可。
                Log.write("桥断开（registry 保持不动）", to: SurfPaths.logURL, tag: "bridge")
            }
    ...
    func connect(baseURL: URL, bridgePath: String) {
        objects.setObject(SurfObjects.Key.endpoint, baseURL as NSURL)
        events.emit(SurfEventBus.Topic.endpointChanged, ["httpBase": baseURL.absoluteString])
        bridge.stop()
                    ...
                    if let first = next.first(where: { $0.id == command.id }),
                       first.key != command.key || first.label != command.label {
                        Log.write("命令 \(command.id) 被 \(first.owner) 与 \(owner) 各声明一份且内容不同，"
                                  + "按 \(first.owner) 那份走", to: SurfPaths.logURL, tag: "menu")
                    }
                    continue
                }
                next.append(command)
        ...
        for source in sources {
            ...
            do {
                ...
                Log.write("插件 \(source.name) 跳过：本机没有 Swift 工具链，"
                          ...
                          to: SurfPaths.logURL, tag: "plugin")
                bridge.send(["type": "compile-result", "plugin": source.name,
    ...
    private func swap(to compiled: CompiledPlugin, source: PluginSource) throws {
        ...
        // 走全局查找会拿到先装的那个（M2 §1）。
        guard let symbol = dlsym(image, "surf_plugin_entry") else {
            throw CompileError.failed(log: "找不到 surf_plugin_entry 符号（插件没写 @_cdecl 入口？）")
        }
        let entry = unsafeBitCast(symbol, to: SurfPluginEntry.self)
        let object = Unmanaged<AnyObject>.fromOpaque(entry()).takeRetainedValue()
        guard let plugin = object as? SurfPlugin else {
            throw CompileError.failed(log: "入口返回的对象没有实现 SurfPlugin（SDK 版本不匹配？）")
        }
        ...
        let host = SurfHost(
            ...
            objects: objects,
            events: events,
            store: SurfStore(directory: storeDir, namespace: name),
            bridge: SurfBridge(send: { [weak self] action, payload in
                self?.bridge.send(["type": "invoke", "plugin": name,
                                   "action": action, "payload": payload])
            }),
            log: { message in
                Log.write("[\(name) g\(generation)] \(message)", to: SurfPaths.logURL, tag: "plugin")
            })
        ...
        if let previous {
            ...
            // SurfRegistry.register 的 token 校验）。旧 dylib 留在内存里。
