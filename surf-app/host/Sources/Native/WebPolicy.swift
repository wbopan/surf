...
/// 而且下载与外链是**逃生舱也得有**的能力——surf-layout 缺席、全出血网页兜底时，
/// 导出 ZIP 照样要能下来。
///
...
final class WebPolicy: NSObject {
    ...
    private let currentEndpoint: () -> SurfEndpoint?
    ...
    init(currentEndpoint: @escaping () -> SurfEndpoint?,
    ...
    func decide(_ webView: WKWebView, action: WKNavigationAction) -> WKNavigationActionPolicy {
        ...
        case .blocked(let reason):
            Log.write("拦下导航 \(url.absoluteString)：\(reason)", to: SurfPaths.logURL, tag: "web")
            closeAuxIfBlank(webView)
            return .cancel
        }
    }
    ...
    private func openExternally(_ url: URL) {
        Log.write("外链交给系统：\(url.absoluteString)", to: SurfPaths.logURL, tag: "web")
    ...
    func webView(_ webView: WKWebView,
        ...
        if let url = navigationAction.request.url {
            ...
            case .external(let target):
                ...
                return nil
            case .blocked(let reason):
                Log.write("拦下新窗口 \(url.absoluteString)：\(reason)", to: SurfPaths.logURL, tag: "web")
                return nil
            case .inPlace:
                break
            }
...
extension WebPolicy: WKDownloadDelegate {
    func download(_ download: WKDownload,
        ...
        Log.write("开始下载 → \(url.path)", to: SurfPaths.logURL, tag: "download")
        completionHandler(url)
    }

    func downloadDidFinish(_ download: WKDownload) {
        guard let url = destinations.removeValue(forKey: ObjectIdentifier(download)) else { return }
        Log.write("下载完成 \(url.path)", to: SurfPaths.logURL, tag: "download")
    ...
    func download(_ download: WKDownload, didFailWithError error: Error, resumeData: Data?) {
        ...
        Log.write("下载失败 \(name)：\(error.localizedDescription)", to: SurfPaths.logURL, tag: "download")
...
/// "看一眼就关掉"的东西（一张图、一份 raw 文本），不该长得像第二个 surf。
@MainActor
