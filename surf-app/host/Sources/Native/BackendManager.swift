...
/// 托管后端：让壳自己保证"有一个后端在跑"（`docs/archive/surf-connection-plan.md` §5）。
...
///    `dsh --profile surf --port 0 --no-open`。两者都经 login shell
...
///    而且两者的 profile 名不同（`surf-dev` vs `surf`，计划 §3.6）。
/// 2. **查重**：spawn 之前先确认没有别人在管这个 profile（健康的 isOwn 端点 /
///    `io.wenbo.surf.dsh` 那个 LaunchAgent）。同一个 profile 的两个 dsh 会
...
final class BackendManager {
    ...
    static var logURL: URL {
        let name = SurfPaths.instanceTag.map { "managed-dsh.\($0).log" } ?? "managed-dsh.log"
        return SurfPaths.logsDir.appendingPathComponent(name)
    }
    ...
    func stop() {
        ...
        if let process {
            ...
            Log.write("停止托管后端（SIGTERM 进程组 \(process.pid)）",
                      to: SurfPaths.logURL, tag: "backend")
            process.terminate()
    ...
    private func launch(attempt: Int) {
        ...
        Task { @MainActor [weak self] in
            ...
            if let external {
                ...
                Log.write("不 spawn：\(external.detail)", to: SurfPaths.logURL, tag: "backend")
            ...
            case .failure(let why):
                Log.write("不 spawn：\(why.detail)", to: SurfPaths.logURL, tag: "backend")
    ...
    private func spawn(_ plan: SpawnPlan) {
        ...
        do {
            ...
            sink.close()
            Log.write("托管后端拉起失败：\(reason)", to: SurfPaths.logURL, tag: "backend")
    ...
    private func processDidExit(_ exit: ManagedProcess.Exit, generation: Int) {
        ...
        Log.write("托管后端意外退出：\(exit.summary)", to: SurfPaths.logURL, tag: "backend")
        let now = Date()
        ...
        Log.write("第 \(attempt) 次重启后端，\(Int(delay)) 秒后", to: SurfPaths.logURL, tag: "backend")
    ...
    private func finishTermination(timedOut: Bool) {
        ...
        if timedOut, let process, process.isAlive {
            ...
            Log.write("后端已退出，壳继续退出流程", to: SurfPaths.logURL, tag: "backend")
        }
    ...
    private func setState(_ next: State) {
        ...
        state = next
        Log.write("托管后端状态：\(diagnosticSummary)", to: SurfPaths.logURL, tag: "backend")
        onStateChange?(next)
    }
    ...
    /// 推不出 worktree（产物被搬走了）。分发计划 §3.6 把无后缀的 `surf`
    /// 收窄成安装形态专属，Dev 壳无论如何都不该去动它。
    private static let devProfile = "surf-dev"
    ...
    private static func resolvePlan() async -> Swift.Result<SpawnPlan, Unavailable> {
        ...
        // **不传任何形态环境变量**（2026-08-30 M4 删掉了 `SURF_RELEASE`）：
        // 形态由 surf-app 自己看"壳源码在不在包里"判定，而自举出来的镜像
        // （`<profile>/.surf/surf-app/`）只有 `lib/` + `package.json`——
        // 没有 `host/` 就构建不了，也就不会去构建 Debug 产物、更不会拉起一个
        // Surf Dev。判据在包的内容里，不在这条命令行上。
    ...
    /// `<worktree>/surf-app/host` → `<worktree>`。Release 安装包为 nil。
    private static func worktreeRoot() -> String? {
        guard let hostDir = SurfPaths.ownHostDir else { return nil }
        let url = URL(fileURLWithPath: hostDir).deletingLastPathComponent()  // surf-app
    ...
    nonisolated static let daemonLabel = "io.wenbo.surf.dsh"
    ...
    /// `--surf-backend-command '<shell 命令>'`：拿一个无害的假进程替掉真 spawn，
    ...
    private static let commandOverride: String? =
        isDevShell ? argument("--surf-backend-command") : nil

    /// `--surf-backend-skip-dedup`：跳过查重。**只给实测用**——开发机上多半
    /// 正跑着真后端，不跳过就永远走不到 spawn 那一步。
    private static let skipDedup = isDevShell
        && ProcessInfo.processInfo.arguments.contains("--surf-backend-skip-dedup")
...
private final class LogSink {
    private let queue = DispatchQueue(label: "io.wenbo.surf.backend-log")
