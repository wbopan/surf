...
struct PluginSource {
    /// 插件名，如 `surf-sidebar`。
    let name: String
    /// 基础 module 名，如 `SurfSidebar`（桥算，壳只用）。
    let module: String
    /// 相对路径 → utf8 源码。
    ...
    /// **不含 `SurfSDK`**——那是无条件的 ABI，见 `CompilerService`。
...
struct CompiledPlugin {
    let name: String
    /// 带世代后缀的实际 module 名，如 `SurfSidebar_h9f31c0aa12b4`。
    ...
    enum Origin: String {
        ...
        case cache = "用户缓存"
        /// App bundle 里随分发走的预编译产物（`Resources/SurfPlugins/`）。
...
actor CompilerService {
    ...
    enum ProductRoot {
        ...
        case cache(URL)
        /// bundle 内预编译：`<App>.app/Contents/Resources/SurfPlugins/<Module>/prebuilt/<hash12>/`。
        case prebuilt(URL)
        ...
        /// bundle 里不存——那份源码已经在 `Resources/SurfNode/<pkg>/swift/` 了，
    ...
    ///
    /// 换 Xcode 或重编 SurfSDK 之后必须全量重编，否则 `.swiftmodule` 会对不上。
    /// 但重编某个共享 module 只该波及 import 它的插件——所以共享 module 的摘要
    /// 按声明逐个折进来，而不是把 `SurfModules/` 里的东西一股脑算成一个数。
    ...
    func compile(_ source: PluginSource,
        ...
        // 写进 bundle 的预编译产物不留——那份源码已经在 `Resources/SurfNode/` 里了。
        let srcDir = writeRoot.keepsSources
            ? dir.appendingPathComponent("src", isDirectory: true)
            : URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
                .appendingPathComponent("surf-prebuild-\(module)", isDirectory: true)
        ...
        // SurfSDK 无条件（它是 ABI 本身）；其余按插件声明加，没声明的不链接
        ...
        let started = Date()
        Log.write("现场编译 \(source.name) → \(module)", to: SurfPaths.logURL, tag: "compile")
        ...
        Log.write(String(format: "编译 %@ 完成 %.2fs → %@", source.name,
                  ...
                  to: SurfPaths.logURL, tag: "compile")
        return CompiledPlugin(name: source.name, module: module, directory: dir,
    ...
    /// （`NativePluginHost.reconcile`），而 dyld 在解析 `@rpath/libSurfLayout_h….dylib`
    ...
    /// `SurfPlugins/<Module>/prebuilt/<hash12>/`，都是"兄弟目录"），而绝对路径在换一台
    ...
    /// `SurfLayout_h9f31c0aa12b4` → `SurfLayout`（-module-alias 的左边）。
    ...
    /// 目标三元组直接从 SurfSDK 的 `.swiftinterface` 头里抄——那是共享 module
    ...
    /// **所有**插件共享的编译基线：ABI 版本 + SurfSDK 的接口。
    ///
    /// 只有 SurfSDK 在这里。它是壳↔插件的 ABI 本身，每个插件都链接它，变了谁都得重编。
    /// 其余共享 module 按插件声明单独折进 `contentHash`
    /// ——把整个 `SurfModules/` 一股脑算进来，就等于让改一行共享 module 把从不
    /// import 它的插件也全量重编一遍。
    ///
    ...
    private func toolchainFingerprint() -> String {
        ...
        var hasher = SHA256Hasher()
        hasher.update(String(surfABIVersionForFingerprint))
...
/// （直接引用 `surfABIVersion` 会把 SurfSDK 拖进本文件的 import，没必要。）
private let surfABIVersionForFingerprint = 1
