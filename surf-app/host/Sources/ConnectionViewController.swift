...
/// 没连上后端时铺满窗口的那一层：**两页**（`docs/archive/surf-connection-plan.md` §3）。
///
...
struct ConnectionActions {
    ...
    var connect: (SurfEndpoint, Bool) -> Void = { _, _ in }
...
private struct OnboardingPage: View {
    ...
    /// 已判定"不含 Surf 组件"的也滤掉——它 HTTP 上是绿的，挂着「连接」
    ...
    private func row(for endpoint: SurfEndpoint) -> some View {
        HStack(spacing: 10) {
            Circle()
...
private enum ConnFormat {
    ...
    static func portLabel(_ endpoint: SurfEndpoint, strings: L) -> String {
    ...
    static func startedAt(_ endpoint: SurfEndpoint, strings: L) -> String? {
