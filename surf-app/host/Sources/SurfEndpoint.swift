...
/// 这里只负责找到它。
struct SurfEndpoint: Equatable, Sendable {
    /// 这个答案是从哪来的（诊断用；flag 优先于发现文件）。
    enum Source: String, Sendable {
        case flag       // 命令行 --surf-endpoint（surf-app 插件拉起时传入）
        case discovery  // endpoint 发现文件（用户手动双击 App 时走这条）
        case manual     // 用户在连接页里敲的地址（一次性目标，不落偏好）
        case fixed      // 连接偏好 `surf.connection.mode = fixed` 钉死的那一个
    }
    ...
    let profile: String?
    /// 写这份发现文件的那个 dsh 的 `surf-app/host` 目录（老版本插件没有这个字段）。
    let hostDir: String?
    /// 这个 dsh 期望的 App bundle 路径（老版本插件没有这个字段）。
    ///
    /// dev 形态（surf-app 带着 `host/` 源码）是本 worktree 的构建产物；
    /// 正式形态（随 App 分发的镜像里没有 `host/`）是 `/Applications/Surf.app`。
    let appPath: String?
    let source: Source
    ...
    ///  2. `hostDir` == `SurfPaths.ownHostDir`。老版本 surf-app 写的发现文件里
    ...
    var isOwn: Bool {
        ...
        guard let own = SurfPaths.ownHostDir, let hostDir else { return false }
...
enum EndpointLocator {
    static let defaultBridgePath = "/surf/bridge"
    ...
    static func candidates() -> [SurfEndpoint] {
        var out: [SurfEndpoint] = []
    ...
    /// `--surf-endpoint <url>`（也吃 `--surf-endpoint=<url>`）；
    /// 可选 `--surf-bridge-path <path>`。
    static func flagEndpoint() -> SurfEndpoint? {
        let args = ProcessInfo.processInfo.arguments
        guard let raw = value(of: "--surf-endpoint", in: args),
              let url = URL(string: raw), url.scheme != nil, url.host != nil else { return nil }
        return SurfEndpoint(httpBase: url,
                            bridgePath: value(of: "--surf-bridge-path", in: args) ?? defaultBridgePath,
    ...
    /// endpoint 发现文件，一个 profile 一份。surf-app 插件在 dsh 启动时写、
    ...
    static func discoveredEndpoints() -> [SurfEndpoint] {
        let files = (try? FileManager.default.contentsOfDirectory(
            at: SurfPaths.endpointsDir, includingPropertiesForKeys: nil)) ?? []
        return files
    ...
    static func probe(_ endpoint: SurfEndpoint, timeout: TimeInterval = 1.5) async -> ConnectFailure? {
    ...
    /// 整轮的最坏耗时就是单个超时。
    static func probeAll(_ list: [SurfEndpoint], timeout: TimeInterval = 1.5) async -> [CandidateStatus] {
    ...
    static func manualEndpoint(_ url: URL, source: SurfEndpoint.Source) -> SurfEndpoint {
        SurfEndpoint(httpBase: url, bridgePath: defaultBridgePath,
                     pid: nil, startedAt: nil, profile: nil, hostDir: nil, appPath: nil,
                     source: source)
    }
}
