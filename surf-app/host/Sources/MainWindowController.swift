...
/// 布局、分栏、侧边栏装配全部搬进 surf-layout 插件，这里负责
...
final class MainWindowController: NSWindowController, WKNavigationDelegate, NSWindowDelegate {
    ...
    /// 窗口最小宽度。sidebar 的自适应折叠归 surf-layout 管，
    /// 这里只兜住"折叠之后窗口还能多窄"。
    ...
    private static let windowAutosaveName = "SurfMainWindow.v1"
    ...
    // 网页侧边栏够不着标题栏区域（旧的 surf-nativeify topInset 让位已随之删除）。
    ...
    private var commands: [SurfCommand] = []

    /// 设置里那份键位覆盖的原文（`surf-shortcuts` 经页面投影过来）。
    /// **要存原文而不是解析结果**：命令表晚于它到达时得能拿它重算一次。
    private var keymapValues: [String: String] = [:]

    /// 键位订阅句柄。**必须有人接住**：`SurfDisposable` 在 deinit 里就把订阅撤了，
    /// 不存下来等于订完当场退订——不报错、不打日志，只是键位设置永远不生效。
    private var keymapSubscription: SurfDisposable?

    /// 页面查询参数的订阅句柄。同上，不接住就等于没订。
    private var webQuerySubscription: SurfDisposable?
    private var relaunchSubscription: SurfDisposable?

    /// 壳有新版时右上角那条浮动提示（surf-app v1 播报，§7.5）。
    ...
    /// 类型是 `SurfWebView` 而不是 `WKWebView`：菜单那边要给它装影子键位表
    /// （`commandKeyEquivalents`），插件从保管箱里借用时照样 `as? WKWebView` 命中。
    private lazy var webView: SurfWebView = {
        let config = WKWebViewConfiguration()
        // UA 追加 "Surf/<version>"（带斜杠，防 "surf" 作为普通子串误命中）：
        // surf-nativeify / surf-layout 的 client 半边以此判断页面运行在壳内（终端 dsh web /
        // 普通浏览器共用同一 profile，不受影响）。
        let shortVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "0"
        config.applicationNameForUserAgent = "Surf/\(shortVersion)"
        // 网页 → 原生通道：插件 v2 桥（ready / currentSession 上报）
        config.userContentController.add(WKScriptMessageHandlerProxy(self), name: "surf")
        applyAppearancePreferences(config.preferences)
        // 用子类而不是 WKWebView：它覆写 willOpenMenu，把右键菜单里 Reload /
        // 前进后退 / 在新窗口打开这类浏览器味的导航项裁掉（Native/SurfWebView.swift）。
        let wv = SurfWebView(frame: .zero, configuration: config)
    ...
    ///   (-apple-visual-effect: …)` 探测为假，surf-nativeify 的手绘玻璃栈照常生效。
    ...
    private static let pageZoomDefaultsKey = "surf.pageZoom"
    ...
    init(backend: BackendManager) {
        ...
        // surf-layout 的 client 半边投影过来（`surf.page.keymap`）。两者各自到齐时
        // 都会重建整条菜单，所以这里不必等谁。
        setupMenus()
        observeMenuTracking()
        observeKeymap()
        ...
        nativeHost.objects.setObject(SurfObjects.Key.webView, webView)
    ...
    /// 状态机选中了一个端点：装页面 + 连桥。
    private func attach(_ endpoint: SurfEndpoint) {
        loadWebUI(endpoint)
    ...
    private func loadWebUI(_ endpoint: SurfEndpoint) {
        guard var components = URLComponents(url: endpoint.httpBase, resolvingAgainstBaseURL: false)
        else { return }
        components.path = "/"
        // 查询参数由插件说了算（`surf.web.query`，见 observeWebQuery）。壳不认得
    ...
    private static let webQueryTopic = "surf.web.query"

    private static let webQueryDefaultsKey = "surf.webQuery"
    ...
    // MARK: - 壳自我重启（`surf.app.relaunch`）

    /// 谁都可以喊一嗓子"把壳重启一下"。**眼下唯一的喊话方是 surf-settings 的
    /// 「连接」栏**：那一页只写 UserDefaults、不当场切后端，改完得重启才生效。
    ///
    /// **不复用 `app-restart` 那条桥路径**：那条是"surf-app 编出了新产物、
    ...
    static let relaunchTopic = "surf.app.relaunch"
    ...
    private func relaunchSelf() {
        ...
        do {
            ...
            Log.write("按请求重启壳：助手已就位（pid \(pid) 退出后 open）",
                      to: SurfPaths.logURL, tag: "app")
        } catch {
            relaunchScheduled = false
            Log.write("重启壳失败，助手起不来：\(error)", to: SurfPaths.logURL, tag: "app")
            return
        }
    ...
    private func applyWebQuery(_ next: [String: String]) {
        ...
        // 而不是"最终"的话——surf-layout 先于 surf-sidebar 上线，那一刻 sidebar 槽
    ...
    private func syncWebQueryGate() {
        ...
        Log.write("页面查询参数变化：\(describe(queryInUse)) → \(describe(webQuery))，重载页面",
                  to: SurfPaths.logURL, tag: "layout")
        loadWebUI(endpoint)
    }
    ...
    // MARK: - 壳自身的构建（surf-app v1）

    /// 壳重建不是插件热替换那个档位：它要重启进程、丢页面状态。所以默认只提示，
    /// 动手归用户（surf-app 配了 `restartOnRebuild` 才自动走）。
    private func applyAppBuild(_ state: AppBuildState) {
        ...
        case "ready":
            guard !state.autoRestart else {
                Log.write("壳有新版且配置了自动重启，立即重启", to: SurfPaths.logURL, tag: "app-build")
    ...
    /// `surf-bridge/lib/plugin.js` 的 CommandDeclaration），经桥 snapshot 到
    ...
    private func setupMenus() {
        ...
        // （壳自己已无偏好可设，设置窗口归 surf-settings / 页内 modal 归 surf-layout）。
        ...
        // 插件自造的顶级菜单（如 surf-sidebar 的「会话」）。全部夹在这儿：
    ...
    /// 交给 `SurfWebView` 在 `performKeyEquivalent` 里抢先匹配。
    /// 为什么非要抢（以及为什么只抢这些），见 `Native/SurfWebView.swift` 那条注释。
    ///
    ...
    private func contributions(in menuId: String) -> [SurfCommand] {
    ...
    // MARK: - 键位表（设置项 `surf-shortcuts`）

    /// 订上设置里那份键位表。链路：dsh settings → surf-layout 的 client 半边订值
    /// → `postToShell({type:"keymap", values})` → 页内桥广播 `surf.page.keymap`。
    ///
    ...
    private func observeKeymap() {
        keymapSubscription = nativeHost.events
            .subscribe(SurfEventBus.Topic.pagePrefix + "keymap") { [weak self] payload in
    ...
    /// 键位也跟着重算——默认键位就住在声明里。
    private func applyCommands(_ next: [SurfCommand]) {
        commands = next
        resolveKeymap()
        rebuildMenus()
    ...
    private func resolveKeymap() {
        ...
        Log.write(line, to: SurfPaths.logURL, tag: "menu")
    ...
    private static let localeDefaultsKey = "surfLocale"

    /// 此刻在役的语言。真相来自页面（`surf.page.locale`）；
    /// 页面还没说话时是 `startupLocale` 那一级的猜测。
    private(set) var activeLocale: SurfLocale = .en
    ...
    /// 同样来自系统语言，而 `SurfLocale.resolve` 复刻的就是 dsh 的推导规则。
    private static var startupLocale: SurfLocale {
        if let raw = UserDefaults.standard.string(forKey: localeDefaultsKey),
           let cached = SurfLocale(rawValue: raw) {
            return cached
        }
        return SurfLocale.resolve(preferred: Locale.preferredLanguages)
    }

    /// 广播当前语言。**粘性**：插件装载晚于壳启动，不粘的话它要等到用户下一次
    /// 切语言才知道现在是哪一种，而那个状态很可能一直不变
    /// （见 `SurfEventBus.emitSticky`）。
    private func publishLocale(_ next: SurfLocale) {
        activeLocale = next
        nativeHost.events.emitSticky(SurfEventBus.Topic.locale, ["locale": next.rawValue])
    }

    /// 页面投影来的语言（`surf.page.locale`，surf-layout 的 client 半边订
    ...
    private func applyProjectedLocale(_ raw: String) {
        guard let next = SurfLocale(rawValue: raw) else {
            Log.write("页面投影的语言 \"\(raw)\" 不在值域内，忽略", to: SurfPaths.logURL, tag: "locale")
            return
        }
        ...
        Log.write("界面语言切到 \(next.rawValue)（来自 dsh 设置）", to: SurfPaths.logURL, tag: "locale")
    }
    ...
    /// 才取文案的，天然就是新的；插件那半边自己订 `surf.locale` 粘性主题
    /// （`SurfLocaleStore`），也不归这里管。
    ...
    private func emitMenuCommand(_ command: String, _ extra: [String: Any] = [:]) {
        ...
        nativeHost.events.emit(SurfEventBus.Topic.menuCommand, payload)
    }
    ...
    private func diagnosticsText() -> String {
        ...
        if let endpoint = connection.activeEndpoint {
            // "不是本 worktree 那一套"是警告，得跟着端点这一行走（见 SurfEndpoint.summary）。
        ...
        // 正是最容易搞错的一步。
        lines.append(s.diagLogPath(SurfPaths.logURL.path))
        lines.append(s.diagEndpointsPath(SurfPaths.endpointsDir.path))
        // 一个 profile 一份，所以这一行同时回答了"这台机器上现在有几套 surf 在跑"。
    ...
    func handleBridgeMessage(_ message: WKScriptMessage) {
        ...
        case "surf":
            ...
            case "ready":
                ...
                Log.write("页内桥就绪：\(caps.joined(separator: ", "))\(diag)", to: SurfPaths.logURL, tag: "bridge")
                nativeHost.events.emit(SurfEventBus.Topic.pageReady, ["capabilities": caps])
            case "currentSession":
                if let id = body["id"] as? String {
                    // **粘性**：插件装载晚于页面，不粘的话它拿不到"现在在哪个会话"
                    // ——而这恰恰是它最需要的那个输入（见 SurfEventBus.emitSticky）。
                    nativeHost.events.emitSticky(SurfEventBus.Topic.pageCurrentSession,
                                                 ["id": id])
                }
            case "locale":
            ...
            default:
                ...
                nativeHost.events.emit(SurfEventBus.Topic.pagePrefix + type, body)
            }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        Log.write("WebView 加载完成：\(webView.url?.absoluteString ?? "?")", to: SurfPaths.logURL, tag: "web")
    ...
    func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        Log.write("WebView 导航失败：\(error.localizedDescription)", to: SurfPaths.logURL, tag: "web")
    }

    func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
        Log.write("WebView 加载失败：\(error.localizedDescription)", to: SurfPaths.logURL, tag: "web")
...
///
/// **形状的权威文档在 `surf-bridge/lib/plugin.js` 的 `CommandDeclaration`**
/// ——声明的一方是插件作者，壳只是读者，所以文档跟着声明走。这里只解析壳用得上的
/// 那几个键：`description` / `keyChoices` 是给 surf-app 拼设置 schema 的，壳不看。
///
/// 缺字段一律有兜底、不整条丢掉（`id` 例外——没有 id 的声明喊不出任何命令）：
/// 一条声明写漏了 `order` 不该让整个菜单消失。
struct SurfCommand: Equatable {
    ...
    func title(_ locale: SurfLocale, index: Int? = nil) -> String {
        let text = label[locale.rawValue] ?? label[SurfLocale.en.rawValue] ?? id
        guard let index else { return text }
        return text.replacingOccurrences(of: "{n}", with: "\(index)")
    }

    /// 自定义顶级菜单的标题；没声明就是 nil（由调用方决定怎么兜）。
    func menuTitle(_ locale: SurfLocale) -> String? {
        menuLabel[locale.rawValue] ?? menuLabel[SurfLocale.en.rawValue]
    }
}

/// 装在菜单上的那份键位表。
///
/// **默认值不住在这里**：它在插件的命令声明里（`SurfCommand.key`），
/// 与 ns `surf-shortcuts` 的 schema 是同一个上游（surf-app 现拼那张 schema）。
/// 从前壳与 surf-app 各存一份默认表、要求逐字一致而没有任何校验——
