/**
 * surf-app —— 壳源码与构建过程的插件化（阶段二计划 §7.5 v0）。
 *
 ...
 *   3. 产物存在且 app 尚未运行 → `open --args --surf-endpoint …` 拉起。
 *
 ...
 * `surf-app/host-build/`（`docs/archive/distribution-plan.md` §3.3），而本包的 `files`
 * 白名单只收 `lib/`、App 的 `SurfNode/` 载荷也只拷 `lib/` 与 `swift/`。
 * 判据因此**不是"探一下 host/ 在不在"，而是"那个模块 import 得到吗"**
 * （见下面那个模块级常量 `hostBuild`）——更诚实，也省掉一次文件系统探路。
 * 拿不到就是这份 surf-app 没有构建能力：`build` / `watch` / `restartOnRebuild`
 * 一律关掉，只剩第 1 和第 3 件事。
 *
 * 曾经的 `SURF_RELEASE` 旋钮 2026-08-30 随 M4 一起删了
...
import { environmentLocale, localeFromTag } from "../../surf-bridge/lib/locale.js";
import { delay, errorText, readTextOrUndefined, resolveProfileName, run } from "./util.js";

export const name = "surf-app";
 ...
 * `surf-shortcuts` ns 在 `apply` 里注册，够得着 ctx，所以它走完整的决议链
...
export const Config = z.object({
	configuration: z.union([z.const("Debug"), z.const("Release")]).default("Debug")
		.description(CONFIG_LOCALE === "zh"
			? "xcodebuild 配置。Debug 产物是 “Surf Dev.app”，Release 是 “Surf.app”。"
			: "The xcodebuild configuration. Debug produces “Surf Dev.app”, Release produces “Surf.app”."),
...
/** 壳的 Application Support 根目录，与 Swift 侧 `SurfPaths.appSupport` 必须一致。 */
const APP_SUPPORT = join(homedir(), "Library", "Application Support", "io.wenbo.surf");

/**
 * endpoint 发现文件的目录；Swift 侧 `SurfPaths.endpointsDir`。
 *
...
/**
 * 桥路径的兜底值。**真相在 surf-bridge 的 config.path**——它才是挂 WS 的那一方，
 * 而且那是个用户可覆写的配置项。本插件经 `surfBridge.path` 取当前值（见 `apply`），
 * 只在桥缺席时用这个默认；写死一份自己的会让"改了桥的 path 壳就静默连不上"。
 * 与 Swift 侧 `SurfEndpoint.defaultBridgePath` 是同一个默认。
 */
const DEFAULT_BRIDGE_PATH = "/surf/bridge";
...
const INSTALLED_APP = "/Applications/Surf.app";

/**
 * 构建那半边（`surf-app/host-build/`）——**`undefined` 就是这份 surf-app
 * 没有构建能力**。
 *
...
async function importHostBuild() {
	try {
		...
		process.stderr.write(`surf-app: 加载 surf-app/host-build 失败，`
 ...
 * `dsh --profile surf` 时，把装好的 App 带起来正是他要的。
...
function applyNoBuilderForm(config, logger) {
	...
	logger.info("这份 surf-app 不带构建能力（没有 host-build/，随包分发的那一半只有 lib/）"
 ...
 * 壳凭它认出"哪一份是我这一套"**（见 Swift 侧 `SurfEndpoint.isOwn`）。
 *
 * 为什么 `hostDir` 不够：装到 `/Applications` 的 Release 壳不在任何 worktree 的
 * `build/Build/Products/` 之下，`SurfPaths.ownHostDir` 推不出来，`isOwn` 于是恒假
...
/**
 * 壳快捷键的设置命名空间（计划：docs/archive/surf-shortcuts-settings-plan.md）。
 *
 * 名字里没有 `surf-app`，因为它描述的是**壳的菜单**而不是本插件的构建行为；
 * 登记权在这儿只是因为 surf-app 就是壳的 node 半身。ns 名进了 wire——
 * client 半边 `surf-layout` 用同一个字符串 bind scope，改名要两边一起改。
 */
const SHORTCUTS_NAMESPACE = "surf-shortcuts";

/**
 * 键位表 schema：**按桥的登记表现拼**（`surfBridge.commands.list()`）。
 *
 * 从前这里是一张手抄的常量表，而壳里另有一张默认键位表——两处必须逐字一致
 * 且**没有任何校验**，分家了不报错，症状是"设置界面写着 ⌘N、按下去却不是"。
 * 现在两张表的上游都是插件自己的 `commands` 声明（形状见
 * `surf-bridge/lib/plugin.js` 的 CommandDeclaration），漂移在结构上就不可能了。
 *
 ...
 * 登记表安静多久之后才注册 schema。
 *
 * **必须去抖**：surf-app 多半比别的插件先挂载，那一刻登记表是空的——照它注册出来的
 * 就是一张空表。插件是一个接一个挂上来的，等它不再动了再注册一次最省事
 * （照 surf-sidebar 的去抖模式）。真的再变（运行时装/卸插件）也接得住，
 * 见 `installShortcutsSettings` 的重注册那一段。
 */
const SHORTCUTS_QUIET_MS = 300;

/**
 * 注册 / 重注册 `surf-shortcuts` 设置 ns。
 *
...
function installShortcutsSettings(scoped, logger) {
	...
	const install = () => {
		if (disposed) return;
		const commands = dedupeCommands(scoped.surfBridge.commands.list());
		...
		queue = queue.then(async () => {
			...
			if (configurable.length === 0) {
				logger.info("没有插件声明可配置的快捷键，surf-shortcuts 设置面不注册。");
				return;
			}
			fiber = scoped.inject(["settings"], (inner) => {
				// 注册一个 ns 就同时点亮了两扇界面，两边都不用改一行：surf-settings
			...
			// 注册失败绝不能赔掉整个 surf-app（它的正事是构建并拉起壳）。
	...
	const off = scoped.surfBridge.commands.subscribe(schedule);
	schedule();
...
export function apply(ctx, rawConfig) {
	const logger = reporter(ctx.logger("surf-app"));
	...
	}, "surf-app endpoint 发现文件");

	// 与桥的全部往来收在这一处：路径、播报、重启请求。
	// 仍是局部 inject 而非顶层依赖——桥缺席时壳照样该起来（WebView 全出血兜底），
	// 只是没有任何原生插件。
	ctx.inject(["surfBridge"], (scoped) => {
		const app = scoped.surfBridge.app;
		scoped.effect(() => {
			const path = typeof scoped.surfBridge.path === "string"
				? scoped.surfBridge.path : DEFAULT_BRIDGE_PATH;
			if (path !== bridge.path) {
				bridge.path = path;
				logger.info(`桥路径取自 surf-bridge 的配置：${path}`);
		...
		}, "surf-app ↔ surf-bridge");
	});

	// 快捷键设置面。schema 由**插件自己的命令声明**拼出来（见 shortcutsSettings），
	// 所以要同时有桥（登记表）和 settings 才做得成。值由 surf-layout 的 client 半边经
	...
	// 嵌套是它表达可选依赖的唯一方式。）
	ctx.inject(["surfBridge", "settings"], (scoped) => {
 ...
 * （`surf-bridge/lib/locale.js`），接线这一半各家自己写：这里只要一个瞬时值，
 * surf-notify 要的是一个活的读取口。
 *
...
function reporter(logger) {
	const emit = (level, message) => {
		...
		process.stderr.write(`surf-app: ${message}\n`);
	};
	return {
...
async function bootstrap({ ctx, config, logger, httpBase, bridge, isDisposed }) {
	...
	if (built.appPath === undefined) {
		logger.warn(`未找到可用的 ${configuration} 产物，${name} 优雅缺席——dsh 照常服务浏览器。`
			+ ` 需要 macOS 壳的话，在仓库里手动跑一次 surf-app/host/scripts/dev.sh。`);
		return;
	}
...
async function launch({ appPath, httpBase, bridgePath, logger }) {
	if (await isRunning(appPath)) {
		logger.info(`surf 已在运行（${appPath}），跳过拉起；它会自己从发现文件接入。`);
		return;
	}
	try {
		await run("open", [appPath, "--args",
			"--surf-endpoint", httpBase,
			"--surf-bridge-path", bridgePath], NEUTRAL_CWD);
		logger.info(`已拉起 surf：${appPath} → ${httpBase}`);
	} catch (error) {
		logger.error(`拉起 surf 失败（不重试）：${errorText(error)}`);
	}
}
...
/**
 * 本进程那一份发现文件的路径。文件名用 profile 名，因为它正是"这套 surf 是
