/**
 * surf-app 的**构建那半边**：算源码 hash、xcodegen + xcodebuild、盯源码后台重建。
 *
 * **这个目录整个不随包分发**（`docs/archive/distribution-plan.md` §3.3）——
 * `surf-app/package.json` 的 `files` 白名单只收 `lib/`，App 的 `SurfNode/` 载荷
 ...
 *
 * **`app-build` 那条播报通道不在这儿**：它长在 surf-bridge 上，本模块只是**触发源**。
 * 触发源缺席时通道空转，正是 §8 M6（Sparkle 自动更新）要复用的形状。
 *
 * @module surf-app/host-build
 */
...
/**
 * xcodegen 二进制。**被 .gitignore 挡在库外**（`/surf-app/host/tools/`），
 * 所以新克隆 / 新 worktree 里它不存在——`surf/bin/surf.js` 的 ensureXcodegen
...
/** 壳的 Application Support 根目录，与 `lib/index.js` 的同名常量、Swift 侧 `SurfPaths.appSupport` 必须一致。 */
...
export const productPath = (configuration) =>
	join(HOST_DIR, "build", "Build", "Products", configuration,
		`${configuration === "Debug" ? "Surf Dev" : "Surf"}.app`);
 ...
 *
 * **只有带构建能力的那一份 surf-app 到得了这里**（`lib/index.js` 拿不到本模块就把
 * `config.build` 关掉了）
...
async function runBuild({ configuration, logger, isDisposed }) {
	...
	try {
		...
		const result = await run("xcodebuild", [
			"-project", join(HOST_DIR, "surf.xcodeproj"),
			"-scheme", "surf",
...
/**
 * 构建日志的路径，**按 profile 分片**（`surf-app-build.<profile>.<配置>.log`）。
 *
...
function buildLogPath(configuration) {
	...
	return join(APP_SUPPORT, "logs", `surf-app-build.${shard}.${configuration}.log`);
}
 ...
 *
 * 播报走 `apply` 里那一处 `ctx.inject` 攒下的 `bridge.announce`：surf-bridge 在
...
export function watchSources({ ctx, config, logger, bridge, isDisposed, configuration, failedHash }) {
	...
	const tick = async () => {
		...
		if (result.ok) {
			...
			logger.info(`壳已重建（${seconds}s）。`
				+ `${config.restartOnRebuild ? "按配置立即重启壳。" : "重启 surf 生效——窗口里有提示。"}`);
	...
	timer.unref?.();
	ctx.effect(() => () => clearInterval(timer), "surf-app 壳源码轮询");
 ...
 *
 * **没有第三跳**（2026-08-30 M4 删掉的那条 `/Applications/Surf.app` 兜底）。
 ...
 *    退到装好的那个正式壳等于让 `surf-dev` 的后端去拉起属于 profile
 *    `surf` 的 App——两套 profile 的内容本来就不同（一个 link 仓库源码、
