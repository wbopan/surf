...
1. **Swift 载荷登记表** —— `ctx.provide('dashBridge')`，各插件经
   `createSwiftPlugin`（子出口 `dash-bridge/plugin`）把自己的 `swift/` 目录登记进来。
2. **一条 WebSocket** —— `/dash/bridge`，与 dsh 同端口（`ctx.webServer.registerUpgrade`）。
...
同一个 hash 还决定 Swift module 名（`DashSidebar_h9f31c0aa12b4`），
...
export default createSwiftPlugin({
  ...
  swiftDeps: ["dash-layout"],       // Swift module 依赖（源码里 import DashLayout）
  subscribe: ({ ctx, push }) => { /* 订宿主事件 → push 给 Swift 半身 */ },
  expose: { archive: (payload, { ctx }) => { /* Swift 半身可触发的动作 */ } },
});
```

`dash-*` 之间用**相对路径 import**：`healProfilesModuleFallback` 只镜像 harness 的依赖闭包，
