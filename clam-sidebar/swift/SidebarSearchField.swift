...
///
/// ## 外框高度的正解：`.controlSize` 必须设在 **SwiftUI 环境**里
///
/// **眼下用 `.extraLarge`（36pt）**。重设计一度改成 `.large`（28pt），实机看
/// 高度不够，改回来了——搜索框是这面板上唯一的输入控件，28 在 32pt 的行列表
/// 上方压不住场。要换档就换下面那一行，**只有那一处**，别去动 frame。
///
...
///
/// 所以唯一有效的写法是在 SwiftUI 侧加 `.controlSize(...)`（见 `body`）：
/// 环境值本身就是那一档，回写反而替我们把值钉住。外框仍是系统自己画的
...
struct SidebarSearchField: View {
    ...
    var body: some View {
        Representable(text: $text, placeholder: placeholder)
            // 外框高度的开关就这一行（extraLarge = 36pt）；直接贴在 representable
            // 上，上游谁再设 controlSize 也盖不过来（环境取最近者）。
