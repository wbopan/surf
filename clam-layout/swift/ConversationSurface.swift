...
///
/// **协议住在消费者侧**（1×N 规则）：拥有 WebView 排版的是 clam-layout，
/// 所以协议定义在这里，随 `ClamLayout.swiftmodule` 传给下游。
/// SDK 只放内核词汇，这种生态词汇不进 SDK（计划 §4.1 结尾）。
///
/// 下游（clam-sidebar）从保管箱按 `ClamObjects.Key.conversationSurface` 取，
/// 转型成本协议使用。上游换代时下游会被级联重编，转型永远对得上。
public protocol ClamConversationSurface: AnyObject {
    /// 切换会话区显示的会话。
...
final class WebViewConversationSurface: ClamConversationSurface {
