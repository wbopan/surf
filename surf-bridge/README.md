...
1. **Swift 载荷登记表** —— `ctx.provide('surfBridge')`，各插件经
   `createSwiftPlugin`（子出口 `surf-bridge/plugin`）把自己的 `swift/` 目录登记进来。
2. **一条 WebSocket** —— `/surf/bridge`，与 dsh 同端口（`ctx.webServer.registerUpgrade`）。
...
同一个 hash 还决定 Swift module 名（`SurfSidebar_h9f31c0aa12b4`），
...
| ↑ | `restart-dsh` | — |
| ↓ | `app-build` | `status` + 详情。**壳的自更新通道**：surf-app 重建完壳源码后播一条，壳在右上角挂「有新版 · 重启 / 稍后」 |
| ↑ | `app-restart` | — 壳自请重启（发完自己退出，surf-app 等它死透再按新产物拉起） |

`app-build` / `app-restart` 是 surf-app 专属的一对（桥为它开了 `bridge.app` 子 API）。
可接受的特例：**surf-app 本质是壳的 node 半身**，不是普通插件。
...
| 出口 | 内容 | 谁用 |
|---|---|---|
| `.` | 桥本体（cordis 插件，`provide("surfBridge")`） | 编排表 |
| `./plugin` | `createSwiftPlugin` 工厂。**`CommandDeclaration` 的权威文档就在这个文件里** | 每个带 Swift 载荷的插件 |
| `./locale` | 语言决议的小工具（node 半边要出双语文案时用） | surf-app、surf-notify |

## 写一个带 Swift 载荷的插件

```js
// 本仓库内用相对路径；外部包用 import … from "@wenbo/surf-bridge/plugin"
import { createSwiftPlugin } from "../../surf-bridge/lib/plugin.js";

export default createSwiftPlugin({
  name: "surf-sidebar",             // ← 也是 Swift module 名的唯一出处（→ SurfSidebar）
  provide: "surf-sidebar",          // 空标记服务，供下游 inject（§4.3）
  inject: ["surf-layout"],          // cordis 依赖 = 挂载时序
  swiftDir: new URL("../swift/", import.meta.url),
  swiftDeps: ["surf-layout"],       // Swift module 依赖（源码里 import SurfLayout）
  sharedModules: [],                // 随 app bundle 分发的共享 module（SurfSDK 无需声明）
...
| 拦什么 | 提示 |
|---|---|
| `name` 推不出合法 Swift 标识符 | 用 kebab-case 裸名（`surf-sidebar`），**别拿 scoped 包名当 `name`** |
| `name` 重复登记 | 检查编排表里是不是列了两行 |
| `swiftDir` 不是目录 / 里面没有 `.swift` | 多半是 `package.json` 的 `files` 白名单漏了 `"swift"` |

### 解析方式

`surf-*` 之间用**相对路径 import**：`healProfilesModuleFallback` 只镜像 harness 的依赖闭包，
不含用户插件，包名 import 要么靠 npm workspace 要么靠手工 symlink——两者都是机器本地状态，
新克隆的仓库拿不到。相对路径在"所有 surf-\* 是同一仓库里的兄弟目录"这个前提下永远成立。

**外部插件没有这个问题**，用包名 `@wenbo/surf-bridge/plugin` 即可（写 peerDependencies）
——完整上手路径见 `docs/extend/plugin-author-guide.md`。

Swift 半身见 `surf-sidebar/swift/SidebarPlugin.swift`（最小的槽插件入口）与
`surf-app/host/Sources/SurfSDK/`（SDK 本体，含纪律说明）。
