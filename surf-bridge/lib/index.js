/**
 * surf-bridge —— 唯一的特权插件（阶段二计划 §5）。
 *
 * 它做三件事，一件不多：
 *
 *   1. **Swift 载荷登记表**：`ctx.provide('surfBridge', api)`，各插件经
 *      `createSwiftPlugin`（见 `./plugin.js`）把自己的 `swift/` 目录登记进来。
 *      同一张表还捎带插件的**命令声明**（菜单项 + 默认键位，形状见 `./plugin.js`）：
 *      桥不解释它，只透传给两个读者——壳（snapshot）与 surf-app（`commands.list()`）。
 *   2. **一条 WebSocket**（`/surf/bridge`，与 dsh 同端口）：壳连上来拉 snapshot、
 ...
 *
 * **级联重编是硬约束**（M2 断言 6 实测，见 `docs/extend/native-abi.md` §4）：上游插件源码
...
// `surf-sidebar` → `SurfSidebar`。单独成模块是因为构建脚本也要用同一份算法
...
export const Config = z.object({
	path: z.string().default("/surf/bridge")
		.description(LOCALE === "zh"
			? "WebSocket 升级路径（与 dsh 同端口）。surf-app 经 surfBridge.path 取这个值写进 endpoint 发现文件，改这里无需再同步别处。"
			: "WebSocket upgrade path, served on the dsh port. surf-app reads it back through surfBridge.path and writes it into the endpoint discovery file, so changing it here needs no matching change elsewhere."),
 ...
 * 一直到壳去 `swiftc -module-name @wenbo/surfFoo` 才炸，而那条错误落在壳的日志里、
...
export function apply(ctx, config) {
	const logger = reporter(ctx.logger("surf-bridge"));
	/** @type {Map<string, Registration>} 插件名 → 登记项 */
	const registry = new Map();
	/** @type {Set<Client>} 已握手的壳客户端 */
	const clients = new Set();
	/** @type {Set<() => void>} 登记表里命令声明变动时要通知的人（眼下只有 surf-app）。 */
	const commandWatchers = new Set();
	/** surf-app 登记的"壳请求重启自己"处理器（§7.5 v1）。 */
	...
	ctx.provide("surfBridge", {
		protocolVersion: PROTOCOL_VERSION,
		/**
		 * 本桥实际挂载的 WS 路径。**这是该路径的唯一真相**——挂 WS 的是这里，
		 * 而 `path` 又是用户可覆写的配置项。surf-app 取它写进 endpoint 发现文件
		 * 与 `--surf-bridge-path`，壳因此永远连得上，不管用户把它改成什么。
		 */
		path: config.path,
		 ...
		 *
		 * 壳走 snapshot 的 `commands` 字段拿同一份；这条 API 是给 **surf-app** 的
		 * ——它要按这张表现拼 `surf-shortcuts` 设置 ns 的 schema，而设置面不过桥。
		 ...
		 * （`SurfNode/<pkg>/swift/` → `<profile>/.surf/<pkg>/swift/`），用户把 App
		...
		register(entry) {
			...
			if (!MODULE_NAME_RE.test(module)) {
				throw new Error(`surf-bridge：插件 name ${JSON.stringify(entry.plugin)} 推不出合法的 `
					+ `Swift module 名（算出来是 ${JSON.stringify(module)}）。`
					+ `name 要用 kebab-case 的裸名（如 surf-sidebar → SurfSidebar），`
					+ `别拿 scoped 包名（@wenbo/surf-sidebar）当 name。`);
			}
			if (registry.has(entry.plugin)) {
				// 覆盖是**最坏的兼容**：两个插件重名时后者把前者的 swiftDir 顶掉，
				// 界面上少的那一块与日志里的 warn 隔着十万八千里。
				throw new Error(`surf-bridge：插件 name "${entry.plugin}" 重复登记 Swift 载荷。`
			...
			if (Object.keys(scanSwiftDir(entry.swiftDir).files).length === 0) {
				throw new Error(`surf-bridge：插件 "${entry.plugin}" 的 swiftDir 里一个 .swift 文件都没有：`
			...
			const record = {
				...
				// 这份源码会不会变（分发计划 §7.10）。分发载荷带 `.surf-static` 标记，
		...
		/**
		 * surf-app 专用通道（§7.5 v1）。壳的构建不是插件热替换那个档位——
	...
	function handleFrame(client, frame) {
		switch (frame?.type) {
			...
			case "app-restart":
				// 壳即将自己退出，让 surf-app 等它死透再拉起新产物（§7.5 v1）。
				if (appRestartHandler === undefined) {
					logger.warn("壳请求重启自己，但没有 surf-app 接管——它退出后不会被拉回来。");
					return;
				}
				logger.info("壳请求重启自己 —— 交给 surf-app 等待并重拉。");
	 ...
	 * `.surf-static` 标记，见 ./swift-payload.js）。一张全是分发载荷的表干脆没有
 ...
 * 桥的进度要给蹲在终端的人看，所以照 surf-app 的做法两边都喂。
 */
function reporter(logger) {
	const emit = (level, message) => {
		logger[level](message);
		process.stderr.write(`surf-bridge: ${message}\n`);
	};
	return {
